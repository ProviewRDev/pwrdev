...
static pwr_tStatus IoRackRead(io_tCtx ctx, io_sAgent* ap, io_sRack* rp)
{
  ...
  if (mp->Status == PB__NORMAL && sp->Super.DisableSlave != 1 && mp->DisableBus != 1)
  {
    ...
    if (data_len > 0)
    {
      for (ii = 0; ii < 4; ii++)
      {
        if (sp->Super.Diag[6 + 19 * ii] & 0x40)
        { // Segment DP1
          drs = &sp->DP1;
        }
        else if (sp->Super.Diag[6 + 19 * ii] & 0x20)
        { // Segment DP2
          drs = &sp->DP2;
        }
        else if (sp->Super.Diag[6 + 19 * ii] & 0x10)
        { // Segment DP3
          drs = &sp->DP3;
        }
        else if (sp->Super.Diag[6 + 19 * ii] & 0x80)
        { // Segment PG
          drs = &sp->PG;
        }
        else
  ...
  if (sp->Super.DisableSlave != 1 && mp->DisableBus != 1)
  {
    ...
    if (sp->Super.ErrorCount > sp->Super.ErrorSoftLimit &&
        sp->Super.StallAction >= pwr_ePbStallAction_ResetInputs)
    {
      memset(&sp->Super.Inputs, 0, sp->Super.BytesOfInput);
    }

    if (sp->Super.ErrorCount > sp->Super.ErrorHardLimit &&
        sp->Super.StallAction >= pwr_ePbStallAction_EmergencyBreak)
    {
...
pwr_dExport pwr_BindIoMethods(Siemens_DiagRepeater) = {pwr_BindIoMethod(IoRackRead), pwr_NullMethod};
