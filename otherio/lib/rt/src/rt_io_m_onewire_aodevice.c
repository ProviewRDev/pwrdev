...
static pwr_tStatus IoCardInit(io_tCtx ctx, io_sAgent* ap, io_sRack* rp, io_sCard* cp)
{
  ...
  if (cp->chanlist[0].cop)
  {
    ...
    if (EVEN(sts))
    {
      errh_Error("OneWire_AoDevice Unable op open %s, '%x'", cp->Name, op->Super.Address);
      op->Status = sts;
    }
    else
    {
...
static pwr_tStatus IoCardWrite(io_tCtx ctx, io_sAgent* ap, io_sRack* rp, io_sCard* cp)
{
  ...
  if (op->Status == IO__FILE)
  {
    ...
    {
      errh_Error("OneWire_AoDevice File open %s, '%x'", cp->Name, op->Super.Address);
      op->Status = sts;
    }
  }
...
pwr_dExport pwr_BindIoMethods(OneWire_AoDevice) = {pwr_BindIoMethod(IoCardInit),
