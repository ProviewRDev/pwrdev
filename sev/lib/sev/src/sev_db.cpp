...
sev_attr::sev_attr(const sev_attr& x) : type(x.type), size(x.size), elem(x.elem), ip(x.ip), refid(x.refid)
{
...
sev_item::sev_item()
    : deadband_active(0), last_id(0), value_size(0), old_value(0), first_storage(1), status(0),
      logged_status(0), cache(0), idx(0), deleted(0), mean_value(0), mean_acc_time(0), variance_acc(0),
      variance_cnt(0)
{
...
sev_item::sev_item(const sev_item& x)
    : id(x.id), oid(x.oid), creatime(x.creatime), modtime(x.modtime), storagetime(x.storagetime),
      sevid(x.sevid), scantime(x.scantime), deadband(x.deadband), options(x.options),
      deadband_active(x.deadband_active), last_id(x.last_id), value_size(x.value_size),
      old_value(x.old_value), first_storage(x.first_storage), attrnum(x.attrnum), attr(x.attr),
      status(x.status), logged_status(x.logged_status), cache(0), idx(x.idx), deleted(x.deleted),
      mean_value(x.mean_value), mean_acc_time(x.mean_acc_time), variance_cnt(x.variance_cnt)
{
...
int sev_db::check_item(pwr_tStatus* sts, pwr_tOid oid, char* oname, char* aname, pwr_tDeltaTime storatetime,
...
int sev_db::add_item(pwr_tStatus* sts, pwr_tOid oid, char* oname, char* aname, pwr_tDeltaTime storagetime,
...
int sev_db::store_value(pwr_tStatus* sts, void* thread, int item_idx, int attr_idx, pwr_tTime time, void* buf,
                        unsigned int size)
{
  *sts = 0;
  return 0;
...
int sev_db::get_values(pwr_tStatus* sts, void* thread, pwr_tOid oid, pwr_tMask options, float deadband,
...
int sev_db::delete_old_data(pwr_tStatus* sts, void* thread, char* tablename, pwr_tMask options,
...
int sev_db::check_objectitem(pwr_tStatus* sts, char* tablename, pwr_tOid oid, char* oname, char* aname,
...
int sev_db::add_objectitem(pwr_tStatus* sts, char* tablename, pwr_tOid oid, char* oname, char* aname,
                           pwr_tDeltaTime storagetime, char* description, pwr_tFloat32 scantime,
                           pwr_tFloat32 deadband, pwr_tMask options, unsigned int attrnum,
                           sev_sHistAttr* attr, unsigned int* idx)
{
...
int sev_db::store_objectitem(pwr_tStatus* sts, char* tablename, pwr_tOid oid, char* oname, char* aname,
                             pwr_tDeltaTime storagetime, char* description, pwr_tFloat32 scantime,
...
int sev_db::store_event(pwr_tStatus* sts, void* thread, int item_idx, sev_event* ep)
{
  *sts = 0;
  return 0;
}

int sev_db::get_item(pwr_tStatus* sts, void* thread, sev_item* item, pwr_tOid oid, char* attributename)
{
  *sts = 0;
  return 0;
}

int sev_db::get_objectitem(pwr_tStatus* sts, void* thread, sev_item* item, pwr_tOid oid, char* attributename)
{
  *sts = 0;
  return 0;
}
...
int sev_db::check_objectitemattr(pwr_tStatus* sts, char* tablename, pwr_tOid oid, char* aname, char* oname,
...
int sev_db::delete_old_objectdata(pwr_tStatus* sts, void* thread, char* tablename, pwr_tMask options,
                                  pwr_tTime limit, pwr_tFloat32 scantime, pwr_tFloat32 garbagecycle)
{
  *sts = 0;
  return 0;
...
int sev_db::get_objectvalues(pwr_tStatus* sts, void* thread, sev_item* item, unsigned int size,
...
int sev_db::handle_objectchange(pwr_tStatus* sts, char* tablename, unsigned int item_idx, bool newObject)
{
  *sts = 0;
  return 0;
}
...
int sev_db::store_stat(sev_sStat* stat) { return 0; }

int sev_db::begin_transaction(void* thread) { return 0; }

int sev_db::commit_transaction(void* thread) { return 0; }

char* sev_db::dbName() { return 0; }

void* sev_db::new_thread() { return 0; }
...
void sev_db::get_item_idx_by_name(pwr_tStatus* sts, unsigned int* item_idx, char* name)
{
  pwr_tAName aname;
  char* s;
...
int sev_db::get_events(pwr_tStatus* sts, void* thread, pwr_tOid oid, pwr_tMask options,
