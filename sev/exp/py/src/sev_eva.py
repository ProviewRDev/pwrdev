...
class FilterDialog:
    def __init__(self, parent, filter_cb):
        ...
        self.eventtextentry = ttk.Entry(
            top, textvariable=defaulteventtext, style='Pwr.TEntry')
        ...
        self.eventnameentry = ttk.Entry(
            top, textvariable=defaulteventname, style='Pwr.TEntry')
        self.eventnameentry.grid(column=1, row=row, padx=20, pady=5, sticky=W)
        row += 1

        button = ttk.Button(top, text="Ok", command=self.ok_cb,
                            width=10, style='Pwr.TButton')
        button.grid(column=0, row=row, padx=10, pady=5, sticky=W)

        button = ttk.Button(
            top, text="Cancel", command=self.cancel_cb, width=10, style='Pwr.TButton')
...
class ValueDialog:
    def __init__(self, parent, title, text, defaultstr, value_cb):
        ...
        self.entry = ttk.Entry(
            top, width=25, textvariable=self.defaultvalue, style='Pwr.TEntry')
...
class SelectDialog:
    ...
    def __init__(self, parent, itemlist, cb_func):
        ...
        button = ttk.Button(self.window, text='Ok', command=self.ok_cb,
                            style='Pwr.TButton').grid(row=1, column=0)
...
class EWData:
    ...
    def __init__(self):
        ...
        self.wd = None
        self.origdata = None
        self.origcol = ['Time', 'Type', 'Prio', 'Text',
                        'Name', 'SupObject', 'Id', 'Status']
    ...
    def read_file(self, file):
        ...
        self.origdata = pd.read_csv(
            file, parse_dates=[0], encoding="ISO-8859-1")
        self.origdata.columns = self.origcol
        self.wd = self.origdata.loc[:, :]
    ...
    def clip(self, c):
        self.wd = self.wd[(self.wd['Time'] >= c.mintime) &
                          (self.wd['Time'] <= c.maxtime)]
...
class TrendHistogram:
    def __init__(self, wdata):
        ...
        self.minentry = ttk.Entry(
            self.trenddia, textvariable=self.defaultmin, width=25, style='Pwr.TEntry')
        ...
        self.maxentry = ttk.Entry(
            self.trenddia, textvariable=self.defaultmax, width=25, style='Pwr.TEntry')
        ...
        binslabel = ttk.Label(
            self.trenddia, text='Number of bins', style='Pwr.TLabel')
        binslabel.grid(column=0, row=2, padx=20, pady=5, sticky=W)
        self.binsentry = ttk.Entry(
            self.trenddia, textvariable=self.defaultbins, width=25, style='Pwr.TEntry')
        ...
        readdatabutton = ttk.Button(
            self.trenddia, text="Apply", command=self.trend_action_ok_cb, style='Pwr.TButton')
        readdatabutton.grid(column=0, row=4, padx=60, pady=20, sticky=W)

        readdatabutton = ttk.Button(
            self.trenddia, text="Cancel", command=self.trend_action_cancel_cb, style='Pwr.TButton')
    ...
    def trend_action_ok_cb(self):
        try:
            mintime = datetime.strptime(
                self.minentry.get(), '%Y-%m-%d %H:%M:%S.%f')
        except ValueError:
            try:
                mintime = datetime.strptime(
                    self.minentry.get(), '%Y-%m-%d %H:%M:%S')
            except ValueError:
                tkinter.messagebox.showerror("Error", "Time syntax error")
                return
        try:
            maxtime = datetime.strptime(
                self.maxentry.get(), '%Y-%m-%d %H:%M:%S.%f')
        except ValueError:
            try:
                maxtime = datetime.strptime(
                    self.maxentry.get(), '%Y-%m-%d %H:%M:%S')
            except ValueError:
                tkinter.messagebox.showerror("Error", "Time syntax error")
                return
        try:
...
class FrequencyHistogram:
    def __init__(self, wdata):
        ...
        while i < len(self.counts):
            res = self.wdata.wd.loc[(
                self.wdata.wd['SupObject'] == self.counts.index[i]) & self.wdata.wd['Type'].isin(types)]
            if len(res) > 0:
                res = res.iloc[0]
        ...
        bprev = matplotlib.widgets.Button(
            axprev, 'Previous', color=bgcolor, hovercolor=bgcolor)
        bprev.on_clicked(self.bprev_cb)
        bnext = matplotlib.widgets.Button(
            axnext, 'Next', color=bgcolor, hovercolor=bgcolor)
        bnext.on_clicked(self.bnext_cb)
        bdisplay = matplotlib.widgets.Button(
            axdisplay, 'Display', color=bgcolor, hovercolor=bgcolor)
        bname = matplotlib.widgets.Button(
            axname, 'Name', color=bgcolor, hovercolor=bgcolor)
        bname.on_clicked(self.bname_cb)
        btext = matplotlib.widgets.Button(
            axtext, 'Text', color=bgcolor, hovercolor=bgcolor)
        btext.on_clicked(self.btext_cb)
        axrows = plt.axes([0.25, 0.01, 0.1, 0.05])
        self.brows = matplotlib.widgets.Button(
            axrows, '', color=bgcolor, hovercolor=bgcolor)
        self.draw()
        plt.show()
...
class WdWindow:
    ...
    def no_of_alarms_plot(self, plottype):
        ...
        ser = pd.DataFrame(
            columns=['Time', 'A', 'B', 'C', 'D', 'Info', 'InfoSucc'])
        j = 0
        k = 0
        a_alarms = []
        b_alarms = []
        ...
        while j < len(self.wdata.wd):
            ...
            if type == Ev.eEvent_Alarm or type == Ev.eEvent_MaintenanceAlarm or \
               ...
               type == Ev.eEvent_UserAlarm3 or type == Ev.eEvent_UserAlarm4:
                ...
                infosucc_msgs = [
                    i for i in infosucc_msgs if not i.startswith(vid)]
                ...
                res = self.wdata.wd.loc[(self.wdata.wd['SupObject'] == self.wdata.wd['SupObject'][j]) & self.wdata.wd['Type'].isin(
                    types) & (self.wdata.wd['Time'] < self.wdata.wd['Time'][j])]
                if len(res) > 0:
                    res = res.iloc[-1]
                    atype = res['Type']
        ...
        if plottype == 0:
            fig.canvas.set_window_title(
                'Number of not acknowledged alarms ' + self.name)
            plt.title('Not acknowledged alarms')
        else:
            fig.canvas.set_window_title(
                'Number of not returned alarms ' + self.name)
        ...
        plt.plot(ser['Time'], ser['A'] + ser['B'] +
                 ser['C'] + ser['D'], 'r', label='A')
    ...
    def frequency_table_action_cb(self):
        ...
        tree["columns"] = ("Frequency", "Type", "Prio",
                           "Text", "Name", "SupObject")
        ...
        while i < len(counts):
            res = self.wdata.wd.loc[(
                self.wdata.wd['SupObject'] == counts.index[i]) & self.wdata.wd['Type'].isin(types)]
            if len(res) > 0:
                res = res.iloc[0]
...
#        plt.show()
                ...
                if self.wdata.wd['Text'][i].lower().find(searchstr) >= 0 or \
                   self.wdata.wd['Name'][i].lower().find(searchstr) >= 0:
                    ...
                    self.tree.yview_moveto(
                        float((0 if i < 5 else i - 5))/len(self.wdata.wd))
    ...
    def clip_action_cb(self):
        ...
        minlabel.grid(column=0, row=0, padx=20, pady=5, sticky=W)
        self.minentry = ttk.Entry(
            self.clipdia, textvariable=self.defaultmin, width=25, style='Pwr.TEntry')
        ...
        maxlabel.grid(column=0, row=1, padx=20, pady=5, sticky=W)
        self.maxentry = ttk.Entry(
            self.clipdia, textvariable=self.defaultmax, width=25, style='Pwr.TEntry')
        self.defaultmax.set(str(self.wdata.wd['Time'][0]))
        self.maxentry.grid(column=1, row=1, padx=20, pady=5, sticky=W)

        readdatabutton = ttk.Button(
            self.clipdia, text="Apply", command=self.clip_action_ok_cb, style='Pwr.TButton')
        readdatabutton.grid(column=0, row=2, padx=60, pady=20, sticky=W)

        readdatabutton = ttk.Button(
            self.clipdia, text="Cancel", command=self.clip_action_cancel_cb, style='Pwr.TButton')
    ...
    def split_action_cb(self):
        ...
        ValueDialog(self.window, "Split Time", "Time",
                    defaultvalue, self.split_ok_cb)
    ...
    def filteronsupobject_action_cb(self):
        ...
        filter = EventFilter(
            0, 0, '', '', self.tree.item(selected)['values'][4])
        self.wdata.filter(filter)
    ...
    def deletecolumn_action_cb(self):
        ...
        answer = tkinter.messagebox.askquestion(
            'Delete columns', 'Do you want to delete the selected columns')
        if answer == 'yes':
            mask = [None] * len(self.datasel)
            i = 0
            for sel in self.datasel:
    ...
    def create_datamenu(self):
        ...
        filemenu.add_command(
            label='New', command=self.new_action_cb, accelerator='Ctrl+N')
        filemenu.add_command(
            label='Open', command=self.open_action_cb, accelerator='Ctrl+O')
        filemenu.add_command(label='Import from server',
                             command=self.fetch_sev_cb)
        filemenu.add_command(label='Save', command=self.savewd_action_cb)
        filemenu.add_command(label='Save Original Data',
                             command=self.saveorig_action_cb)
        ...
        filemenu.add_command(
            label='Close', command=self.close_action_cb, accelerator='Ctrl+W')
        menubar.add_cascade(label='File', menu=filemenu)

        editmenu = Menu(menubar, bg=menucolor, fg=menufgcolor,
                        activebackground=menuactivecolor, activeforeground=menufgcolor)
        editmenu.add_command(
            label='Search', command=self.search_action_cb, accelerator='Ctrl+F')
        editmenu.add_command(
            label='Search Next', command=self.searchnext_action_cb, accelerator='Ctrl+G')
        editmenu.add_command(
            label='Search Previous', command=self.searchprev_action_cb, accelerator='Shift+Ctrl+G')
        editmenu.add_command(label='Clip', command=self.clip_action_cb)
        editmenu.add_command(label='Crop not selected',
                             command=self.clip_select_action_cb)
        ...
        editmenu.add_command(label='Filter on SupObject',
                             command=self.filteronsupobject_action_cb)
        ...
        viewmenu = Menu(menubar, bg=menucolor, fg=menufgcolor,
                        activebackground=menuactivecolor, activeforeground=menufgcolor)
        viewmenu.add_command(label='Event frequency table',
                             command=self.frequency_table_action_cb)
        viewmenu.add_command(label='Event frequency histogram',
                             command=self.frequency_histogram_action_cb)
        viewmenu.add_command(label='Trend histogram',
                             command=self.trend_histogram_action_cb)
        viewmenu.add_command(
            label='Number of not acked alarms plot', command=self.alarmplot_ack_action_cb)
        viewmenu.add_command(label='Number of not returned alarms plot',
                             command=self.alarmplot_notret_action_cb)
        ...
        self.window.bind("<Control-Shift-Control-G>",
                         self.searchprev_action_cb)

    def create_toolbar(self):

        self.toolbar = Frame(self.window, bg=bgcolor, bd=1, relief=RAISED)

        self.toolbar_revert_img = PhotoImage(
            file=pwr_exe+"/evatoolbar_revert.png")
        button = ttk.Button(self.toolbar, image=self.toolbar_revert_img,
                            command=self.revert_action_cb, style='Pwr.TButton')
        wow.Tooltip(button, text='Revert', bg=bgcolor)
        button.pack(side=LEFT, padx=2, pady=2)

        self.toolbar_prev_img = PhotoImage(file=pwr_exe+"/evatoolbar_prev.png")
        button = ttk.Button(self.toolbar, image=self.toolbar_prev_img,
                            command=self.previous_action_cb, style='Pwr.TButton')
        wow.Tooltip(button, text='Back', bg=bgcolor)
        button.pack(side=LEFT, padx=2, pady=2)

        self.toolbar_crop_img = PhotoImage(file=pwr_exe+"/evatoolbar_crop.png")
        button = ttk.Button(self.toolbar, image=self.toolbar_crop_img,
                            command=self.clip_select_action_cb, style='Pwr.TButton')
        wow.Tooltip(button, text='Crop not selected', bg=bgcolor)
        button.pack(side=LEFT, padx=2, pady=2)

        self.toolbar_filter_img = PhotoImage(
            file=pwr_exe+"/evatoolbar_filter.png")
        button = ttk.Button(self.toolbar, image=self.toolbar_filter_img,
                            command=self.filter_action_cb, style='Pwr.TButton')
        wow.Tooltip(button, text='Filter', bg=bgcolor)
        button.pack(side=LEFT, padx=2, pady=2)

        self.toolbar_filteralarms_img = PhotoImage(
            file=pwr_exe+"/evatoolbar_filteralarms.png")
        button = ttk.Button(self.toolbar, image=self.toolbar_filteralarms_img,
                            command=self.filteralarms_action_cb, style='Pwr.TButton')
        wow.Tooltip(button, text='Alarm and info message filter', bg=bgcolor)
        button.pack(side=LEFT, padx=2, pady=2)

        self.toolbar_filtersup_img = PhotoImage(
            file=pwr_exe+"/evatoolbar_filtersup.png")
        button = ttk.Button(self.toolbar, image=self.toolbar_filtersup_img,
                            command=self.filteronsupobject_action_cb, style='Pwr.TButton')
        wow.Tooltip(button, text='SupObject filter', bg=bgcolor)
        button.pack(side=LEFT, padx=2, pady=2)

        self.toolbar_plot_img = PhotoImage(file=pwr_exe+"/evatoolbar_plot.png")
        button = ttk.Button(self.toolbar, image=self.toolbar_plot_img,
                            command=self.alarmplot_notret_action_cb, style='Pwr.TButton')
        wow.Tooltip(
            button, text='Number of not returned alarms plot', bg=bgcolor)
        button.pack(side=LEFT, padx=2, pady=2)

        self.toolbar_frequencyplot_img = PhotoImage(
            file=pwr_exe+"/evatoolbar_frequencyplot.png")
        button = ttk.Button(self.toolbar, image=self.toolbar_frequencyplot_img,
                            command=self.frequency_histogram_action_cb, style='Pwr.TButton')
    ...
    def create_dataframe(self):
        ...
        self.img_evsystemyellow = PhotoImage(
            file=pwr_exe+"/eva_evsystemyellow.png")
        self.img_evsystemblue = PhotoImage(
            file=pwr_exe+"/eva_evsystemblue.png")
        self.img_evsystemviolet = PhotoImage(
            file=pwr_exe+"/eva_evsystemviolet.png")
        self.img_evmaintred = PhotoImage(file=pwr_exe+"/eva_evmaintred.png")
        self.img_evmaintyellow = PhotoImage(
            file=pwr_exe+"/eva_evmaintyellow.png")
        self.img_evmaintblue = PhotoImage(file=pwr_exe+"/eva_evmaintblue.png")
        self.img_evmaintviolet = PhotoImage(
            file=pwr_exe+"/eva_evmaintviolet.png")
        ...
        self.tree["columns"] = ("Type", "Prio", "Text",
                                "Name", "SupObject", "Id", "Status")
        ...
        scroll = ttk.Scrollbar(
            self.dataframe, orient="vertical", command=self.tree.yview)
        scroll.pack(anchor=E, expand=1, side=RIGHT, fill=Y)
        ...
        while j < len(self.wdata.wd):
            ...
            self.tree.insert('', 'end', iid=str(j), text=str(self.wdata.wd.iat[j, 0]),
                             ...
                             values=(Ev.eventtype_to_str(self.wdata.wd.iat[j, 1]),
                                     Ev.eventprio_to_str(
                                         self.wdata.wd.iat[j, 2]),
                                     '' if not isinstance(
                                         self.wdata.wd.iat[j, 3], str) else self.wdata.wd.iat[j, 3],
                                     self.wdata.wd.iat[j, 4],
                                     self.wdata.wd.iat[j, 5], str(
                                         self.wdata.wd.iat[j, 6]),
...
class FetchSev:
    ...
    def create_filterframe(self):
        ...
        serverlabel = ttk.Label(
            self.filterframe, text='Sev server', style='Pwr.TLabel')
        serverlabel.grid(column=0, row=0, padx=20, pady=5, sticky=W)
        self.serverentry = ttk.Entry(
            self.filterframe, textvariable=self.defaultserver, style='Pwr.TEntry')
        self.defaultserver.set(srv)
        self.serverentry.grid(column=1, row=0, padx=20, pady=5, sticky=W)

        # Filter entry
        filterlabel = ttk.Label(
            self.filterframe, text='Item Filter', style='Pwr.TLabel')
        ...
        # Fetch items button
        filterbutton = ttk.Button(
            self.filterframe, text='Fetch Items', command=self.fetchitems_cb, style='Pwr.TButton')
    ...
    def fetchitems_cb(self):
        ...
        self.canvas = Canvas(self.itemframe, bd=0,
                             bg=bgcolor, yscrollcommand=scrollbar.set)
        ...
        self.fromentry = ttk.Entry(
            valframe, textvariable=self.defaultfrom, style='Pwr.TEntry')
        ...
        self.toentry = ttk.Entry(
            valframe, textvariable=self.defaultto, style='Pwr.TEntry')
        ...
        # EventType entry
        eventtypelabel = ttk.Label(
            valframe, text='EventType', style='Pwr.TLabel')
        eventtypelabel.grid(column=0, row=row, padx=0, pady=5, sticky=W)
        ...
        # EventPrio entry
        eventpriolabel = ttk.Label(
            valframe, text='EventPrio', style='Pwr.TLabel')
        eventpriolabel.grid(column=0, row=row, padx=0, pady=5, sticky=W)
        ...
        self.maxentry = ttk.Entry(
            valframe, textvariable=self.defaultmax, style='Pwr.TEntry')
        self.defaultmax.set('500')
        self.maxentry.grid(column=1, row=row, padx=20, pady=5, sticky=W)
        row += 1

        readdatabutton = ttk.Button(
            valframe, text="Read DataSet", command=self.readdata_cb, style='Pwr.TButton')
        ...
        for item in self.items:
            self.sel[i] = IntVar()
            text = item[0]
            item_checkbox = ttk.Checkbutton(
                self.iteminnerframe, text=text, variable=self.sel[i], style='Pwr.TCheckbutton')
...
class MvMain:
    ...
    def __init__(self):
        ...
        bgcolor, fgcolor, menucolor, menufgcolor, menuactivecolor = wow.style(
            colortheme)
        window.withdraw()
        wdwindow = WdWindow()
        if file != '':
