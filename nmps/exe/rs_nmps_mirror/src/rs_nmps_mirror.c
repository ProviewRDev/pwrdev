...
static pwr_tStatus nmpsmir_find_collect_cell(mir_ctx mirctx, pwr_tObjid collect_cell,
                                             pwr_sClass_NMpsCell** collect_cell_ptr);
pwr_tStatus nmpsmir_data_db_delete(nmpsmir_t_data_list** data_list, nmpsmir_t_data_list* data_ptr);
...
static pwr_tStatus nmpsmir_data_db_create(mir_ctx mirctx, nmpsmir_t_data_list** data_list, pwr_tObjid objid,
                                          pwr_tObjid mircell_objid, int convert,
                                          pwr_sClass_NMpsConvConfig* convconfig,
                                          nmpsmir_t_data_list** datalist_ptr);
static pwr_tStatus nmpsmir_data_db_find_orig(nmpsmir_t_data_list* data_list, pwr_tObjid objid,
                                             pwr_tObjid mircell_objid, nmpsmir_t_data_list** datalist_ptr);
static pwr_tStatus nmpsmir_data_db_find_orig_name(nmpsmir_t_data_list* data_list, pwr_tObjid objid,
                                                  pwr_tObjid mircell_objid,
                                                  nmpsmir_t_data_list** datalist_ptr);
...
static pwr_tStatus nmpsmir_convconfiglist_add(pwr_tObjid objid, nmpsmir_t_convconfig_list** convconfiglist,
 ...
 *
 **************************************************************************/
static pwr_tStatus nmpsmir_bckremoved_store(nmpsmir_t_bckremoved** bckremlist, pwr_tObjid objid,
 ...
 *
 **************************************************************************/
static int nmnps_time_to_release(pwr_tTime* remove_time, pwr_tTime* current_time, float release_time)
{
  /***********
          pwr_tTime	diff_time;
          float		time;
...
static pwr_tStatus nmpsmir_data_copy(nmpsmir_t_data_list* datalist_ptr)
{
  ...
  if (!datalist_ptr->local)
  {
    ...
    from_ptr = datalist_ptr->orig_data_ptr + datalist_ptr->convconfig->CopyOffset;
...
static pwr_tStatus nmpsmir_data_handler(mir_ctx mirctx)
{
  ...
  pwr_sClass_NMpsCell* collect_cell_ptr = NULL;
  nmpsmir_t_bckremoved* bckremoved_ptr;
  ...
  while (data_ptr != NULL)
  {
    ...
    if (data_ptr->removed)
    {
      ...
      if (nmnps_time_to_release(&data_ptr->remove_time, &current_time, data_ptr->release_time))
      {
        mirctx->RemoveDataCount++;
        if (data_ptr->collect)
        {
          /* Put into collect cell */
          sts = nmpsmir_find_collect_cell(mirctx, data_ptr->collect_cell, &collect_cell_ptr);
          if (EVEN(sts))
  ...
  {
    /* Check time since remove */
    if (nmnps_time_to_release(&bckremoved_ptr->remove_time, &current_time, bckremoved_ptr->release_time))
    {
      mirctx->RemoveDataCount++;

      /* Put into collect cell */
      sts = nmpsmir_find_collect_cell(mirctx, bckremoved_ptr->collect_cell, &collect_cell_ptr);
      if (EVEN(sts))
        errh_CErrLog(sts, NULL);
                                          ...
                                          nmpsmir_t_data_list** datalist_ptr)
{
  ...
  *data_list = *datalist_ptr;
  ...
  if (convert)
  {
    ...
    sts = gdh_CreateObject(new_name, cdh_ClassObjidToId(convconfig->OutDataClass), 0,
    ...
    if (local)
    {
      ...
      sts = gdh_DLRefObjectInfoAttrref(&attrref, (pwr_tAddress*)&(*datalist_ptr)->orig_data_ptr,
    ...
    else
    {
      ...
      sts = gdh_ClassAttrToAttrref(cdh_ClassObjidToId(convconfig->OutDataClass), NULL, &attrref);
      if (EVEN(sts))
        LogAndReturn(NMPS__MIRORIGDATA, sts);
      ...
      **/
      sts = gdh_SubRefObjectInfoAttrref(&attrref, &(*datalist_ptr)->orig_data_subid);
      if (EVEN(sts))
        LogAndReturn(NMPS__MIRORIGDATA, sts);
    ...
    sts = gdh_DLRefObjectInfoAttrref(&attrref, (pwr_tAddress*)&(*datalist_ptr)->data_ptr,
 ...
 *
 **************************************************************************/
static pwr_tStatus nmpsmir_data_db_find(nmpsmir_t_data_list* data_list, pwr_tObjid objid,
                                        pwr_tObjid mircell_objid, nmpsmir_t_data_list** datalist_ptr)
{
  nmpsmir_t_data_list* data_ptr;
  int found;
 ...
 *
 **************************************************************************/
static pwr_tStatus nmpsmir_origcell_init(pwr_tObjid objid, char** orig_cell, gdh_tSubid* orig_subid,
                                         pwr_tBoolean* orig_local, pwr_tClassId* orig_class)
{
  pwr_sAttrRef attrref;
  pwr_tStatus sts;
  pwr_tAName name;
  ...
  if (*orig_local)
  {
    ...
    sts = gdh_DLRefObjectInfoAttrref(&attrref, (pwr_tAddress*)orig_cell, orig_subid);
    if (EVEN(sts))
      LogAndReturn(NMPS__MIRORIGCELL, sts);
  }
  else
                                           ...
                                           nmpsmir_t_cellmir_list** cellmirlist, int* cellmirlist_count)
{
  ...
  {
    new_cellmirlist = calloc(*cellmirlist_count + 1, sizeof(nmpsmir_t_cellmir_list));
    if (new_cellmirlist == 0)
      return NMPS__NOMEMORY;
    memcpy(new_cellmirlist, *cellmirlist, *cellmirlist_count * sizeof(nmpsmir_t_cellmir_list));
    free(*cellmirlist);
  ...
  attrref = cdh_ObjidToAref(objid);
  sts = gdh_DLRefObjectInfoAttrref(&attrref, (pwr_tAddress*)&cellmirlist_ptr->cell, &cellmirlist_ptr->subid);
  if (EVEN(sts))
  ...
  if (cellmirlist_ptr->data_collect)
  {
    ...
    sts = gdh_DLRefObjectInfoAttrref(&attrref, (pwr_tAddress*)&cellmirlist_ptr->collect_cell_ptr,
  ...
  for (i = 0; i < (int)cellmirlist_ptr->cell->NumberOfCellObj; i++)
  {
    ...
    sts = nmpsmir_origcell_init(cellmirlist_ptr->orig_objid[i], (char**)&cellmirlist_ptr->orig_cell[i],
                                &cellmirlist_ptr->orig_subid[i], &cellmirlist_ptr->orig_local[i],
  ...
  if (cellmirlist_ptr->conversion)
  {
    ...
    {
      if (cdh_ObjidIsEqual(convconf_ptr->objid, cellmirlist_ptr->cell->ConvConfig))
      {
                                              ...
                                              int* convconfiglist_count)
{
  ...
  {
    new_convconfiglist = calloc(*convconfiglist_count + 1, sizeof(nmpsmir_t_convconfig_list));
    if (new_convconfiglist == 0)
      return NMPS__NOMEMORY;
    memcpy(new_convconfiglist, *convconfiglist, *convconfiglist_count * sizeof(nmpsmir_t_convconfig_list));
    free(*convconfiglist);
  ...
  /* Direct link to the cell */
  attrref = cdh_ObjidToAref(objid);
  sts = gdh_DLRefObjectInfoAttrref(&attrref, (pwr_tAddress*)&convconfiglist_ptr->convconfig,
                                   &convconfiglist_ptr->subid);
  if (EVEN(sts))
    return sts;

  /* Check syntax of the ConvConfig object */
  sts = gdh_GetObjectClass(convconfiglist_ptr->convconfig->OutDataClass, &class);
  if (EVEN(sts))
  {
    errh_CErrLog(NMPS__CONVOUTDATACLASS, errh_ErrArgL(objid.oix), NULL);
    return NMPS__CONVOUTDATACLASS;
  }
  sts = gdh_GetObjectClass(convconfiglist_ptr->convconfig->OutDataParent, &class);
  if (EVEN(sts))
  {
...
static pwr_tStatus nmps_cellmir_init(mir_ctx mirctx)
{
  ...
  /* Direct link to the cell */
  attrref = cdh_ObjidToAref(objid);
  sts = gdh_DLRefObjectInfoAttrref(&attrref, (pwr_tAddress*)&mirctx->mirrorconfig, &mirctx->mirrorconf_dlid);
  if (EVEN(sts))
    return sts;
  ...
  while (ODD(sts))
  {
    ...
    sts = nmpsmir_convconfiglist_add(objid, &mirctx->convconfiglist, &mirctx->convconfig_count);
    if (EVEN(sts))
      return sts;
  ...
  while (ODD(sts))
  {
    ...
    sts = nmpsmir_cellmirlist_add(mirctx, objid, &mirctx->cellmirlist, &mirctx->cellmir_count);
    if (EVEN(sts))
      return sts;
  ...
  for (i = 0; i < mirctx->cellmir_count; i++)
  {
    if (!cellmir_ptr->initialized)
    {
      ...
      for (j = 0; j < cellmir_ptr->orig_cell_count; j++)
      {
        ...
        {
          sts = nmpsmir_origcell_init(cellmir_ptr->orig_objid[j], (char**)&cellmir_ptr->orig_cell[j],
                                      &cellmir_ptr->orig_subid[j], &cellmir_ptr->orig_local[j],
...
static pwr_tStatus nmpsmir_cellmir_handler(mir_ctx mirctx)
{
  ...
  for (i = 0; i < mirctx->cellmir_count; i++)
  {
    ...
    else
    {
      ...
      for (j = 0; j < cellmir_ptr->data_count; j++)
      {
        ...
        if (!cellmir_ptr->data_noinit[j])
        {
          ...
          {
            if (cdh_ObjidIsEqual(copyarea->datainfo[count].DataP.Aref.Objid, cellmir_ptr->data_objid[j]))
            {
              cellmir_ptr->datalist_ptr[j]->plc_data_ptr = (char*)copyarea->datainfo[count].DataP.Ptr;
              cellmir_ptr->datalist_ptr[j]->plc_dlid = copyarea->datainfo[count].Data_Dlid;
            }
          }
          count++;
        }
    ...
    for (j = 0; j < cellmir_ptr->orig_cell_count; j++)
    {
      ...
      {
        old = 0;
        sts = gdh_SubData(cellmir_ptr->orig_subid[j], orig_cellbuf, sizeof(orig_cellbuf));
        if (EVEN(sts))
          old = 1;
        /*	      sts = gdh_GetSubscriptionOldness(
        ...
        */
        if (old)
        {
          if (!cellmir_ptr->connect_error_logged[j])
          {
            ...
            errh_CErrLog(NMPS__REMCELLDOWN, errh_ErrArgL(cellmir_ptr->orig_objid[j].oix), errh_ErrArgMsg(sts),
                         NULL);
        ...
        {
          mirctx->OrigCellDownCount--;
          errh_CErrLog(NMPS__REMCELLUP, errh_ErrArgL(cellmir_ptr->orig_objid[j].oix), NULL);
      ...
      case pwr_cClass_NMpsStoreCell:
      {
        ...
        if (orig_cell_ptr->MirrorRestart && !cellmir_ptr->orig_restart_old[j])
        {
          ...
          /* Reset the orig cell */
          sts = gdh_ObjidToName(cellmir_ptr->orig_objid[j], name, sizeof(name), cdh_mNName);
          if (ODD(sts))
          {
        ...
        for (k = 0; k < orig_cell_ptr->LastIndex; k++)
        {
          ...
          if (j > 0)
          {
            ...
            {
              if (cdh_ObjidIsEqual(cellmir_ptr->orig_data_objid[l], data_block_ptr->DataP.Aref.Objid))
              {
                objid_found = 1;
                break;
              }
          ...
          if (!objid_found)
          {
            cellmir_ptr->orig_data_objid[orig_data_count] = data_block_ptr->DataP.Aref.Objid;
            orig_data_count++;
          }
          data_block_ptr++;
        }
    ...
    if (cellmir_ptr->restart && cellmir_ptr->conversion)
    {
      ...
      {
        sts = nmpsmir_data_db_find_orig_name(mirctx->data_list, cellmir_ptr->orig_data_objid[j],
                                             cellmir_ptr->objid, &datalist_ptr);
        if (sts == NMPS__NODEDOWN)
        {
        ...
        {
          errh_CErrLog(NMPS__DATARESTART, errh_ErrArgAF(datalist_ptr->orig_data_name), NULL);
    ...
    for (j = 0; j < orig_data_count; j++)
    {
      ...
      sts = nmpsmir_data_db_find_orig(mirctx->data_list, cellmir_ptr->orig_data_objid[j], cellmir_ptr->objid,
                                      &datalist_ptr);
      if (sts == NMPS__DATANOTFOUND)
      {
        /* Insert the object */
        sts = nmpsmir_data_db_create(mirctx, &mirctx->data_list, cellmir_ptr->orig_data_objid[j],
                                     cellmir_ptr->objid, cellmir_ptr->conversion, cellmir_ptr->convconfig,
                                     &datalist_ptr);
        ...
        else if (sts == NMPS__DATAALREXIST && mirctx->first_scan)
        {
          ...
          {
            if (cdh_ObjidIsEqual(copyarea->datainfo[k].DataP.Aref.Objid, datalist_ptr->data_objid))
            {
              datalist_ptr->plc_data_ptr = (char*)copyarea->datainfo[k].DataP.Ptr;
    ...
    if (cellmir_ptr->cell->TempLastIndex == orig_data_count)
    {
      ...
      {
        if (cdh_ObjidIsNotEqual(copyarea->datainfo[j].DataP.Aref.Objid, cellmir_ptr->data_objid[j]))
        {
          change_detected = 1;
          break;
        }
    ...
    for (j = 0; j < cellmir_ptr->cell->TempLastIndex; j++)
    {
      ...
      {
        if (cdh_ObjidIsEqual(copyarea->datainfo[j].DataP.Aref.Objid, cellmir_ptr->data_objid[k]))
        {
          found = 1;
          break;
        }
      ...
      if (!found)
      {
        ...
        sts = nmpsmir_data_db_find(mirctx->data_list, copyarea->datainfo[j].DataP.Aref.Objid,
                                   cellmir_ptr->objid, &datalist_ptr);
        if (EVEN(sts))
        {
          if (cellmir_ptr->cell->InitFlag && (cellmir_ptr->cell->Function & NMPS_CELLFUNC_BACKUP) &&
              cellmir_ptr->cell->ReloadDone && cellmir_ptr->data_collect)
          {
            ...
            sts = nmpsmir_bckremoved_store(&mirctx->bckremoved_list, copyarea->datainfo[j].DataP.Aref.Objid,
                                           &bckremlist_ptr);
            if (ODD(sts))
            {
    ...
    else if (change_detected)
    {
      ...
      for (j = 0; j < orig_data_count; j++)
      {
        ...
        if (!cellmir_ptr->data_noinit[j])
        {
          ...
          copyarea->datainfo[count].DataP.Ptr = (pwr_tFloat32*)cellmir_ptr->datalist_ptr[j]->plc_data_ptr;
          copyarea->datainfo[count].Data_Dlid = cellmir_ptr->datalist_ptr[j]->plc_dlid;
          count++;
        }
      }
