 ...
 *
 * Beskrivning:
 *	H�mta pekare till backup konfig objektet.
 *
 **************************************************************************/
...
static pwr_tInt32 ssabutil_chksum_calculate(pwr_tInt32 value, pwr_tInt16* weights, pwr_tInt16 num_figures)
{
  ...
  /* Ber�kna perkaren till entalssiffran */

  weightP = weights + num_figures - 1;

  /* Ber�kna den viktade summan av siffrorona i talet */
  ...
  /* Ber�kna skillnaden till n�rmast h�gre tiotal */
  sum = ((sum / 10) + 1) * 10 - sum;
  if (sum == 10)
    sum = 0;
