...
void pn_gsdml::_build_diagnostics()
{
  for (pugi::xml_node& chanDiagItem : _xmlChannelDiagList->children("ChannelDiagItem"))
    _channelDiagList.emplace(std::make_pair(chanDiagItem.attribute("ErrorType").as_uint(), GSDML::ChannelDiagItem(std::move(chanDiagItem), this)));
}
...
int pn_gsdml::read(const char* filename)
{
  ...
  dcli_translate_filename(_filename, _filename);

  _doc.reset(new pugi::xml_document);

  pugi::xml_parse_result result = _doc->load_file(_filename);
...
int pn_gsdml::string_to_value_datatype(char const* str, gsdml_eValueDataType* type)
{
  if (streq(str, "Bit"))
...
gsdml_DeviceAccessPointItem::gsdml_DeviceAccessPointItem(pn_gsdml* g)
    : ModuleInfo(0), CertificationInfo(0), SubslotList(0), IOConfigData(0),
      UseableModules(0), VirtualSubmoduleList(0), SystemDefinedSubmoduleList(0),
      Graphics(0), ApplicationRelations(0), UseableSubmodules(0), SlotList(0),
      SlotGroups(0), SystemRedundancy(0), gsdml(g)
{
  memset(&Body, 0, sizeof(Body));
}
...
void gsdml_ExtChannelDiagItem::build()
{
  if (!streq(Body.Name.ref, ""))
  {
    Body.Name.p = gsdml->find_text_ref(Body.Name.ref);
...
void gsdml_ExtChannelDiagList::build()
{
  for (auto& ext_channel_diag_item : ExtChannelDiagItem)
  {
    ext_channel_diag_item->build();
  }
}
...
void pn_gsdml::_build_moduleList(pugi::xml_node&& dataAccessPointItem)
{
  for (pugi::xml_node& moduleItemRef : dataAccessPointItem.child("UseableModules").children("ModuleItemRef"))
  {
    ...
    _moduleList[std::move(ID)].reset(new GSDML::ModuleItem(std::move(result), this)); 
  }
}
...
void pn_gsdml::_build_DAPs()
{
  for (pugi::xml_node& deviceAccessPointItem : _xmlDeviceAccessPointList->children("DeviceAccessPointItem"))
  {
    ...
    _deviceAccessPointList[std::move(ID)].reset(new GSDML::DeviceAccessPointItem(std::move(deviceAccessPointItem), this)); 
  } 
}
...
void pn_gsdml::_build_moduleList()
{
  for (pugi::xml_node& moduleItem : _xmlModuleList->children("ModuleItem"))
  {
    ...
    _moduleList[std::move(ID)].reset(new GSDML::ModuleItem(std::move(moduleItem), this)); 
  } 
}
...
void pn_gsdml::_build_valueList()
{
  ...
  {
    _valueList[std::move(valueItem.attribute("ID").value())].reset(new GSDML::ValueItem(std::move(valueItem), this)); 
  } 
}
...
void pn_gsdml::_build_textIdList()
{
  ...
  {
    _textIdList[std::move(text.attribute("TextId").value())].reset(new std::string(text.attribute("Value").as_string())); 
  } 
}

  // std::map<ushort, GSDML::ChannelDiagItem> _channelDiagList;
  // std::map<std::string, std::shared_ptr<GSDML::ModuleItem>> _moduleList;
  // std::map<std::string, std::shared_ptr<GSDML::ValueItem>> _valueList;
  // std::map<std::string, std::shared_ptr<std::string>> _textIdList;
  // std::map<std::string, std::shared_ptr<GSDML::DeviceAccessPointItem>> _deviceAccessPointList;

std::unordered_map<ushort, GSDML::ChannelDiagItem>& pn_gsdml::getChannelDiagList()
{
  return _channelDiagList;
}

std::unordered_map<std::string, std::shared_ptr<GSDML::ModuleItem>>& pn_gsdml::getModuleList()
{
  return _moduleList;
}

std::unordered_map<std::string, std::shared_ptr<GSDML::ValueItem>>& pn_gsdml::getValueList()
{
  return _valueList;
}

std::unordered_map<std::string, std::shared_ptr<std::string>>& pn_gsdml::getTextIdList()
{
  return _textIdList;
}

std::map<std::string, std::shared_ptr<GSDML::DeviceAccessPointItem>>& pn_gsdml::getDeviceAccessPointList()
{
