...
#define PNAK_WAIT_OBJECT_SWITCH_REDUNDANCY_ROLE       ((T_PNAK_WAIT_OBJECT) 0x00008000uL)
...
#define PNAK_WAIT_OBJECTS_EVENT_IND                   (PNAK_WAIT_OBJECT_STATE_CHANGED            | \
                                                       ...
                                                       PNAK_WAIT_OBJECT_SWITCH_REDUNDANCY_ROLE)
 ...
 */
#ifndef MAX_NUMBER_CMDEV_CONNECTIONS
#define MAX_NUMBER_CMDEV_CONNECTIONS    5u
#endif
...
#define PNAK_REDUNDANCY_ROLE_PRIMARY                   (PN_U16) 0x0001u
#define PNAK_REDUNDANCY_ROLE_BACKUP                    (PN_U16) 0x0002u

typedef struct _T_PNAK_EVENT_SWITCH_REDUNDANCY
{
  PN_U16    Role;
  PN_U16    ARIdentifier;

} PACK_WORD_ALIGNMENT (T_PNAK_EVENT_SWITCH_REDUNDANCY);
...
extern T_PNAK_TIMEOUT   pnak_get_remaining_timeout                (unsigned short);
...
extern T_PNAK_RESULT    pnak_set_switch_redundancy_ack            (unsigned short, PN_U16);
...
extern T_PNAK_RESULT    pnak_get_switch_redundancy_ind            (unsigned short, T_PNAK_EVENT_SWITCH_REDUNDANCY*);
...
typedef void            (*T_CMCTL_HOOK_FUNCTION)                  (unsigned short, unsigned short, unsigned long, T_PROFINET_BLOCK_HEADER*);
typedef struct _T_PNAK_CMCTL_HOOKS
{
  T_CMCTL_HOOK_FUNCTION   pConnectReqHook;
  T_CMCTL_HOOK_FUNCTION   pConnectResHook;

  T_CMCTL_HOOK_FUNCTION   pWriteReqHook;
  T_CMCTL_HOOK_FUNCTION   pWriteResHook;

  T_CMCTL_HOOK_FUNCTION   pReadReqHook;
  T_CMCTL_HOOK_FUNCTION   pReadResHook;

  T_CMCTL_HOOK_FUNCTION   pControlReqHook;
  T_CMCTL_HOOK_FUNCTION   pControlResHook;

} T_PNAK_CMCTL_HOOKS;

extern T_PNAK_RESULT    pnak_register_cmctl_hooks                 (unsigned short, T_PNAK_CMCTL_HOOKS*);
extern T_PNAK_RESULT    pnak_unregister_cmctl_hooks               (unsigned short);
 ...
 * easily adapted to the available memory provided by the used the hardware platform.
 *
 ...
 * to be processed by the realtime ethernet protocol limits the available bandwidth and the performance of the
