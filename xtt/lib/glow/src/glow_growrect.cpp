...
GrowRect::GrowRect(GrowCtx* glow_ctx, const char* name, double x, double y, double w, double h,
                   glow_eDrawType border_d_type, int line_w, int fix_line_w, glow_mDisplayLevel display_lev,
                   int fill_rect, int display_border, int display_shadow, glow_eDrawType fill_d_type,
                   int nodraw)
    : GlowRect(glow_ctx, x, y, w, h, border_d_type, line_w, fix_line_w, display_lev, fill_rect), hot(0),
      ...
      border(display_border), dynamic(0), dynamicsize(0), shadow(display_shadow), shadow_width(5),
      relief(glow_eRelief_Up), shadow_contrast(2), disable_shadow(0), invisible(0), fixcolor(0),
      fixposition(0), gradient(glow_eGradient_No), gradient_contrast(4), disable_gradient(0),
      bgcolor_gradient(0), fill_eq_background(0), transparency(0)
{
  strcpy(n_name, name);
  pzero.nav_zoom();
  strcpy(last_group, "");
...
int GrowRect::event_handler(GlowWind* w, glow_eEvent event, int x, int y, double fx, double fy)
{
  int sts;
...
void GrowRect::save(std::ostream& fp, glow_eSaveMode mode)
{
  char* s;
  ...
  fp << int(glow_eSave_GrowRect_original_border_drawtype) << FSPACE << int(original_border_drawtype) << '\n';
  fp << int(glow_eSave_GrowRect_original_fill_drawtype) << FSPACE << int(original_fill_drawtype) << '\n';
  ...
  fp << int(glow_eSave_GrowRect_gradient_contrast) << FSPACE << gradient_contrast << '\n';
  ...
  fp << int(glow_eSave_GrowRect_background_drawtype) << FSPACE << int(background_drawtype) << '\n';
  fp << int(glow_eSave_GrowRect_fill_eq_background) << FSPACE << fill_eq_background << '\n';
...
void GrowRect::open(std::istream& fp)
{
  int type = 0;
  int end_found = 0;
  char dummy[40];
...
void GrowRect::select_region_insert(double ll_x, double ll_y, double ur_x, double ur_y,
...
void GrowRect::set_scale(double scale_x, double scale_y, double x0, double y0, glow_eScaleType type)
{
  double old_x_left, old_x_right, old_y_low, old_y_high;
...
void GrowRect::set_rotation(double angle, double x0, double y0, glow_eRotationPoint type)
{
  double old_x_left, old_x_right, old_y_low, old_y_high;
                    ...
                    void* transpnode)
{
  ...
  if (fill_eq_background)
    fillcolor = ctx->get_drawtype(background_drawtype, glow_eDrawType_FillHighlight, highlight,
  ...
  if (fill)
  {
    if (display_shadow && ish != 0)
    {
      ...
      else
      {
        ...
        {
          f2 = fillcolor;
          f1 = ctx->get_drawtype(background_drawtype, glow_eDrawType_FillHighlight, highlight,
                                 (GrowNode*)colornode, 1);
          f0 = f1;
        }
        ...
        ctx->gdraw->gradient_fill_rect(w, ll_x + ish, ll_y + ish, ur_x - ll_x - 2 * ish,
                                       ur_y - ll_y - 2 * ish, f0, f1, f2,
    ...
    else
    {
      ...
      else
      {
        ...
        ctx->gdraw->gradient_fill_rect(w, ll_x, ll_y, ur_x - ll_x, ur_y - ll_y, f0, f1, f2,
                                       ctx->gdraw->gradient_rotate(rotation, grad), transp);
      }
    }
  }
...
void GrowRect::get_borders(GlowTransform* t, double* x_right, double* x_left, double* y_high, double* y_low)
{
...
void GrowRect::init_transform(double a11, double a12, double a13, double a21, double a22, double a23,
                              double rotation)
{
...
int GrowRect::draw_annot_background(GlowTransform* t, void* node, double x, double y)
{
  if (fill)
  {
...
int GrowRect::get_annot_background(GlowTransform* t, void* node, glow_eDrawType* background)
{
  if (fill)
  {
...
void GrowRect::export_javabean(GlowTransform* t, void* node, glow_eExportPass pass, int* shape_cnt,
                               int node_cnt, int in_nc, std::ostream& fp)
{
  ...
  ctx->export_jbean->rect(ll_x, ll_y, ur_x - ll_x, ur_y - ll_y, fill, jborder, fill_drawtype, draw_type, idx,
                          ish, shadow, drawtype_incr, fixcolor, gradient, gc1, gc2, pass, shape_cnt, node_cnt,
                          fp);
}

void GrowRect::set_last_group(char* name) { strcpy(last_group, name); }
...
void GrowRect::export_flow(GlowExportFlow* ef) { ef->rect(this); }
int GrowRect::export_script(GlowExportScript* es, void* o, void* m) { return es->rect(this, o, m); }
