...
GlowArrow::GlowArrow(GrowCtx* glow_ctx, double x1, double y1, double x2, double y2, double w, double l,
                     glow_eDrawType d_type)
    : GlowArrayElem(glow_ctx), p_dest(glow_ctx, x2, y2), arrow_width(w), arrow_length(l), draw_type(d_type),
      line_width(1)
{
...
void GlowArrow::draw(GlowWind* w, void* pos, int highlight, int hot, void* node)
{
  ...
  ctx->gdraw->arrow(w, p_dest_x + ((GlowPoint*)pos)->z_x - w->offset_x,
                    ...
                    p1_x + ((GlowPoint*)pos)->z_x - w->offset_x, p1_y + ((GlowPoint*)pos)->z_y - w->offset_y,
                    p2_x + ((GlowPoint*)pos)->z_x - w->offset_x, p2_y + ((GlowPoint*)pos)->z_y - w->offset_y,
...
void GlowArrow::erase(GlowWind* w, void* pos, int hot, void* node)
{
  ...
  idx = MAX(0, idx);
  idx = MIN(idx, DRAW_TYPE_SIZE - 1);
  ctx->gdraw->arrow_erase(
      w, p_dest_x + ((GlowPoint*)pos)->z_x - w->offset_x, p_dest_y + ((GlowPoint*)pos)->z_y - w->offset_y,
      p1_x + ((GlowPoint*)pos)->z_x - w->offset_x, p1_y + ((GlowPoint*)pos)->z_y - w->offset_y,
      p2_x + ((GlowPoint*)pos)->z_x - w->offset_x, p2_y + ((GlowPoint*)pos)->z_y - w->offset_y, idx);
}

void GlowArrow::move(void* pos, double x1, double y1, double x2, double y2, int highlight, int hot)
{
...
void GlowArrow::shift(void* pos, double delta_x, double delta_y, int highlight, int hot)
{
  erase(&ctx->mw, pos, hot, NULL);
  erase(&ctx->navw, pos, 0, NULL);
  p_dest.x += delta_x;
...
int GlowArrow::event_handler(GlowWind* w, void* pos, glow_eEvent event, int x, int y, void* node)
{
  return 0;
}
