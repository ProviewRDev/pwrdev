...
void ColPalCtx::configure()
{
  ...
  display_fill = new GrowRect(this, "FillRect", d, 0, display_entry_width - 2 * d, entry_height,
                              glow_eDrawType_Color37, 1, 0, glow_mDisplayLevel_1, 1, 1, 0, current_fill);
  insert((GlowArrayElem*)display_fill);
  active_fill = new GrowRect(this, "FillRectActive", d, 0, display_entry_width - 2 * d, entry_height,
  ...
  text_fill = new GrowText(this, "FillText", "Fill", 0.3, entry_height / 2 + 0.2,
                           glow_eDrawType_TextHelvetica, glow_eDrawType_Line, 2);
  insert(text_fill);

  display_border =
      new GrowRect(this, "BorderRect", display_entry_width + d, 0, display_entry_width - 2 * d, entry_height,
                   glow_eDrawType_Color37, 1, 0, glow_mDisplayLevel_1, 1, 1, 0, current_border);
  insert((GlowArrayElem*)display_border);
  active_border =
      new GrowRect(this, "BorderRectActive", display_entry_width + d, 0, display_entry_width - 2 * d,
                   entry_height, glow_eDrawType_Line, 1, 0, glow_mDisplayLevel_1, 0, 1, 1, current_border);
  ...
  text_border = new GrowText(this, "BorderText", "Border", display_entry_width + 0.3, entry_height / 2 + 0.2,
                             glow_eDrawType_TextHelvetica, glow_eDrawType_Color4, 2);
  insert(text_border);

  display_text =
      new GrowRect(this, "TextRect", display_entry_width * 2 + d, 0, display_entry_width - 2 * d,
                   entry_height, glow_eDrawType_Color37, 1, 0, glow_mDisplayLevel_1, 1, 1, 0, current_text);
  insert((GlowArrayElem*)display_text);
  active_text =
      new GrowRect(this, "TextRectActive", display_entry_width * 2 + d, 0, display_entry_width - 2 * d,
                   entry_height, glow_eDrawType_Line, 1, 0, glow_mDisplayLevel_1, 0, 1, 1, current_text);
  ...
  ((GrowRect*)active_text)->display_level = glow_mDisplayLevel_2;
  text_text = new GrowText(this, "TextText", "Text", display_entry_width * 2 + 0.3, entry_height / 2 + 0.2,
                           glow_eDrawType_TextHelvetica, glow_eDrawType_Color4, 2);
  insert(text_text);
  ...
  for (i = glow_eDrawType_Line; i <= glow_eDrawType_Color300; i++)
  {
    ...
    rect = new GrowRect(this, "ColorEntry", x, y, entry_width, entry_height, glow_eDrawType_Line, 1, 0,
                        glow_mDisplayLevel_1, 1, 1, 0, (glow_eDrawType)i);
    insert(rect);
  }
  ...
  for (i = glow_eDrawType_CustomColor1; i <= glow_eDrawType_CustomColor90; i += 4)
  {
    ...
    x = custom_entry_width * (double(i_pos) / custom_columns - floor(double(i_pos) / custom_columns)) *
        custom_columns;
    y = entry_height * (1 + 300 / columns + floor((i_pos - 300) / custom_columns + DBL_EPSILON));
    rect = new GrowRect(this, "ColorEntry", x, y, custom_entry_width - dw, entry_height, glow_eDrawType_Line,
                        1, 0, glow_mDisplayLevel_1, 1, 1, 0, (glow_eDrawType)i);
    insert(rect);
  }
  y += entry_height;
  x = 0;
  color_description_bg =
      new GrowRect(this, "ColorDescriptionBg", x, y + 0.04, custom_entry_width * 16, entry_height - 0.04,
                   glow_eDrawType_Line, 1, 0, glow_mDisplayLevel_1, 1, 0, 0, glow_eDrawType_LineErase);
  ...
  active_tone = new GrowRect(this, "ToneRect", x, y, tone_entry_width * 2, entry_height, glow_eDrawType_Line,
                             1, 0, glow_mDisplayLevel_1, 1, 1, 0, glow_eDrawType_Color34);
  insert(active_tone);
  text_tone = new GrowText(this, "ToneText", "Tone", x + 0.15, y + entry_height / 2 + 0.25,
                           glow_eDrawType_TextHelveticaBold, glow_eDrawType_Line, 2);
  insert(text_tone);

  x += 2 * tone_entry_width;
  sprintf(name, "ToneEntry%d", 0);
  rect = new GrowRect(this, name, x, y, tone_entry_width * 2, entry_height, glow_eDrawType_Line, 1, 0,
                      glow_mDisplayLevel_1, 1, 1, 1, glow_eDrawType_Color34);
  rect->shadow_width = 15;
  rect->relief = glow_eRelief_Down;
  insert(rect);
  text = new GrowText(this, name, "Reset", x + 0.15, y + entry_height / 2 + 0.25,
                      glow_eDrawType_TextHelvetica, glow_eDrawType_Line, 2);
  ...
  rect->relief = glow_eRelief_Down;
  insert(rect);
  text = new GrowText(this, name, " Bg", x + 0.15, y + entry_height / 2 + 0.25, glow_eDrawType_TextHelvetica,
  ...
  for (i = glow_eDrawTone_No + 1; i < glow_eDrawTone_GrayHighSaturation; i++)
  {
    ...
    if (i == glow_eDrawTone_Gray || i == glow_eDrawTone_LightGray || i == glow_eDrawTone_DarkGray)
    {
      // New row
      y += entry_height;
      x = 0;
    ...
    rect = new GrowRect(this, name, x, y, tone_entry_width, entry_height, glow_eDrawType_Line, 1, 0,
                        glow_mDisplayLevel_1, 1, 1, 0, drawtype);
    insert(rect);
  }
  get_borders();
  a.zoom();
  ...
  case glow_eEvent_MB2Click:
    ...
    if (callback_object_type != glow_eObjectType_NoObject)
    {
      if (callback_object->type() == glow_eObjectType_GrowText)
      {
        ...
        else if (str_StartsWith(name, "BorderText") && event == glow_eEvent_MB1Click)
        {
          set_active(colpal_eActive_BorderColor);
        }
        else if (str_StartsWith(name, "TextText") && event == glow_eEvent_MB1Click)
        {
          set_active(colpal_eActive_TextColor);
        }
        else if (str_StartsWith(name, "ToneEntry") && event == glow_eEvent_MB1Click)
        {
          glow_eDrawTone tone;
      ...
      else if (callback_object->type() == glow_eObjectType_GrowRect)
      {
        ...
        else if (str_StartsWith(name, "BorderRect") && event == glow_eEvent_MB1Click)
        {
          set_active(colpal_eActive_BorderColor);
        }
        else if (str_StartsWith(name, "TextRect") && event == glow_eEvent_MB1Click)
        {
          set_active(colpal_eActive_TextColor);
        }
        else if (str_StartsWith(name, "ToneEntry") && event == glow_eEvent_MB1Click)
        {
          ...
          current_tone = (glow_eDrawType)tone;
          ((GrowRect*)active_tone)->set_fill_color((glow_eDrawType)tone_color_match((int)tone));
          break;
        }
...
void ColPalCtx::set_colors()
{
  ...
  {
    if (current_fill == glow_eDrawType_Line || (((current_fill + 1) % 10 == 0) && current_fill > 20) ||
  ...
  {
    if (current_border == glow_eDrawType_Line || (((current_border + 1) % 10 == 0) && current_border > 20) ||
  ...
  {
    if (current_text == glow_eDrawType_Line || (((current_text + 1) % 10 == 0) && current_text > 20) ||
...
char* ColPalCtx::color_idx_to_text(int idx)
{
  ...
  static char tonestr[9][20] = {"Gray",    "Yellowgreen", "Yellow",  "Orange", "Red",
                                "Magenta", "Blue",        "SeaBlue", "Green"};
...
{
  static char text[90][40] = {"Background",
                              "Background gradient",
                              "Delimiter area",
                              "Delimiter line",
                              "Text/Lines on background",
                              "Inputfield fill color",
                              "Inputfield border color",
                              "Inputfield text color",
                              " ",
                              " ",
                              "Red indicator",
                              "Red curve",
                              "Red bar",
                              "Red bar limit",
                              "Red valuefield color",
                              "Indicator border color",
                              "Indicator low color",
                              "Indicator on delimiter low color",
                              "Slider color",
                              "Slider background color",
                              "Valuefield fill color",
                              "Valuefield border color",
                              "Valuefield text color",
                              "Folder active color",
                              "Folder inactive color",
                              "Green indicator",
                              "Green curve",
                              "Green bar",
                              "Green bar limit",
                              "Green valuefield color",
                              "Limitswitch high color",
                              "Limitswitch low color",
                              "Limitswitch border color",
                              "Text/lines on delimiter",
                              "Button active color",
                              "Button fill color",
                              "Button border color",
                              "Button text color",
                              "Button insensitive text color",
                              "Button insensitive border color",
                              "Yellow indicator",
                              "Yellow curve",
                              "Yellow bar",
                              "Yellow bar limit",
                              "Yellow valuefield color",
                              "Symbol fill color",
                              "Symbol border color",
                              "Symbol low color",
                              "Symbol empty color",
