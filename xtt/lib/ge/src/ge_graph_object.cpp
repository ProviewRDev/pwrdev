...
static graph_sObjectFunction graph_object_functions[] = {{pwr_cClass_Di, &graph_object_dx},
...
int Graph::init_object_graph(int mode)
{
  ...
  if ((s = strrchr(filename, '/')) || (s = strrchr(filename, '>')) || (s = strrchr(filename, ']')) ||
      (s = strrchr(filename, ':')))
  {
...
static int graph_object_dx(Graph* graph, pwr_sAttrRef* arp)
{
  ...
  od->local_conv_p = (pwr_tBoolean*)graph->localdb_ref_or_create("ConversionOn", pwr_eType_Boolean);
  od->local_inv_p = (pwr_tBoolean*)graph->localdb_ref_or_create("InvertOn", pwr_eType_Boolean);
  od->local_test_p = (pwr_tBoolean*)graph->localdb_ref_or_create("TestOn", pwr_eType_Boolean);
  ...
  default:
  {
    ...
    sts = gdh_AttrrefToName(&chanaref, chan_name, sizeof(chan_name), cdh_mNName);
    if (ODD(sts))
    {
...
void Graph::trend_scan(graph_sObjectTrend* td)
{
       ...
       !feqf(*td->pres_min_limit_p, td->pres_min_limit_old)))
  {
    ...
    {
      if (td->bar_object)
        grow_SetBarRange(td->bar_object, double(*td->pres_min_limit_p), double(*td->pres_max_limit_p));
      if (td->slider_object)
      {
        GeDyn* dyn;
        grow_SetSliderRange(td->slider_object, double(*td->pres_min_limit_p), double(*td->pres_max_limit_p));
        grow_GetUserData(td->slider_object, (void**)&dyn);
        dyn->update();
      }
      grow_SetTrendRangeY(td->trend_object, 0, double(*td->pres_min_limit_p), double(*td->pres_max_limit_p));
    }
...
int Graph::trend_init(graph_sObjectTrend* td, pwr_sAttrRef* arp)
{
  ...
  // Configure slider
  sts = grow_FindObjectByName(grow->ctx, "ActualValueSlider", &td->slider_object);
  if (ODD(sts))
  {
    if (td->pres_min_limit_p && td->pres_max_limit_p)
    {
  ...
  td->scan_time_p = (float*)localdb_ref_or_create("ScanTime", pwr_eType_Float32);
  ...
  *td->scan_time_p = td->old_scan_time;
  ...
  if (ODD(sts))
    td->hold_button_p = (int*)localdb_ref_or_create("TrendHold", pwr_eType_Boolean);
  ...
  sts = grow_FindObjectByName(grow->ctx, "SliderDisable", &td->slider_button_object);
  if (ODD(sts))
    td->slider_button_p = (int*)localdb_ref_or_create("SliderDisable", pwr_eType_Boolean);
...
static int graph_object_collect_build(Graph* graph, pwr_sAttrRef* attrref)
{
  ...
  graph->create_node(NULL, "pwrct_valueinputsmallbg", x0, y0 - 1.3, 4, y0 - 1.3 + 1.2, &scantime_button);
  ...
  // Hold button
  graph->create_node("TrendHold", "pwrct_buttonsmalltoggle", x0 + trend_width / 2 - 3. / 2, y0 - 1.4,
                     x0 + trend_width / 2 + 3. / 2, y0 - 1.4 + 1.2, &hold_button);
  ...
  //  Zero text
  grow_CreateGrowText(graph->grow->ctx, "", "0", x0 + trend_width - 0.2, y0 - 0.3,
                      glow_eDrawType_TextHelvetica, glow_eDrawType_CustomColor5, 3, glow_eFont_Helvetica,
  ...
  while (cdh_ObjidIsNotNull(ap->Objid))
  {
    if (*is_attrp)
    {
      ...
      case pwr_eType_Boolean:
      {
        ...
        grow_CreateGrowTrend(graph->grow->ctx, "ActualValueTrend", x1, y1, trend_width, trend_height,
                             glow_eDrawType_Color37, 0, glow_mDisplayLevel_1, 1, 1, glow_eDrawType_Color40,
                             NULL, &trend);
        dyn = new GeDyn(graph);
        ...
        grow_GetTextExtent(graph->grow->ctx, name, strlen(name), glow_eDrawType_TextHelvetica, 4,
        ...
        grow_CreateGrowText(graph->grow->ctx, "", name, x1 + trend_width + 1,
                            ...
                            glow_eDrawType_CustomColor5, 4, glow_eFont_Helvetica, glow_mDisplayLevel_1, NULL,
                            &t1);
  ...
  y1 = y0;
  x1 = x0 + trend_width;
  grow_CreateGrowLine(graph->grow->ctx, "", x0 + trend_width, y1, x0 + trend_width + name_width + 2, y1,
  ...
  // Draw frame
  grow_CreateGrowRect(graph->grow->ctx, "R1", x0 - 1.5, y0 - 2.7, trend_width + name_width + 5.5, 1,
  ...
  grow_CreateGrowRect(graph->grow->ctx, "R2", x0 - 1.5, y0 + trend_cnt * trend_height + 0.5,
                      trend_width + name_width + 5.5, 5, glow_eDrawType_CustomColor3, 1, 0,
                      glow_mDisplayLevel_1, 1, 0, 0, glow_eDrawType_CustomColor3, NULL, &l1);
  grow_CreateGrowRect(graph->grow->ctx, "R3", x0 + trend_width + name_width + 2, y0 - 1.7, 2,
                      y0 + trend_cnt * trend_height + 0.9, glow_eDrawType_CustomColor3, 1, 0,
                      glow_mDisplayLevel_1, 1, 0, 0, glow_eDrawType_CustomColor3, NULL, &l1);
  grow_CreateGrowRect(graph->grow->ctx, "R4", x0 - 1.5, y0 - 1.7, 1, y0 + trend_cnt * trend_height + 0.9,
                      glow_eDrawType_CustomColor3, 1, 0, glow_mDisplayLevel_1, 1, 0, 0,
                      glow_eDrawType_CustomColor3, NULL, &l1);

  grow_SetLayout(graph->grow->ctx, x0 - 1, y0 - 2.3, x0 + trend_width + name_width + 3,
                 y0 + trend_cnt * trend_height + 1.5);

  return 1;
}

int Graph::create_node(const char* node_name, const char* subgraph_str, double x1, double y1, double x2,
                       double y2, grow_tNode* node)
{
  int sts;
  grow_tNodeClass nc;
  double sx, sy;
...
static void graph_object_PlcThread_scan(Graph* graph)
{
  ...
  if (od->set_max_show_p && od->set_min_show_p &&
      (!feqf(*od->set_max_show_p, od->set_max_show_old) || !feqf(*od->set_min_show_p, od->set_min_show_old)))
  {
    if (!feqf(*od->set_max_show_p, *od->set_min_show_p))
    {
      grow_SetBarRange(od->set_bar_object, double(*od->set_min_show_p), double(*od->set_max_show_p));
      grow_SetTrendRangeY(od->set_trend_object, 0, double(*od->set_min_show_p), double(*od->set_max_show_p));
      grow_SetTrendRangeY(od->set_trend_object, 1, double(*od->set_min_show_p), double(*od->set_max_show_p));
    }
...
static void graph_object_PlcThread_close(Graph* graph) { free(graph->graph_object_data); }
...
static int graph_object_PlcThread(Graph* graph, pwr_sAttrRef* arp)
{
  ...
  sts = grow_FindObjectByName(graph->grow->ctx, "ActualScanTimeBar", &od->set_bar_object);
  if (EVEN(sts))
    return sts;
  ...
  // Get pointers to max and min value
  od->set_max_show_p = (float*)graph->localdb_ref_or_create("MaxShow", pwr_eType_Float32);
  *od->set_max_show_p = od->set_max_show_old;

  od->set_min_show_p = (float*)graph->localdb_ref_or_create("MinShow", pwr_eType_Float32);
  *od->set_min_show_p = od->set_min_show_old;

  // Configure SetVal  trend
  sts = grow_FindObjectByName(graph->grow->ctx, "ActualScanTimeTrend", &od->set_trend_object);
  if (EVEN(sts))
    return sts;
