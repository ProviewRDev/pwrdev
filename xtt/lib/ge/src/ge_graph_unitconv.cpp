...
static double uc_length_a1[uc_length_size] =
    // m,      cm,       dm,       ft,         in,           km,        mi,         mm,       nm,        NM,
    // um,       yd
    {1.0,        1.0e2, 1.0e1, 3.28083989, 39.3700787, 1.0e-3,
     6.21371e-4, 1.0e3, 1.0e9, 5.39957e-4, 1.0e6,      1.0936133};
...
static double uc_speed_a1[uc_speed_size] =
    // m/s,       ft/min,      ft/s,       in/min,      in/s,       km/h,        kn,         m/min,      mm/s,
    // mi/h,       yd/min,    yd/s
    {1.0,       1.96850393e2, 3.28083989, 2362.2047244, 39.37007874, 3.6,
     1.9438444, 60,           1e3,        2.23693629,   65.617979,   1.093613298};
...
static double uc_volumeflow_a1[uc_volumeflow_size] =
    // m3/s,      cl/s,       in3/min,       in3/s,         ft3/min,      ft3/s,      l/s, m3/min, mm3/s,
    // yd3/min,      yd3/s
    {1.0, 1e5, 3.661424929e6, 6.102374321e4, 2.118879972e3, 35.31466621,
     1e3, 60,  1e9,           78.477036028,  1.30795060046};

static int uc_attrtype[] = {ge_eAttrType_UcAcceleration, ge_eAttrType_UcAngle,       ge_eAttrType_UcArea,
                            ge_eAttrType_UcEnergy,       ge_eAttrType_UcForce,       ge_eAttrType_UcFrequency,
                            ge_eAttrType_UcGeneral,      ge_eAttrType_UcLength,      ge_eAttrType_UcMass,
                            ge_eAttrType_UcMassFlow,     ge_eAttrType_UcPower,       ge_eAttrType_UcPressure,
                            ge_eAttrType_UcSpeed,        ge_eAttrType_UcTemperature, ge_eAttrType_UcTime,
                            ge_eAttrType_UcVolume,       ge_eAttrType_UcVolumeFlow};

int uc_entity_to_attrtype(int entity) { return uc_attrtype[entity]; }
...
double uc_convert(graph_eUcEntity entity, int from, int to, double value)
{
  ...
  case graph_eUcEntity_Temperature:
    if (from >= uc_temperature_size || to >= uc_temperature_size)
      return 0;
    return uc_temperature_a0[to] +
           (value - uc_temperature_a0[from]) * uc_temperature_a1[to] / uc_temperature_a1[from];
