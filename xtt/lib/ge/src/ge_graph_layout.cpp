  ...
  for (int j = 0; j < ctx->msize; j++)
  {
    if (a_prev[j] == idx && j != a_next[idx])
    {
      ...
      for (int k = 0; k < orig_row_size; k++)
        ctx->row_idx[dir][ctx->row_cnt[dir] * ctx->msize + k].idx =
            ctx->row_idx[dir][orig_row * ctx->msize + k].idx;
            ...
            if (ctx->row_idx[dir][i * ctx->msize + j].idx == k)
            {
              ...
              else
              {
                if (!(fabs(last_size - ctx->row_idx[dir][i * ctx->msize + j].size) < DBL_EPSILON))
                {
                  ...
                  {
                    // Adjust previous
                    ctx->row_idx[dir][i * ctx->msize + j - 1].size -=
                        last_size - ctx->row_idx[dir][i * ctx->msize + j].size;
