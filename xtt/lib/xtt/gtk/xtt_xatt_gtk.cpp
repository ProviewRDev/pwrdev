...
void XAttGtk::activate_cmd_input(GtkWidget* w, gpointer data)
{
  ...
  {
    if (text)
      sts = xatt->xattnav->set_attr_value(xatt->input_node, xatt->input_name, text);
...
void XAttGtk::activate_cmd_scrolled_ok(GtkWidget* w, gpointer data)
{
  ...
  if (xatt->input_open)
  {
    ...
    textutf8 = gtk_text_buffer_get_text(xatt->cmd_scrolled_buffer, &start_iter, &end_iter, FALSE);
    text = g_convert(textutf8, -1, "ISO8859-1", "UTF-8", NULL, NULL, NULL);
    g_free(textutf8);
    ...
    int h;
    h = gdk_window_get_height(gtk_widget_get_window(xatt->pane));
...
void XAttGtk::pop() { gtk_window_present(GTK_WINDOW(toplevel)); }
...
XAttGtk::XAttGtk(GtkWidget* xa_parent_wid, void* xa_parent_ctx, pwr_sAttrRef* xa_objar, int xa_advanced_user,
                 int* xa_sts)
    : XAtt(xa_parent_ctx, xa_objar, xa_advanced_user, xa_sts), parent_wid(xa_parent_wid)
{
  int sts;
  pwr_tAName title;
  ...
  *xa_sts = gdh_AttrrefToName(&objar, title, sizeof(title), cdh_mNName);
  ...
  xattnav = new XAttNavGtk((void*)this, pane, xattnav_eType_Object, "Plant", &objar, xa_advanced_user, 0,
                           &brow_widget, &sts);
