...
void XttGe::eventlog_enable(int enable) { Graph::eventlog_enable(enable); }
...
void XttGe::ge_popup_menu_cb(void* ge_ctx, pwr_sAttrRef attrref, unsigned long item_type,
...
void XttGe::ge_context_menu_cb(void* ge_ctx, void* menudata, unsigned long utility, char* arg, int x, int y)
{
  XttGe* ge = (XttGe*)ge_ctx;
...
int XttGe::ge_call_method_cb(void* ge_ctx, char* method, char* filter, pwr_sAttrRef attrref,
...
int XttGe::ge_get_current_objects_cb(void* ge_ctx, pwr_sAttrRef** alist, int** is_alist)
{
  XttGe* ge = (XttGe*)ge_ctx;
...
int XttGe::ge_get_rtplant_select_cb(void* ge_ctx, char* attr_name, int size, pwr_tTypeId* type)
{
  XttGe* ge = (XttGe*)ge_ctx;
...
void XttGe::message_cb(void* ctx, char severity, const char* msg) { ((XttGe*)ctx)->message(severity, msg); }
...
void XttGe::get_current_colors_cb(void* ctx, glow_eDrawType* fill_color, glow_eDrawType* border_color,
...
void XttGe::set_current_colors_cb(void* ctx, glow_eDrawType fill_color, glow_eDrawType border_color,
                                  glow_eDrawType text_color)
{
...
int XttGe::set_object_focus(const char* name, int empty) { return graph->set_object_focus(name, empty); }

int XttGe::set_folder_index(const char* name, int idx) { return graph->set_folder_index(name, idx); }
...
void XttGe::disable_subwindow_events(int disable) { return graph->disable_subwindow_events(disable); }
...
void XttGe::swap(int mode) { graph->swap(mode); }
...
void XttGe::signal_send(char* signalname) { graph->signal_send(signalname); }
...
void XttGe::set_text_coding(lng_eCoding coding) { graph->set_text_coding(coding); }

int XttGe::key_pressed(int key) { return graph->key_pressed(key); }

void XttGe::close_input_all() { graph->close_input_all(); }
...
int XttGe::in_edit_mode() { return (graph->mode == graph_eMode_Development); }
...
void XttGe::activate_graphattributes() { graph->edit_graph_attributes(); }
...
XttGe::XttGe(void* xg_parent_ctx, const char* xg_name, const char* xg_filename, int xg_scrollbar, int xg_menu,
    ...
    : parent_ctx(xg_parent_ctx), scrollbar(xg_scrollbar), navigator(xg_navigator), menu(xg_menu),
      ...
      keyboard_cb(xg_keyboard_cb), namechanged_cb(0), width(xg_width), height(xg_height), options(xg_options),
