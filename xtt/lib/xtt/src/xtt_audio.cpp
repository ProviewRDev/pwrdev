  ...
  if (arp && cdh_ObjidIsNotNull(arp->Objid))
  {
    while (1)
    {
      ...
      case pwr_cClass_Sound:
      {
        ...
        if (str_StartsWith(sound.Source, "Sine") || str_StartsWith(sound.Source, "Square"))
        {
            ...
            if (sound.VolumeTable[i] > 0)
            {
              ...
              else if (str_StartsWith(sound.Source, "Square"))
              {
                MakeSquare(buffer, asize, 0, 0, sound.Length, sound.BaseTone + sound.ToneTable[i],
                           ...
                           sound.Volume / 100 * sound.VolumeTable[i], sound.Attack, sound.Decay,
                           sound.Sustain / 100, sound.Release, sound.Tremolo / 100);
              }
            }
          }
        ...
        else if (strstr(sound.Source, ".wav") != 0)
        {
          ...
          if (!(chunk.wFormatTag == 1 && (chunk.wBitsPerSample == 16 || chunk.wBitsPerSample == 8) &&
      ...
      case pwr_cClass_SoundSequence:
      {
        ...
        for (int i = 0; i < int(sizeof(seq.SequenceTable) / sizeof(seq.SequenceTable[0])); i++)
        {
          if (!seq.SequenceTable[i].Used ||
              (feqf(seq.SequenceTable[i].VolumeRight, 0.0f) && feqf(seq.SequenceTable[i].VolumeLeft, 0.0f)))
            continue;
          ...
          for (int j = 0; j < int(sizeof(sound[0].ToneTable) / sizeof(sound[0].ToneTable[0])); j++)
          {
            ...
            {
              if (str_StartsWith(sound[seq.SequenceTable[i].SoundIdx].Source, "Sine"))
              {
                MakeSine(buffer, size, 0, seq.SequenceTable[i].StartTime, seq.SequenceTable[i].EndTime,
              ...
              else if (str_StartsWith(sound[seq.SequenceTable[i].SoundIdx].Source, "Square"))
              {
                MakeSquare(buffer, size, 0, seq.SequenceTable[i].StartTime, seq.SequenceTable[i].EndTime,
                           seq.SequenceTable[i].Tone + sound[seq.SequenceTable[i].SoundIdx].ToneTable[j],
                           seq.SequenceTable[i].VolumeRight, seq.SequenceTable[i].VolumeLeft,
                           sound[seq.SequenceTable[i].SoundIdx].Attack,
                           sound[seq.SequenceTable[i].SoundIdx].Decay,
                           sound[seq.SequenceTable[i].SoundIdx].Sustain / 100,
                           sound[seq.SequenceTable[i].SoundIdx].Release,
...
void XttAudio::audio_write(void* data)
{
  ...
  if (audio->OSS_audio_ok)
  {
    ...
    rc = write(audio->OSS_handle, &audio->write_buffer[audio->write_buffer_idx], size * 2);
    if (rc == -EPIPE)
    {
...
void XttAudio::audio_write_buff(void* a)
{
  ...
  rc = snd_pcm_writei(audio->ALSA_handle, &audio->write_buffer[audio->write_buffer_idx], size / 2);

  if (debug)
    printf("Write idx: %6d totsize: %6d size: %6d written: %6d actsize: %4.2f\n", audio->write_buffer_idx,
           audio->write_buffer_size, size, rc * 2, ((float)time) / 1000);
...
int XttAudio::Init_ALSA(char* device, unsigned int samplerate)
{
  ...
  else
  {
    ...
    if (err >= 0)
    {
      ...
      if ((err = snd_pcm_hw_params_set_buffer_size_near(ALSA_handle, hw_params, (snd_pcm_uframes_t*)&msize)) <
          0)
      {
