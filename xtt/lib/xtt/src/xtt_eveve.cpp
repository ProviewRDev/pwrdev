...
EvEve::EvEve(void* ev_parent_ctx, char* eve_name, pwr_tObjid ev_user, int ev_eventname_seg, int ev_width,
    ...
    : parent_ctx(ev_parent_ctx), user(ev_user), eventname_seg(ev_eventname_seg), width(ev_width),
      ...
      acknowledge_cb(0), name_to_alias_cb(0), copy_list_cb(0), close_cb(0), ala(NULL), eve_displayed(0),
      list_copied(0)
{
}
...
void EvEve::eve_popup_menu_cb(void* ctx, pwr_tAttrRef attrref, unsigned long item_type, unsigned long utility,
                              char* arg, int x, int y)
{
  if (((EvEve*)ctx)->popup_menu_cb)
    (((EvEve*)ctx)->popup_menu_cb)(((EvEve*)ctx)->parent_ctx, attrref, item_type, utility, arg, x, y);
}
...
int EvEve::get_last_not_acked_prio(mh_sEventId** id, unsigned long type, unsigned long prio)
{
