...
FlowCtx::FlowCtx(const char* ctx_name, double zoom_fact, int offs_x, int offs_y)
    : ctx_type(flow_eCtxType_Flow), zoom_factor(zoom_fact), base_zoom_factor(zoom_fact), offset_x(offs_x),
      offset_y(offs_y), nav_zoom_factor(zoom_fact), print_zoom_factor(100), x_right(0), x_left(0), y_high(0),
      y_low(0), window_width(0), window_height(0), old_window_width(0), old_window_height(0),
      nav_window_width(0), nav_window_height(0), nav_rect_ll_x(0), nav_rect_ll_y(0), nav_rect_ur_x(0),
      nav_rect_ur_y(0), node_movement_active(0), node_movement_paste_active(0),
      node_movement_paste_pending(0), nav_rect_movement_active(0), nav_rect_zoom_active(0),
      select_rect_active(0), con_create_active(0), conpoint_select_idx(0), auto_scrolling_active(0),
      defered_redraw_active(0), a(50, 50), a_sel(20, 20), a_paste(20, 20), a_move(20, 20), a_nc(20, 20),
      a_cc(20, 20), event_region_select(flow_eEvent_Null), event_region_add_select(flow_eEvent_Null),
      event_create_con(flow_eEvent_Null), event_create_node(flow_eEvent_Null),
      event_move_node(flow_eEvent_Null), event_select_conpoint(flow_eEvent_Null), callback_object(0),
      callback_object_type(flow_eObjectType_NoObject), cursor_present(0), cursor_x(0), cursor_y(0),
      user_highlight(0), application_paste(0), grid_size_x(2), grid_size_y(1), grid_on(1), draw_delta(0.3),
      grafcet_con_delta(2), refcon_cnt(0), refcon_width(1.5), refcon_height(0.8), refcon_textsize(3),
      refcon_linewidth(2), trace_connect_func(0), trace_scan_func(0), trace_started(0), user_version(0),
      unobscured(1), nodraw(0), no_nav(1), widget_cnt(0), select_policy(flow_eSelectPolicy_Partial),
      tiptext(0), inverse_color(flow_eDrawType_Line), text_coding(flow_eTextCoding_ISO8859_1),
      display_level(flow_mDisplayLevel_1), scroll_size(0), scroll_callback(0), scroll_data(NULL)
{
...
void FlowCtx::con_create_dest(FlowArrayElem* node, int cp_num, flow_eEvent event, int x, int y)
{
...
void FlowCtx::redraw_defered()
{
  ...
  if (!defered_redraw_active)
  {
    ...
    if (defered_x_low_nav < defered_x_high_nav && defered_y_low_nav < defered_y_high_nav)
      nav_draw(defered_x_low_nav, defered_y_low_nav, defered_x_high_nav, defered_y_high_nav);
  }
}
...
void FlowCtx::draw(int ll_x, int ll_y, int ur_x, int ur_y)
{
  ...
  fdraw->invalidate(ll_x, ll_y, ur_x - ll_x, ur_y - ll_y);
}

void FlowCtx::draw_invalidated(int ll_x, int ll_y, int ur_x, int ur_y)
{
  int i;
  ...
  if (select_rect_active)
  {
    fdraw->rect(this, select_rect_ll_x, select_rect_ll_y, select_rect_ur_x - select_rect_ll_x,
                select_rect_ur_y - select_rect_ll_y, flow_eDrawType_Line, 0, 0, 0);
  }
  tiptext->draw();
  if (con_create_active)
    fdraw->line(this, con_create_conpoint_x, con_create_conpoint_y, con_create_last_x, con_create_last_y,
                flow_eDrawType_Line, 0, 0, 0);
}

void FlowCtx::clear() { fdraw->clear(this); }
...
void FlowCtx::nav_zoom_invalidated()
{
  if (a.size() == 0)
    return;
  ...
  if (feq(x_nav_right, x_nav_left) || feq(y_nav_high, y_nav_low))
    return;
  nav_zoom_factor =
      MIN(nav_window_width / (x_nav_right - x_nav_left), nav_window_height / (y_nav_high - y_nav_low));
  nav_offset_x = int(x_nav_left * nav_zoom_factor);
  nav_offset_y = int(y_nav_low * nav_zoom_factor);
  a.nav_zoom();
}

void FlowCtx::print_zoom() { a.print_zoom(); }

int FlowCtx::print_region(double ll_x, double ll_y, double ur_x, double ur_y, char* filename)
{
  int sts;
...
int FlowCtx::print_pdf_region(double ll_x, double ll_y, double ur_x, double ur_y, char* filename)
{
  int sts;
...
void FlowCtx::print_draw_page(void* context, const char* title, int page, flow_eOrientation orientation,
                              double scale)
{
  double ll_x, ll_y, ur_x, ur_y;
  int sts;
  ...
  {
    if (a.a[i]->type() == flow_eObjectType_Node && ((FlowNode*)a.a[i])->nc->group == flow_eNodeGroup_Document)
    {
      doc_cnt++;
      if (doc_cnt == page + 1)
      {
...
void FlowCtx::nav_clear() { fdraw->nav_clear(this); }
...
void FlowCtx::nav_draw(int ll_x, int ll_y, int ur_x, int ur_y)
{
  ...
  fdraw->invalidate_nav(ll_x, ll_y, ur_x - ll_x, ur_y - ll_y);
}

void FlowCtx::nav_draw_invalidated(int ll_x, int ll_y, int ur_x, int ur_y)
{
  int i;
  ...
  nav_rect_ur_x = int(nav_zoom_factor * (offset_x + window_width) / zoom_factor - nav_offset_x);
  nav_rect_ll_y = int(nav_zoom_factor * offset_y / zoom_factor - nav_offset_y);
  nav_rect_ur_y = int(nav_zoom_factor * (offset_y + window_height) / zoom_factor - nav_offset_y);

  fdraw->nav_rect(this, nav_rect_ll_x, nav_rect_ll_y, nav_rect_ur_x - nav_rect_ll_x,
...
int FlowCtx::event_handler(flow_eEvent event, int x, int y, int w, int h)
{
  ...
  else if (event == event_move_node)
  {
    ...
    if (sts)
    {
      ...
      if (trace_started && a_move.size() > 0 && a_move[0]->type() == flow_eObjectType_Node &&
      ...
      for (i = 0; i < node_cnt; i++)
      {
        for (j = 0; j < a.size(); j++)
        {
          if (a[j]->type() == flow_eObjectType_Con && ((FlowCon*)a[j])->is_connected_to((FlowNode*)a_move[i]))
          {
            if (move_insert(a[j]))
  ...
  case flow_eEvent_Exposure:
  {
    ...
    ctx->fdraw->get_window_size(ctx, &window_width, &window_height);
  ...
  case flow_eEvent_ButtonMotion:
    ...
    {
      int last_x, last_y;
      last_x = con_create_last_x;
      last_y = con_create_last_y;
      con_create_last_x = x;
      con_create_last_y = y;

      int ll_x, ll_y, ur_x, ur_y, tmp;
      ll_x = con_create_conpoint_x;
      ll_y = con_create_conpoint_y;
      ur_x = last_x;
      ur_y = last_y;
      if (ll_x > ur_x)
      {
      ...
      draw(ll_x - DRAW_FMP, ll_y - DRAW_FMP, ur_x + DRAW_FMP, ur_y + DRAW_FMP);

      ll_x = con_create_conpoint_x;
      ll_y = con_create_conpoint_y;
      ur_x = con_create_last_x;
      ur_y = con_create_last_y;
      if (ll_x > ur_x)
      {
        tmp = ur_x;
        ur_x = ll_x;
      ...
      draw(con_create_conpoint_x, con_create_conpoint_y, con_create_last_x, con_create_last_y);
      for (i = 0; i < a.a_size; i++)
      {
    ...
    else if (select_rect_active)
    {
      ...
      ll_x = select_rect_ll_x;
      ur_x = select_rect_ur_x;
      ll_y = select_rect_ll_y;
      ur_y = select_rect_ur_y;
      ...
      ll_x = MIN(ll_x, select_rect_ll_x);
      ur_x = MAX(ur_x, select_rect_ur_x);
      ll_y = MIN(ll_y, select_rect_ll_y);
      ur_y = MAX(ur_y, select_rect_ur_y);
  ...
  case flow_eEvent_ButtonRelease:
    ...
    else if (select_rect_active)
    {
      ...
      draw(ll_x - 1, ll_y - 1, ur_x + 1, ur_y + 1);
  ...
  if (event_callback[event] && sts != FLOW__NO_PROPAGATE && event != event_move_node)
  {
    static flow_sEvent e;
    e.event = event;
    e.any.type = flow_eEventType_Object;
...
int FlowCtx::event_handler_nav(flow_eEvent event, int x, int y)
{
  ...
  case flow_eEvent_MB1ClickShift:
    ...
    fdraw->nav_rect_erase(this, nav_rect_ll_x, nav_rect_ll_y, nav_rect_ur_x - nav_rect_ll_x,
  ...
  case flow_eEvent_Exposure:
    ...
    nav_draw_invalidated(0, 0, nav_window_width, nav_window_height);
    change_scrollbar();
    break;
  case flow_eEvent_ButtonMotion:
    if (nav_rect_movement_active)
    {
      ...
      nav_rect_move_last_x = x;
      nav_rect_move_last_y = y;
      nav_draw(nav_rect_ll_x - 10, nav_rect_ll_y - 10, nav_rect_ur_x + 10, nav_rect_ur_y + 10);
      ...
      else
      {
        ...
        else if (mainwind_delta_x <= 0 && mainwind_delta_y <= 0)
        {
          ...
          if (mainwind_delta_y)
            draw(0, window_height + mainwind_delta_y, window_width + mainwind_delta_x, window_height);
        }
        else if (mainwind_delta_x <= 0 && mainwind_delta_y >= 0)
        {
          if (mainwind_delta_x)
        ...
        else
        {
          if (mainwind_delta_x)
            draw(0, 0, mainwind_delta_x, window_height);
          if (mainwind_delta_y)
            draw(mainwind_delta_x, window_height + mainwind_delta_y, window_width, window_height);
        }
      }
      change_scrollbar();
    }
...
FlowArrayElem* FlowCtx::get_node_from_name(char* name)
{
  ...
  for (i = 0; i < a.a_size; i++)
  {
    if (a.a[i]->type() == flow_eObjectType_Node && streq(((FlowNode*)a.a[i])->n_name, name))
      return a.a[i];
  }
  return 0;
}
...
void FlowCtx::remove_trace_objects()
{
  ...
  for (i = 0; i < a.a_size; i++)
  {
    if (a.a[i]->type() == flow_eObjectType_Node && ((FlowNode*)a.a[i])->nc->group == flow_eNodeGroup_Trace)
    {
      e = a.a[i];
      remove(e);
      select_remove(e);
    ...
    if (a.a[i]->type() == flow_eObjectType_Con && ((FlowCon*)a.a[i])->cc->group == flow_eConGroup_Trace)
    {
      e = a.a[i];
      remove(e);
      select_remove(e);
...
int FlowCtx::trace_init(int (*connect_func)(void*, char*, char*, flow_eTraceType, void**),
...
FlowArrayElem* FlowCtx::get_document(double x, double y)
{
  ...
  for (i = 0; i < a.a_size; i++)
  {
    if (a.a[i]->type() == flow_eObjectType_Node &&
        ((FlowNode*)a.a[i])->nc->group == flow_eNodeGroup_Document && ((FlowNode*)a.a[i])->x_left < x &&
        ((FlowNode*)a.a[i])->x_right > x && ((FlowNode*)a.a[i])->y_low < y && ((FlowNode*)a.a[i])->y_high > y)
      return a.a[i];
  }
  return 0;
}
...
int FlowCtx::get_next_object(FlowArrayElem* object, flow_eDirection dir, FlowArrayElem** next)
{
  ...
  if (!object)
  {
    ...
    {
      if (a[i]->type() == flow_eObjectType_Node && ((FlowNode*)a[i])->get_group() != flow_eNodeGroup_Document)
      {
        NextElem n;
...
int FlowCtx::get_next_conpoint(FlowArrayElem* object, int cp_num, flow_eDirection dir, FlowArrayElem** next,
                               int* next_cp_num)
{
  int sts;
  int loop_cnt = 0;
...
void FlowCtx::change_scrollbar()
{
  ...
  data.window_width = int((double)window_width / scroll_size / zoom_factor) + 1;
  data.window_height = int((double)window_height / scroll_size / zoom_factor) + 1;
  data.offset_x = int((double)offset_x / scroll_size / zoom_factor - x_left / scroll_size);
  data.offset_y = int((double)offset_y / scroll_size / zoom_factor - y_low / scroll_size);
  ...
  else
  {
    ...
    else
    {
      ...
      else if (delta_x <= 0 && delta_y <= 0)
      {
        ...
        if (delta_y)
          draw(0, window_height + delta_y, window_width + delta_x, window_height);
      }
      else if (delta_x <= 0 && delta_y >= 0)
      {
        if (delta_x)
...
void flow_scroll_horizontal(FlowCtx* ctx, int value, int bottom)
{
  ...
  x_pix = int(-(double)value * ctx->scroll_size * ctx->zoom_factor +
...
void flow_scroll_vertical(FlowCtx* ctx, int value, int bottom)
{
  ...
  y_pix = ctx->offset_y -
          int((double)value * ctx->scroll_size * ctx->zoom_factor + ctx->y_low * ctx->zoom_factor);
  ctx->scroll(0, y_pix);
}
...
void FlowCtx::update_color_theme(int ct)
{
