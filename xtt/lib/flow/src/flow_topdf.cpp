...
void CnvContent::add(CnvContentElem& elem) { tab.push_back(elem); }
...
void CnvPdfObj::print_begin()
{
  ...
  case pdf_eObjType_Page:
    topdf->fp[topdf->cf] << number + topdf->v_outline.size() << " 0 obj\n"
                         ...
                         << "     /Mediabox [0 0 " << pdf_cPageWidth << " " << pdf_cPageHeight << "]\n"
...
int CnvPdfObj::print_image() { return 1; }

CnvToPdf::CnvToPdf() : cf(0), ci(0), prev_ci(0), conf_pass(false), xref_offset(0), im_cnt(0), use_outlines(1)
{
  ...
  style[pdf_eId_TitlePage].text = CnvStyle("Helvetica-ISOLatin1", 8, 0, 9, 1, cnv_eAlignment_Center, 0, 0, 0);
  ...
  style[pdf_eId_TopicL3].h2 = CnvStyle("Helvetica-Bold-ISOLatin1", 8, 0, 16, 5, cnv_eAlignment_Left, 0, 0, 0);
  style[pdf_eId_Function].h1 =
      CnvStyle("Helvetica-Bold-ISOLatin1", 24, 0, 24, 20, cnv_eAlignment_Left, 1, 0, 1);
  style[pdf_eId_Class].h1 = CnvStyle("Helvetica-Bold-ISOLatin1", 24, 0, 24, 20, cnv_eAlignment_Left, 1, 0, 1);
  style[pdf_eId_Class].h2 = CnvStyle("Helvetica-Bold-ISOLatin1", 20, 0, 24, 8, cnv_eAlignment_Left, 0, 0, 0);
  ...
  if (page_number[cf] != 1 || last)
  {
    ...
    if (!(prev_ci == pdf_eId_TitlePage || prev_ci == pdf_eId_InfoPage))
    {
      ...
      fp[cf] << "  1 w\n"
             ...
             << "  " << pdf_cPageWidth + 65 << " " << pdf_cPageHeight - 20 << " l\n"
             << "  S\n"
             << "  BT\n"
             << "    /F1 10 Tf\n"
             << "    " << pdf_cPageWidth / 2 - 10 * 0.5 * strlen(previous_chapter) << " " << pdf_cPageNumY
             << " Td\n"
             << "    (" << previous_chapter << ") Tj\n"
             << "  ET\n"
             << "  BT\n"
...
int CnvToPdf::print_image_inline(char* filename) { return 1; }

int CnvToPdf::print_image(char* filename) { return 1; }
...
void CnvToPdf::print_h2(char* text) { print_text(text, style[ci].h2); }

void CnvToPdf::print_h3(char* text) { print_text(text, style[ci].h3); }

void CnvToPdf::set_filename(int idx, char* name) { strcpy(filename[idx], name); }
