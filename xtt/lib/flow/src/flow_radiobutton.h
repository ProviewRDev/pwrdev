...
{
public:
  FlowRadiobutton(FlowCtx* flow_ctx, double x = 0, double y = 0, double w = 0, double h = 0,
                  int annot_num = 0, flow_eDrawType d_type = flow_eDrawType_Line, int line_w = 1)
      : ctx(flow_ctx), ll(flow_ctx, x, y), ur(flow_ctx, x + w, y + h), draw_type(d_type), line_width(line_w),
        number(annot_num)
  {
  }
