...
public:
  FlowLine() {}
  FlowLine(FlowCtx* flow_ctx, double x1 = 0, double y1 = 0, double x2 = 0, double y2 = 0,
           flow_eDrawType d_type = flow_eDrawType_Line, int line_w = 1, int fix_line_w = 0)
      : ctx(flow_ctx), p1(flow_ctx, x1, y1), p2(flow_ctx, x2, y2), draw_type(d_type), line_width(line_w),
        fix_line_width(fix_line_w)
  {
  }
  ...
  void draw_inverse(void* pos, int hot, void* node) { erase(pos, hot, node); }
  void erase(void* pos, int hot, void* node);
  void nav_erase(void* pos, void* node);
  void get_borders(double pos_x, double pos_y, double* x_right, double* x_left, double* y_high, double* y_low,
                   void* node);
  void move(void* pos, double x1, double y1, double x2, double y2, int highlight, int dimmed, int hot);
