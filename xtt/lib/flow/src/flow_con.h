...
{
public:
  FlowCon(FlowCtx* flow_ctx, const char* name, FlowConClass* con_class, FlowNode* source, FlowNode* dest,
          int source_cp, int dest_cp, int* rsts, int nodraw = 0, int point_num = 0, double* x_vect = 0,
          double* y_vect = 0);
  ~FlowCon();
  FlowCon() {}
  ...
  int con_route(double src_x, double src_y, flow_eDirection src_dir, double dest_x, double dest_y,
                flow_eDirection dest_dir);
  int con_route_noobstacle(double src_x, double src_y, flow_eDirection src_dir, double dest_x, double dest_y,
                           flow_eDirection dest_dir);
  int con_route_grafcet(flow_eConType con_type, double src_x, double src_y, double dest_x, double dest_y);
  ...
  int find_horiz_line_up(double check_y, double check_l_x, double check_u_x, FlowNode* nodelist,
                         FlowNode* next_node, FlowCon* conlist, FlowCon* next_con, double wind_ll_x,
                         double wind_ur_x);
  int find_horiz_line_down(double check_y, double check_l_x, double check_u_x, FlowNode* nodelist,
                           FlowNode* next_node, FlowCon* conlist, FlowCon* next_con, double wind_ll_x,
                           double wind_ur_x);
  int find_vert_line_right(double check_x, double check_l_y, double check_u_y, FlowNode* nodelist,
                           FlowNode* next_node, FlowCon* conlist, FlowCon* next_con, double wind_ll_y,
                           double wind_ur_y);
  int find_vert_line_left(double check_x, double check_l_y, double check_u_y, FlowNode* nodelist,
                          FlowNode* next_node, FlowCon* conlist, FlowCon* next_con, double wind_ll_y,
                          double wind_ur_y);
  ...
  int sort_lines(double dest_x, double dest_y, flow_eDirection dest_dir, double src_x, double src_y,
  ...
  void set_movement_type(flow_eMoveType move_type) { movement_type = move_type; }
  int is_connected_to(FlowNode* node) { return source_node == node || dest_node == node; }
};
