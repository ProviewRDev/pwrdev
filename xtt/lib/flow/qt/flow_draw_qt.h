...
public:
  ...
  FlowDrawQt(QWidget* toplevel, void** flow_ctx,
             int (*init_proc)(QWidget* w, FlowCtx* ctx, void* client_data), void* client_data,
             flow_eCtxType type);
  ...
  int get_text_extent(FlowCtx* ctx, const char* text, int len, flow_eDrawType gc_type, int idx, int* width,
                      int* height, double size);
  ...
  int text_pango(FlowCtx* ctx, int x, int y, char* text, int len, flow_eDrawType gc_type, int idx,
                 int highlight, int dimmed, int line, double size);
  int text_inverse_pango(FlowCtx* ctx, int x, int y, char* text, int len, flow_eDrawType gc_type, int idx,
                         int line, double size);
  int text_erase_pango(FlowCtx* ctx, int x, int y, char* text, int len, flow_eDrawType gc_type, int idx,
                       int line, double size);
  int get_text_extent_pango(FlowCtx* ctx, const char* text, int len, flow_eDrawType gc_type, int idx,
                            double size, int* width, int* height);
...
private:
  ...
  int rect_helper(FlowCtx* ctx, int painter_type, int size, int x, int y, int width, int height,
                  bool nav = false, bool fill = false);
  int triangle_helper(FlowCtx* ctx, int painter_type, int size, int x, int y, int width, int height,
                      bool nav = false, bool fill = false);
  int arrow_helper(FlowCtx* ctx, int painter_type, int size, int x1, int y1, int x2, int y2, int x3, int y3,
                   bool nav = false);
  int arc_helper(FlowCtx* ctx, int painter_type, int size, int x, int y, int width, int height, int angle1,
                 int angle2, bool nav = false);
  int line_helper(FlowCtx* ctx, int painter_type, int size, int x1, int y1, int x2, int y2, bool nav = false);
  int text_pango_helper(FlowCtx* ctx, int x, int y, char* text, int len, flow_eDrawType painter_type, int idx,
                        double size, int dimmed = 0, int erase_rect = 0);
  int nav_text_helper(FlowCtx* ctx, flow_eDrawType painter_type, int size, int x, int y, char* text, int len);
