...
int co_xml_interpreter::tag(const char* name) { return 0; }

int co_xml_interpreter::metatag(const char* name) { return 0; }

int co_xml_interpreter::tag_end(const char* name) { return 0; }

int co_xml_interpreter::metatag_end(const char* name) { return 0; }

int co_xml_interpreter::tag_value(const char* value) { return 0; }

int co_xml_interpreter::tag_attribute(const char* name, const char* value) { return 0; }

co_xml_parser::co_xml_parser(co_xml_interpreter* i)
    : interpreter(i), logglevel(0), first_token(true), state(xml_eState_Init), line_cnt(1), char_cnt(0),
      in_comment(false), c(0), c_f(0), c_ff(0), c_sts(0), c_f_sts(0), c_ff_sts((void*)1), current_tag_idx(0),
      current_attribute_name_idx(0), current_attribute_value_idx(0), current_tag_value_idx(0)
{
...
bool co_xml_parser::is_space(const char c) { return c == ' ' || c == '	' || c == '\n' || c == '\r'; }
      ...
      else
      {
        if (state & xml_eState_TagName)
        {
          ...
          else if (c == '?' && c_f == '>')
          {
            ...
            next_token();
            current_tag[current_tag_idx] = 0;
            if (state & xml_eState_AttributeName || state & xml_eState_AttributeValue)
