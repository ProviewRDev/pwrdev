...
#define dbs_dAlignPage(size) (((size) + dbs_cAlignPage) & ~dbs_cAlignPage)
  ...
  typedef union
  {
    struct
    {
      pwr_Endian_4(pwr_Bits(offs, dbs_cOffsBits), pwr_Bits(sect, dbs_cSectBits), , )
    } b;
    dbs_tRef m;
  } dbs_uRefBits;
...
#define dbs_mSect_fixup pwr_Bit(dbs_eSect_fixup)
  ...
  {
    pwr_tBitMask m;
    pwr_32Bits(pwr_Bits(isAliasClient, 1), pwr_Bits(isMountClient, 1), pwr_Bits(isMountClean, 1),
               pwr_Bits(devOnly, 1), pwr_Bits(readOnly, 1), pwr_Bits(fill0, 3), , ,
