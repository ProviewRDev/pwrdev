...
int statussrv_client::server_connect()
{
  ...
  sts = setsockopt(m_sock, SOL_SOCKET, SO_RCVBUF, &buffsize, sizeof(buffsize));
  ...
  *msg = (char*)rb;
  ...
  { /* Receive answer */
    ...
    */
    ...
    while ((remaining_data > 0) && (sts > 0))
    { // if there is data to read and
      ...
      { // There are something to read (no timeout and no error). Could
        // be a closed socket too, so we have to check later anyway.
        data_size = recv(m_sock, &rcv_buffer[received_data], remaining_data, 0);
