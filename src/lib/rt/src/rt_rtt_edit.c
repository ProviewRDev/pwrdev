...
static int dtt_create_ctx(edit_ctx* ctx, edit_ctx parent_ctx, char* title, int index);
static int dtt_edit_write(edit_ctx ctx, char* filename, char* menu_filename, int userspec_filename,
                          int nocompile, int generate_only, int opsys, int return_wait, int* messages);
...
static int dtt_upd_item_show(edit_ctx parent_ctx, int arg1, int arg2, int arg3, int arg4);
...
static int rtt_edit_draw_section(rtt_t_backgr* chartable, int x1, int y1, int x2, int y2, int inverse);
static int dtt_edit_delete_section(rtt_t_backgr* chartable, dtt_item itemlist, int x1, int y1, int x2,
                                   int y2);
static int dtt_paste_insert(rtt_t_backgr* chartable, dtt_item itemlist, int x1, int y1, int x2, int y2);
static int dtt_paste_copy(rtt_t_backgr* chartable, dtt_item* itemlist_ptr, int x, int y);
static int dtt_paste_item_insert(dtt_item item, int x, int y);
static int dtt_paste_item_copy(dtt_item paste_item, dtt_item* itemlist_ptr, int x, int y);
static int dtt_edit_save_menues(char* filename, int generate_only);
static int dtt_edit_save_one_menu(FILE* fout, FILE* fout_c, FILE* fout_h, FILE* fout_db1, FILE* fout_db2,
                                  FILE* fout_decl, menu_ctx ctx, int index, int generate_only, int nocompile);
static int dtt_edit_read_menues(char* filename);
static int dtt_edit_read_one_menu(FILE* fin, menu_ctx parent_ctx, int parent_item);
...
static int dtt_edit_process_menues(int (*func)(), unsigned long arg1, unsigned long arg2, unsigned long arg3,
                                   unsigned long arg4, unsigned long arg5);
static int dtt_edit_process_one_menu(int (*func)(), unsigned long arg1, unsigned long arg2,
                                     unsigned long arg3, unsigned long arg4, unsigned long arg5, menu_ctx ctx,
                                     int index);
static int dtt_edit_write_gdhrefs(char* filename);
static int dtt_edit_write_menu_gdhrefs(menu_ctx ctx, int menu_idx, FILE* outfile, int dum1, int dum2,
                                       int dum3, int dum4);
static int dtt_edit_pwrplc_gdhrefs(char* filename);
static int dtt_edit_pwrplc_menu_gdhrefs(menu_ctx ctx, int menu_idx, FILE* outfile, int* externref_index,
                                        int dum2, int dum3, int dum4);
static int dtt_edit_list_items_all(char* filename);
static int dtt_edit_list_items_picture(menu_ctx ctx, int menu_idx, FILE* outfile, int dum1, int dum2,
                                       int dum3, int dum4);
...
static int dtt_edit_save_picture(menu_ctx ctx, int menu_idx, int keep_ctx, int index, int generate_only,
                                 int opsys, int dum4);
static int dtt_edit_convert_picture(menu_ctx ctx, int menu_idx, int keep_ctx, int index, int dum2, int dum3,
                                    int dum4);
...
static int dtt_edit_include_one_menu(FILE* fin, menu_ctx parent_ctx, int parent_item);
static int dtt_check_if_function_menu(menu_ctx ctx, int menu_idx, int* function_found, int dum1, int dum2,
                                      int dum3, int dum4);
static int dtt_function_menues_exist();
static int dtt_show_menu(menu_ctx ctx, char* menu_name);
static int dtt_command_get_input(char* chn, char* out_string, unsigned long* out_terminator, int out_maxlen,
                                 unsigned long option, int timeout);
static char* dtt_quotes_convert(char* str);
static int dtt_edit_print_syntaxerror(dtt_item item_ptr, char* error_text, int error, int* error_count,
                                      int* warning_count);
static int dtt_edit_check_items_syntax(edit_ctx picture_ctx, int* error_count, int* warning_count);
static int dtt_edit_save_by_index(int index, int generate_only, int nocompile, int opsys);
static int dtt_edit_draw_position(edit_ctx ctx);
static int dtt_menu_item_show(menu_ctx ctx, int arg1, int arg2, int arg3, int arg4);
...
static int write_func(edit_ctx ctx, int flag)
{
  ...
  if (ODD(rtt_get_qualifier("rtt_arg1", arg1_str)))
  {
    if (str_NoCaseStrncmp(arg1_str, "ITEMS", strlen(arg1_str)) == 0)
    {
      ...
      {
        if ((ctx->ctx_type == DTT_CTX_EDIT) ||
            ((ctx->parent_ctx != 0) && (((edit_ctx)(ctx->parent_ctx))->ctx_type == DTT_CTX_EDIT)))
        {
    ...
    else if (str_NoCaseStrncmp(arg1_str, "PICTURE", strlen(arg1_str)) == 0)
    {
          ...
          ((ctx->parent_ctx != 0) && (((edit_ctx)(ctx->parent_ctx))->ctx_type == DTT_CTX_EDIT)))
      {
        ...
        sts = dtt_edit_write(ctx, filename, menu_filename, 1, 1, 0, 0, 0, &messages);
        if (EVEN(sts))
        {
          char tmp[512];
...
static int save_func(edit_ctx ctx, int flag)
{
  ...
  if (ODD(rtt_get_qualifier("/ALL", arg1_str)))
  {
    ...
    else
    {
      ...
      else if (str_NoCaseStrncmp(arg1_str, "X86_64_FREEBSD", strlen(arg1_str)) == 0)
  ...
  else
  {
        ...
        ((ctx->parent_ctx != 0) && (((edit_ctx)(ctx->parent_ctx))->ctx_type == DTT_CTX_EDIT)))
    {
      ...
      if (EVEN(rtt_get_qualifier("rtt_arg1", arg1_str)))
      {
        sts = dtt_edit_write(ctx, filename, menu_filename, 0, nocompile, 0, dtt_opsys, 1, &messages);
      }
      else
      {
        if (str_NoCaseStrncmp(arg1_str, "PPC_LINUX", strlen(arg1_str)) == 0)
        ...
        sts = dtt_edit_write(ctx, filename, menu_filename, 0, nocompile, 0, opsys, 1, &messages);
      }
      if (EVEN(sts))
      {
        char tmp[512];
...
static int syntax_func(edit_ctx ctx, int flag)
{
      ...
      ((ctx->parent_ctx != 0) && (((edit_ctx)(ctx->parent_ctx))->ctx_type == DTT_CTX_EDIT)))
  {
    ...
    if ((error_count || warning_count) && !(rtt_commandmode & RTT_COMMANDMODE_FILE))
    {
      rtt_wait_for_return();
      return RTT__SUCCESS;
    }
...
static int dtt_exit_func(edit_ctx ctx, int flag)
{
      ...
      ((ctx->parent_ctx != 0) && (((edit_ctx)(ctx->parent_ctx))->ctx_type == DTT_CTX_EDIT)))
  {
    ...
    sts = dtt_edit_write(ctx, filename, menu_filename, 0, 0, 0, dtt_opsys, 1, &messages);
    if (EVEN(sts))
    {
      char tmp[512];
      snprintf(tmp, sizeof(tmp), "Unable to open file \"%s\"", filename);
...
static int dtt_export_func(edit_ctx ctx, int flag)
{
  ...
  if (ODD(rtt_get_qualifier("rtt_arg1", arg1_str)))
  {
    ...
    else if (str_NoCaseStrncmp(arg1_str, "EXTERNREF", strlen(arg1_str)) == 0)
    {
...
static int dtt_connect_func(edit_ctx ctx, int flag)
{
  ...
  {
    /* Get update item */
    sts = dtt_get_upd_item(ctx->upd_items, ctx->cursor_x, ctx->cursor_y, &item_ptr);
    if (EVEN(sts))
    {
...
static int dtt_link_func(edit_ctx ctx, int flag)
{
  ...
  if (EVEN(rtt_get_qualifier("rtt_arg1", arg1_str)))
  {
    ...
    {
      /* Link current opsys */
      sprintf(msg, "Linking %s%s...", debug ? "with debug " : "", dtt_opsys_to_name(dtt_current_opsys));
      rtt_message('I', msg);
      rtt_cursor_abs(0, 21);
      r_print_buffer();
    ...
    else
    {
      for (opsys = 1; opsys < pwr_mOpSys_; opsys = opsys << 1)
      {
        if (dtt_opsys & opsys)
        {
          sprintf(msg, "Linking %s%s...", debug ? "with debug " : "", dtt_opsys_to_name(opsys));
          rtt_message('I', msg);
          rtt_cursor_abs(0, 21);
          r_print_buffer();
  ...
  else
  {
    ...
    if (dtt_opsys & opsys || (!dtt_opsys && dtt_current_opsys == opsys))
    {
      ...
      sprintf(msg, "Program linked for %s %s", dtt_opsys_to_name(opsys), debug ? "with debug" : "");
      rtt_message('I', msg);
    }
    else
    {
...
static int dtt_compile_func(edit_ctx ctx, int flag)
{
  ...
  if (EVEN(rtt_get_qualifier("rtt_arg1", arg1_str)))
  {
    if (!dtt_opsys)
    {
      ...
      sprintf(msg, "Compiling %s%s...", debug ? "with debug " : "", dtt_opsys_to_name(dtt_current_opsys));
      rtt_message('I', msg);
      rtt_cursor_abs(0, 21);
      r_print_buffer();
    ...
    else
    {
      for (opsys = 1; opsys < pwr_mOpSys_; opsys = opsys << 1)
      {
        if (dtt_opsys & opsys)
        {
          sprintf(msg, "Compiling %s%s...", debug ? "with debug " : "", dtt_opsys_to_name(opsys));
          rtt_message('I', msg);
          rtt_cursor_abs(0, 21);
          r_print_buffer();
...
static int dtt_run_func(edit_ctx ctx, int flag) { return RTT__SUCCESS; }
/*************************************************************************
 *
...
static int dtt_delete_func(edit_ctx ctx, int flag)
{
  ...
  if (EVEN(rtt_get_qualifier("rtt_arg1", arg1_str)))
  {
    ...
    {
      sts = dtt_edit_delete_section((rtt_t_backgr*)ctx->chartable, ctx->upd_items, ctx->cursor_x,
                                    ctx->cursor_y, ctx->select_x, ctx->select_y);
...
static int dtt_paste_func(edit_ctx ctx, int flag)
{
  ...
  sts = dtt_paste_copy((rtt_t_backgr*)ctx->chartable, &(ctx->upd_items), ctx->cursor_x, ctx->cursor_y);
  rtt_edit_draw_section(&(ctx->chartable), ctx->cursor_x, ctx->cursor_y, ctx->cursor_x + dtt_paste_size_x,
...
static int dtt_cut_func(edit_ctx ctx, int flag)
{
  ...
  if (ctx->select_active)
  {
    sts = dtt_paste_insert((rtt_t_backgr*)ctx->chartable, ctx->upd_items, ctx->cursor_x, ctx->cursor_y,
                           ctx->select_x, ctx->select_y);
    sts = dtt_edit_delete_section((rtt_t_backgr*)ctx->chartable, ctx->upd_items, ctx->cursor_x, ctx->cursor_y,
                                  ctx->select_x, ctx->select_y);
    dtt_edit_unselect(ctx);
    return RTT__NOPICTURE;
  }
...
static int create_func(edit_ctx ctx, int flag)
{
  ...
  if (ctx->ctx_type == DTT_CTX_EDIT)
  {
    ...
    if (ODD(rtt_get_qualifier("/TIME", arg1_str)))
    {
      ...
      sts = dtt_upd_item_add(&(ctx->upd_items), ctx->cursor_x, ctx->cursor_y, text_str);
      if (EVEN(sts))
        return sts;
      sts = dtt_get_upd_item(ctx->upd_items, ctx->cursor_x, ctx->cursor_y, &item_ptr);
      if (EVEN(sts))
  ...
  else
  {
    ...
    if (EVEN(rtt_get_qualifier("/CHILD", arg1_str)))
    {
      ...
      sts = rtt_menu_list_insert(&(m_ctx->menu), new_item, text_str, 0, 0, 0, pwr_cNObjid, 0, 0, 0, 0);
      if (EVEN(sts))
        return sts;
    ...
    else
    {
      ...
      if (!(parent_menu_ptr->type == DTT_MENUITEM_MENU || parent_menu_ptr->type == DTT_MENUITEM_KEYS))
      {
      ...
      {
        /* Create a new menu */
        sts = rtt_menu_list_add(&menulist, 0, 0, text_str, 0, 0, 0, pwr_cNObjid, 0, 0, 0, 0);
        if (EVEN(sts))
          return sts;
        ...
        /* Create a context */
        sts = rtt_menu_create_ctx(&newctx, m_ctx, menulist, parent_menu_ptr->text,
      ...
      else
      {
        ...
        sts = rtt_menu_list_add(&(newctx->menu), size, 0, text_str, 0, 0, 0, pwr_cNObjid, 0, 0, 0, 0);
        if (EVEN(sts))
          return sts;
...
static int modify_func(edit_ctx ctx, int flag)
{
  ...
  if (ctx->ctx_type == DTT_CTX_EDIT)
  {
    ...
    {
      if ((str_NoCaseStrcmp(arg1_str, "UPDATE") == 0) || (str_NoCaseStrcmp(arg1_str, "SET") == 0) ||
          (str_NoCaseStrcmp(arg1_str, "RESET") == 0) || (str_NoCaseStrcmp(arg1_str, "SET_RESET") == 0) ||
          (str_NoCaseStrcmp(arg1_str, "TOGGLE") == 0) || (str_NoCaseStrcmp(arg1_str, "DUAL_SET") == 0) ||
    ...
    {
      if ((str_NoCaseStrcmp(arg1_str, "OP") == 0) || (str_NoCaseStrcmp(arg1_str, "NO") == 0) ||
          (str_NoCaseStrcmp(arg1_str, "EL") == 0) || (str_NoCaseStrcmp(arg1_str, "PROC") == 0) ||
    ...
    {
      if ((str_NoCaseStrcmp(arg1_str, "ONOFF") == 0) || (str_NoCaseStrcmp(arg1_str, "NO") == 0) ||
          (str_NoCaseStrcmp(arg1_str, "TRUEFALSE") == 0) || (str_NoCaseStrcmp(arg1_str, "AUTOMAN") == 0) ||
          (str_NoCaseStrcmp(arg1_str, "") == 0) || (str_NoCaseStrcmp(arg1_str, "BAR") == 0) ||
          (str_NoCaseStrcmp(arg1_str, "TEXT") == 0) || (str_NoCaseStrcmp(arg1_str, "FLASHTEXT") == 0) ||
    ...
    {
      if ((str_NoCaseStrcmp(arg1_str, "GDH") == 0) || (str_NoCaseStrcmp(arg1_str, "RTT") == 0) ||
    ...
    {
      if ((str_NoCaseStrcmp(arg1_str, "INT") == 0) || (str_NoCaseStrcmp(arg1_str, "SHORT") == 0) ||
          (str_NoCaseStrcmp(arg1_str, "BOOLEAN") == 0) || (str_NoCaseStrcmp(arg1_str, "FLOAT") == 0) ||
          (str_NoCaseStrcmp(arg1_str, "STRING") == 0) || (str_NoCaseStrcmp(arg1_str, "STRING4") == 0) ||
          (str_NoCaseStrcmp(arg1_str, "STRING10") == 0) || (str_NoCaseStrcmp(arg1_str, "STRING20") == 0) ||
          (str_NoCaseStrcmp(arg1_str, "STRING40") == 0) || (str_NoCaseStrcmp(arg1_str, "OBJID") == 0) ||
          (str_NoCaseStrcmp(arg1_str, "ATTRREF") == 0) || (str_NoCaseStrcmp(arg1_str, "TIME") == 0) ||
  ...
  else if ((ctx->parent_ctx != 0) && (((edit_ctx)(ctx->parent_ctx))->ctx_type == DTT_CTX_EDIT))
  {
...
static int dtt_show_func(menu_ctx ctx, int flag)
{
  ...
  else if (str_NoCaseStrncmp(arg1_str, "TITLEPREFIX", strlen(arg1_str)) == 0)
  {
  ...
  else if (str_NoCaseStrncmp(arg1_str, "COMMAND", strlen(arg1_str)) == 0)
  {
    ...
    if (!(menu_ptr->type != DTT_MENUITEM_COMMAND || menu_ptr->type != DTT_MENUITEM_COMMANDHOLD ||
          menu_ptr->type != DTT_MENUITEM_FILESPEC))
    {
      rtt_message('E', "This is not a command menu item");
      return RTT__NOPICTURE;
  ...
  else if (str_NoCaseStrncmp(arg1_str, "FUNCTION", strlen(arg1_str)) == 0)
  {
    ...
    menu_ptr = ctx->menu;
    menu_ptr += ctx->current_item;
    if (!(menu_ptr->type == DTT_MENUITEM_PIFUNC || menu_ptr->type == DTT_MENUITEM_PIFUNCPERM))
    {
      rtt_message('E', "This is not a function menu item");
      return RTT__NOPICTURE;
    }
...
static rtt_t_comtbl dtt_command_table[] = {
    ...
    {"PRINT", &rttcmd_print_func, {"rtt_arg1", "/FILE", "/APPEND", "/TSIZE", "/PSIZE", "/TEXT", ""}},
    {"SAY", &rttcmd_say_func, {"rtt_arg1", "/TEXT", ""}},
    {"DEFINE", &rttcmd_define_func, {"rtt_arg1", "rtt_arg2", "rtt_arg4", "rtt_arg4", ""}},
    {"SET", &set_func, {"rtt_arg1", "rtt_arg2", "/NAME", "/ON", "/OFF", "/MESSAGE", "/COMMAND", ""}},
    ...
    {"SYNTAX", &syntax_func, {""}},
    {"SETUP", &dtt_setup_func, {""}},
    {"CREATE",
     &create_func,
     {"rtt_arg1", "/TEXT", "/MENU", "/PICTURE", "/CHILD", "/EXIT", "/OBJECTHIERARCHY", "/COMMAND",
      "/COMMANDHOLD", "/FUNCTION", "/TIME", "/FULLTIME", "/SYSPICTURE", "/OBJPICTURE", "/NAME",
      "/PERMPICTURE", "/KEYS", "/FILESPEC", ""}},
    {"MODIFY",
     &modify_func,
     {"rtt_arg1", "/NUMBER", "/TEXT", "/TYPE", "/PARAMETER", "/DUALPARAMETER", "/PRIVILEGES", "/OUTPUTFLAGS",
      "/MAINTITLE", "/MAXLIMIT", "/MINLIMIT", "/CHARACTERS", "/DECIMALS", "/DATABASE", "/DECLARATION",
      "/TITLEPREFIX", ""}},
    {"EXIT", &dtt_exit_func, {"rtt_arg1", ""}},
    {"QUIT",
     &dtt_quit_func,
     {
         "",
     }},
    {"HELP", &dtt_help_func, {"rtt_arg1", "rtt_arg2", "rtt_arg3", "rtt_arg4", ""}},
...
static int dtt_edit_draw(edit_ctx ctx)
{
  ...
  if (ctx->select_active)
    rtt_edit_draw_section(&(ctx->chartable), ctx->cursor_x, ctx->cursor_y, ctx->select_x, ctx->select_y, 1);
  ...
  if (item_ptr != 0)
  {
    ...
    while (item_ptr->number != 0)
    {
      ...
      case DTT_DISPLAY_MODE_FORMAT:
        ...
        if (str_NoCaseStrcmp(item_ptr->outflags, "NO") != 0 && !streq(item_ptr->text, "%"))
          r_print(" ");
        ...
        {
          if (item_ptr->decimals > 0 && i == item_ptr->characters - item_ptr->decimals - 1)
            r_print(".");
          else
            r_print("F");
        }
                 ...
                 unsigned long arg4)
{
  ...
  option = RTT_OPT_NORECALL | RTT_OPT_NOEDIT | RTT_OPT_NOECHO | RTT_OPT_NOSCROLL;
  ...
  while (1)
  {
    ...
    rtt_cursor_abs(ctx->cursor_x + 1, ctx->cursor_y + 1);
    r_print_buffer();
    dtt_command_get_input((char*)rtt_chn, input_str, &terminator, maxlen, option, 0);
    rtt_message('S', "");
    c = input_str[0];
    rtt_cursor_abs(ctx->cursor_x + 1, ctx->cursor_y + 1);
      ...
      if (ctx->cursor_x > 0)
      {
        ...
        if (ctx->select_active)
        {
          if (ctx->cursor_x >= ctx->select_x)
            rtt_edit_draw_section(&(ctx->chartable), ctx->cursor_x + 1, ctx->cursor_y, ctx->cursor_x + 1,
                                  ctx->select_y, 0);
          else
            rtt_edit_draw_section(&(ctx->chartable), ctx->cursor_x, ctx->cursor_y, ctx->cursor_x,
                                  ctx->select_y, 1);
          rtt_cursor_abs(ctx->cursor_x + 1, ctx->cursor_y + 1);
        }
      }
      ...
      if (ctx->cursor_y > 0)
      {
        ...
        if (ctx->select_active)
        {
          if (ctx->cursor_y >= ctx->select_y)
            rtt_edit_draw_section(&(ctx->chartable), ctx->cursor_x, ctx->cursor_y + 1, ctx->select_x,
                                  ctx->cursor_y + 1, 0);
          else
            rtt_edit_draw_section(&(ctx->chartable), ctx->cursor_x, ctx->cursor_y, ctx->select_x,
                                  ctx->cursor_y, 1);
          rtt_cursor_abs(ctx->cursor_x + 1, ctx->cursor_y + 1);
        }
      }
    ...
    {
      dtt_edit_normal_settings();
      sts = rtt_get_command((menu_ctx)ctx, (char*)&rtt_chn, rtt_recallbuff, 0, 0, ctx, "rtt_edit> ", 0,
                            RTT_ROW_COMMAND, dtt_command_table);
                          ...
                          int nocompile, int generate_only, int opsys, int return_wait, int* messages)
{
  ...
  if (!generate_only)
  {
    ...
    for (i = 0; i < (int)(sizeof(ctx->chartable) / sizeof(ctx->chartable[0])); i++)
    {
      for (j = 0; j < (int)(sizeof(ctx->chartable[0]) / sizeof(ctx->chartable[0][0])); j++)
      {
        for (k = 0; k < (int)(sizeof(ctx->chartable[0][0]) / sizeof(ctx->chartable[0][0][0])); k++)
        {
  ...
  sts = dtt_edit_check_items_syntax(ctx, &error_count, &warning_count);
  if ((error_count || warning_count) && return_wait && !(rtt_commandmode & RTT_COMMANDMODE_FILE))
  {
    rtt_wait_for_return();
    *messages = 1;
  }
  ...
  *s = 0;
  ...
  if (item_ptr != 0)
  {
    while (item_ptr->number != 0)
    {
      ...
      if (str_NoCaseStrcmp(item_ptr->type, "UPDATE") == 0)
        ...
        fprintf(fout, "&rtt_menu_parameter_dual_set,&rtt_menu_parameter_dual_reset,\n");
        ...
        fprintf(fout, "&rtt_menu_parameter_set,&rtt_menu_parameter_dual_set,\n");
      else if (str_NoCaseStrcmp(item_ptr->type, "TOGGLE_DUALTOGGLE") == 0)
        fprintf(fout, "&rtt_menu_parameter_toggle,&rtt_menu_parameter_dual_toggle,\n");
          ...
          (str_NoCaseStrcmp(item_ptr->database, "RTTSYS") == 0))
      {
        if (!((str_NoCaseStrcmp(item_ptr->parameter, "RTT_TIME") == 0) ||
              ...
              (str_NoCaseStrcmp(item_ptr->parameter, "RTT_ALARMTEXT1") == 0) ||
              ...
              (str_NoCaseStrcmp(item_ptr->parameter, "RTT_ALARMTEXT3") == 0)))
        {
          ...
          fprintf(fout_db2, "{\"%s\", (char *) &%s},\n", item_ptr->parameter, item_ptr->parameter);
        }
      }
      item_ptr++;
    }
  ...
  fprintf(fout, "{ \"\",}};\n");
  fprintf(fout,
          "pwr_dExport rtt_t_menu_update *%s_euptr = (rtt_t_menu_update "
          "*)  &%s_eu;\n",
    ...
    for (j = 0; j < (int)(sizeof(ctx->chartable[0]) / sizeof(ctx->chartable[0][0])); j++)
    {
      ...
      sscanf(line, "%d %d %d %d %d %d %d %d %d %d \
			%d %d %d %d %d %d %d %d %d %d %d %d",
             &ctx->chartable[i][j][0], &ctx->chartable[i][j][1], &ctx->chartable[i][j][2],
             &ctx->chartable[i][j][3], &ctx->chartable[i][j][4], &ctx->chartable[i][j][5],
             &ctx->chartable[i][j][6], &ctx->chartable[i][j][7], &ctx->chartable[i][j][8],
             &ctx->chartable[i][j][9], &ctx->chartable[i][j][10], &ctx->chartable[i][j][11],
             &ctx->chartable[i][j][12], &ctx->chartable[i][j][13], &ctx->chartable[i][j][14],
             &ctx->chartable[i][j][15], &ctx->chartable[i][j][16], &ctx->chartable[i][j][17],
             &ctx->chartable[i][j][18], &ctx->chartable[i][j][19], &ctx->chartable[i][j][20],
...
static int dtt_upd_item_show(edit_ctx parent_ctx, int arg1, int arg2, int arg3, int arg4)
{
  ...
  sts = dtt_get_upd_item(parent_ctx->upd_items, parent_ctx->cursor_x, parent_ctx->cursor_y, &item_ptr);
  if (EVEN(sts))
  {
  ...
  sts = rtt_menu_list_add(&menulist, i, 0, text, 0, 0, 0, pwr_cNObjid, item_ptr, 0, 0, 0);
  if (EVEN(sts))
    return sts;
  i++;
  ...
  sts = dtt_menu_new((menu_ctx)parent_ctx, &menulist, title, 0, RTT_MENUTYPE_DYN);
  if (sts == RTT__FASTBACK)
    return sts;
  else if (EVEN(sts))
    return sts;
...
static int dtt_menu_item_show(menu_ctx ctx, int arg1, int arg2, int arg3, int arg4)
{
  ...
  sts = rtt_menu_list_add(&menulist, i, 0, text, 0, 0, 0, pwr_cNObjid, menu_ptr, 0, 0, 0);
  if (EVEN(sts))
    return sts;
  i++;
                 ...
                 unsigned long flag)
{
  ...
  while (1)
  {
    rtt_command_get_input_string((char*)&rtt_chn, input_str, &terminator, maxlen, rtt_recallbuff, option, 0,
                                 0, ctx, NULL, RTT_COMMAND_PICTURE);
    /*
    ...
    */
    ...
    if (terminator == RTT_K_RETURN)
    {
      if ((menu_ptr + ctx->current_item)->func != NULL)
      {
        sts = ((menu_ptr + ctx->current_item)->func)(
            ctx, (menu_ptr + ctx->current_item)->arg1, (menu_ptr + ctx->current_item)->arg2,
            (menu_ptr + ctx->current_item)->arg3, (menu_ptr + ctx->current_item)->arg4);
        if (EVEN(sts))
          return sts;
        menu_ptr = ctx->menu;
        if (sts == RTT__FASTBACK)
    ...
    if (terminator == RTT_K_PF1)
    {
      ...
      {
        sts = ((menu_ptr + ctx->current_item)->func2)(
            ctx, (menu_ptr + ctx->current_item)->arg1, (menu_ptr + ctx->current_item)->arg2,
            (menu_ptr + ctx->current_item)->arg3, (menu_ptr + ctx->current_item)->arg4);
        if (EVEN(sts))
          return sts;
    ...
    if (terminator == RTT_K_COMMAND)
    {
      sts = rtt_get_command(ctx, (char*)&rtt_chn, rtt_recallbuff, 0, 0, ctx, "rtt_edit> ", 0, RTT_ROW_COMMAND,
                            dtt_command_table);
      if (EVEN(sts))
        return sts;
    ...
    if (terminator == RTT_K_HELP)
    {
      if ((ctx->parent_ctx != 0) && (((edit_ctx)(ctx->parent_ctx))->ctx_type == DTT_CTX_EDIT))
...
static int dtt_edit_select(edit_ctx ctx)
{
  ...
  ctx->select_active = 1;
  rtt_edit_draw_section(&(ctx->chartable), ctx->cursor_x, ctx->cursor_y, ctx->cursor_x, ctx->cursor_y, 1);
  return RTT__SUCCESS;
}
...
static int dtt_edit_unselect(edit_ctx ctx)
{
  ...
  ctx->select_active = 0;
  rtt_edit_draw_section(&(ctx->chartable), ctx->cursor_x, ctx->cursor_y, ctx->select_x, ctx->select_y, 0);
  dtt_edit_draw_itemlist(ctx);
  ...
  for (i = y1; i < y2 + 1; i++)
  {
    for (j = x1; j < x2 + 1; j++)
    {
      if (j < 80 && i < 22)
      {
        ...
        else if (!(c1 & RTT_CHARSET_LINE) && (current_charset != RTT_CHARSET_ASCII))
        {
          rtt_charset_ascii();
          current_charset = RTT_CHARSET_ASCII;
        }
        if (!inverse)
        {
          if ((c1 & RTT_CHARSET_INVERSE) && (current_inverse != RTT_CHARSET_INVERSE))
          {
            rtt_char_inverse_start();
            current_inverse = RTT_CHARSET_INVERSE;
          }
          else if (!(c1 & RTT_CHARSET_INVERSE) && (current_inverse != RTT_CHARSET_NOINVERSE))
          {
            rtt_char_inverse_end();
            current_inverse = RTT_CHARSET_NOINVERSE;
          }
...
static int dtt_edit_delete_section(rtt_t_backgr* chartable, dtt_item itemlist, int x1, int y1, int x2, int y2)
{
  int i, j;
  int dum;
  dtt_item item_ptr;
...
static int dtt_paste_copy(rtt_t_backgr* chartable, dtt_item* itemlist_ptr, int x, int y)
{
  ...
  {
    if (item_ptr->x >= x && item_ptr->x < x + dtt_paste_size_x && item_ptr->y >= y &&
        item_ptr->y < y + dtt_paste_size_y)
    {
...
#include \"%s.rh\"\n",
  ...
  /* Write the root menu */
  sts = dtt_edit_save_one_menu(fout, fout_c, fout_h, fout_db1, fout_db2, fout_decl, dtt_root_ctx, 0,
                               generate_only, nocompile);
                                  ...
                                  FILE* fout_decl, menu_ctx ctx, int index, int generate_only, int nocompile)
{
  ...
  while (menu_ptr->text[0] != 0)
  {
    if (!generate_only)
    {
      ...
      if ((menu_ptr->type == DTT_MENUITEM_COMMAND) || (menu_ptr->type == DTT_MENUITEM_COMMANDHOLD) ||
          (menu_ptr->type == DTT_MENUITEM_SYSPICTURE) || (menu_ptr->type == DTT_MENUITEM_PIFUNCPERM) ||
          (menu_ptr->type == DTT_MENUITEM_PIFUNC))
      {
        fprintf(fout, "%s\n", (char*)menu_ptr->arg1);
      }
    ...
    if (ODD(dtt_get_stored_menuctx(&childctx, menu_ptr->index)))
    {
      if (!generate_only)
        fprintf(fout, "down\n");
      /* Write this menu */
      sts = dtt_edit_save_one_menu(fout, fout_c, fout_h, fout_db1, fout_db2, fout_decl, childctx,
                                   menu_ptr->index, generate_only, nocompile);
      if (EVEN(sts))
        return sts;
      if (!generate_only)
        fprintf(fout, "up\n");
  ...
  while (menu_ptr->text[0] != 0)
  {
    ...
    else if (menu_ptr->type == DTT_MENUITEM_EDIT || menu_ptr->type == DTT_MENUITEM_PIPERM)
    {
    ...
    else if (menu_ptr->type == DTT_MENUITEM_PIFUNC || menu_ptr->type == DTT_MENUITEM_PIFUNCPERM)
    {
      dtt_get_picturename(menu_ptr->index, item_menu_name);
...
static int dtt_edit_read_one_menu(FILE* fin, menu_ctx parent_ctx, int parent_item)
{
  ...
  while (1)
  {
    ...
    if ((type == DTT_MENUITEM_COMMAND) || (type == DTT_MENUITEM_COMMANDHOLD) ||
        (type == DTT_MENUITEM_SYSPICTURE) || (type == DTT_MENUITEM_PIFUNCPERM) ||
    ...
    if (!first)
    {
      ...
      /* Create a new menu */
      sts = rtt_menu_list_add(&menulist, i, 0, text, 0, 0, 0, pwr_cNObjid, 0, 0, 0, 0);
      if (EVEN(sts))
        return sts;
      ...
      else if ((type == DTT_MENUITEM_COMMAND) || (type == DTT_MENUITEM_COMMANDHOLD) ||
      ...
      else if (type == DTT_MENUITEM_OBJPICTURE || type == DTT_MENUITEM_FILESPEC)
      {
        /* Put a pointer to the command in arg1 */
        command_ptr = calloc(80, sizeof(char));
        strcpy(command_ptr, command);
      ...
      else if (type == DTT_MENUITEM_PIFUNC || type == DTT_MENUITEM_PIFUNCPERM)
      {
      ...
      /* Create a context */
      sts = rtt_menu_create_ctx(&ctx, parent_ctx, menulist, title, RTT_MENUTYPE_MENU | RTT_MENUTYPE_DYN);
      if (EVEN(sts))
        return sts;
    ...
    else
    {
      ...
      sts = rtt_menu_list_add(&(ctx->menu), i, 0, text, 0, 0, 0, pwr_cNObjid, 0, 0, 0, 0);
      if (EVEN(sts))
        return sts;
...
/* Figure out the current opsys */
  ...
  if (EVEN(sts))
  {
    ...
    sts = rtt_menu_list_add(&menulist, 0, 0, first_item, 0, 0, 0, pwr_cNObjid, 0, 0, 0, 0);
    if (EVEN(sts))
      return sts;
  ...
  {
    sts = dtt_menu_new(0, &dtt_root_ctx->menu, dtt_root_ctx->title, 1, RTT_MENUTYPE_DYN);
    if (EVEN(sts))
      return sts;
  }
  return RTT__SUCCESS;
...
{
  char cmd[400];
  sprintf(cmd, "%s/wb_rtt_appl.sh %s %d %d %d", dtt_exe_dir, dtt_programname, action, dtt_is_rttsys, opsys);
  system(cmd);
  return RTT__SUCCESS;
}
...
static int dtt_compile_picture(char* filename, int opsys)
{
  ...
  if (!opsys)
  {
    ...
    rtt_cursor_abs(0, 21);
    r_print_buffer();
    sprintf(cmd, "%swb_rtt_comppicture.sh %s %s %d %d", dtt_exe_dir, filename, dtt_programname, dtt_is_rttsys,
            dtt_current_opsys);
  ...
  else
  {
    for (os = 1; os < pwr_mOpSys_; os = os << 1)
    {
      if (opsys & os)
      {
        ...
        rtt_cursor_abs(0, 21);
        r_print_buffer();
        sprintf(cmd, "%swb_rtt_comppicture.sh %s %s %d %d", dtt_exe_dir, filename, dtt_programname,
                dtt_is_rttsys, os);
...
static int dtt_menu_draw_item(menu_ctx ctx, int item)
{
  ...
  if ((item < ctx->current_page * ctx->page_len) || (item > (ctx->current_page + 1) * ctx->page_len) ||
      (item > ctx->no_items))
    return 0;
                                   ...
                                   unsigned long arg4, unsigned long arg5)
{
  ...
  sts = dtt_edit_process_one_menu(func, arg1, arg2, arg3, arg4, arg5, dtt_root_ctx, 0);
  if (EVEN(sts))
    return sts;
                                     ...
                                     int index)
{
  ...
  while (menu_ptr->text[0] != 0)
  {
    ...
    {
      /* Process this menu */
      sts = dtt_edit_process_one_menu(func, arg1, arg2, arg3, arg4, arg5, childctx, menu_ptr->index);
      if (EVEN(sts))
        return sts;
    }
    menu_idx++;
...
static int dtt_edit_write_gdhrefs(char* filename)
{
  ...
  sts = dtt_edit_process_menues(&dtt_edit_write_menu_gdhrefs, (unsigned long)outfile, 0, 0, 0, 0);
                                       ...
                                       int dum3, int dum4)
{
  ...
  if ((menu_ptr->type == DTT_MENUITEM_EDIT) || (menu_ptr->type == DTT_MENUITEM_PIPERM) ||
      (menu_ptr->type == DTT_MENUITEM_PIFUNCPERM) || (menu_ptr->type == DTT_MENUITEM_PIFUNC))
  {
    /* Get this picture */
...
static int dtt_edit_pwrplc_gdhrefs(char* filename)
{
  ...
  fprintf(outfile, "!\n");
  fprintf(outfile, "create object/dest=\"\"/class=$planthier/name=\"RttDum\"\n");
  fprintf(outfile, "create object/dest=rttdum/class=$planthier/name=\"Rtt\"\n");

  sts = dtt_edit_process_menues(&dtt_edit_pwrplc_menu_gdhrefs, (unsigned long)outfile,
                                (unsigned long)&externref_index, 0, 0, 0);
      ...
      while (item_ptr->number != 0)
      {
        if (!str_NoCaseStrcmp(item_ptr->database, "GDH"))
        {
          if (!gdh_item_found)
          {
            ...
            fprintf(outfile, "create object/dest=rttdum-rtt/class=$planthier/name=\"%s\"\n", objname);
            gdh_item_found = 1;
          }
          ...
          if (!(!str_NoCaseStrcmp(item_ptr->priv, "NO") || !strcmp(item_ptr->text, "%")) &&
...
static int dtt_edit_list_items_all(char* filename)
{
  ...
  sts = dtt_edit_process_menues(&dtt_edit_list_items_picture, (unsigned long)outfile, 0, 0, 0, 0);
  rtt_fgetname(outfile, outfilename, filename);
  fclose(outfile);
  if (EVEN(sts))
    return sts;
  ...
  for (i = 0; i < 2; i++)
  {
    ...
    while (ODD(sts = dtt_read_line(line, sizeof(line), infile)))
    {
      ...
      if (EVEN(sts))
        goto error;
      row++;
      sts = rtt_parse(line, " 	", "", (char*)value_str, sizeof(value_str) / sizeof(value_str[0]),
      ...
      if (strlen(text_dualpar) > 0 && text_dualpar[strlen(text_dualpar) - 1] == '"')
...
static int dtt_edit_save_all(int generate_only, int nocompile, int opsys)
{
  ...
  /* Save all pictures */
  sts = dtt_edit_process_menues(&dtt_edit_save_picture, 0, 0, generate_only, opsys, nocompile);
  if (EVEN(sts))
    return sts;
...
static int dtt_edit_save_by_index(int index, int generate_only, int nocompile, int opsys)
{
  ...
  /* Save all pictures */
  sts = dtt_edit_process_menues(&dtt_edit_save_picture, 0, index, generate_only, opsys, nocompile);
  if (EVEN(sts))
    return sts;
                                    ...
                                    int dum4)
{
      ...
      (menu_ptr->type == DTT_MENUITEM_PIFUNCPERM) || (menu_ptr->type == DTT_MENUITEM_PIFUNC))
  {
    ...
    sts = dtt_edit_write(picture_ctx, picturefilename, menufilename, 0, nocompile, generate_only, 0, 0,
                         &messages);
    if (EVEN(sts))
      return sts;
                                 ...
                                 int opsys, int nocompile)
{
      ...
      (menu_ptr->type == DTT_MENUITEM_PIFUNCPERM) || (menu_ptr->type == DTT_MENUITEM_PIFUNC))
  {
    ...
    dtt_get_menufilename(menufilename);
    sts = dtt_edit_write(picture_ctx, picturefilename, menufilename, 0, nocompile, generate_only, opsys, 0,
                         &messages);
    if (EVEN(sts))
      return sts;
  ...
  */
  ...
  if (all)
  {
    ...
    while (menu_ptr->text[0] != 0)
    {
      ...
      if (menu_ptr->type == DTT_MENUITEM_OBJPICTURE || menu_ptr->type == DTT_MENUITEM_FILESPEC)
      {
        fprintf(fout, "%s\n", (char*)menu_ptr->arg1);
        fprintf(fout, "%s\n", (char*)menu_ptr->arg2);
      }
...
static int dtt_edit_include_menue(menu_ctx ctx, char* filename)
{
  ...
  while (1)
  {
    ...
    {
      /* Add an item to the menulist */
      sts = rtt_menu_list_insert(&(ctx->menu), i, text, 0, 0, 0, pwr_cNObjid, 0, 0, 0, 0);
      if (EVEN(sts))
        return sts;
...
static int dtt_function_menues_exist()
{
  ...
  function_found = 0;
  sts = dtt_edit_process_menues(&dtt_check_if_function_menu, (unsigned long)&function_found, 0, 0, 0, 0);
                                      ...
                                      int dum3, int dum4)
{
  ...
  /* Which type of menue is this */
  if ((menu_ptr->type == DTT_MENUITEM_PIFUNCPERM) || (menu_ptr->type == DTT_MENUITEM_PIFUNC))
  {
    *function_found = 1;
  }
  return RTT__SUCCESS;
...
static int dtt_show_menu(menu_ctx ctx, char* menu_name)
{
  ...
  /* Parse the menu_name */
  nr = rtt_parse(menu_name, "-", "", (char*)name_array, sizeof(name_array) / sizeof(name_array[0]),
  ...
  if (menu_ptr->func)
  {
    sts = (menu_ptr->func)(menuctx, menu_ptr->arg1, menu_ptr->arg2, menu_ptr->arg3, menu_ptr->arg4);
    return sts;
  }
  else
  {
                                 ...
                                 unsigned long option, int timeout)
{
  ...
  {
    sts = rtt_get_input(chn, out_string, out_terminator, out_maxlen, option, timeout);
  }
  ...
  if (item_ptr != 0)
  {
    while (item_ptr->number != 0)
    {
      ...
      if (item_ptr->number < 1)
        dtt_edit_print_syntaxerror(item_ptr, "Number out of range", 1, &errors, &warnings);

      /* Check text */
      if (streq(item_ptr->text, ""))
        dtt_edit_print_syntaxerror(item_ptr, "Text i missing", 1, &errors, &warnings);
      if (streq(item_ptr->text, "%"))
        strcpy(item_ptr->priv, "NO");

      /* Check Parameter */
      if (streq(item_ptr->parameter, "") && str_NoCaseStrcmp(item_ptr->type, "COMMAND") != 0)
        dtt_edit_print_syntaxerror(item_ptr, "Parameter i missing", 1, &errors, &warnings);
      ...
      {
        if (!streq(item_ptr->text, "%"))
          dtt_edit_print_syntaxerror(item_ptr, "Text should be % in alarmtext entry", 1, &errors, &warnings);
        strcpy(item_ptr->priv, "NO");
        strcpy(item_ptr->type, "UPDATE");
      ...
      {
        if (!streq(item_ptr->text, "%"))
          dtt_edit_print_syntaxerror(item_ptr, "Text should be % in time entry", 1, &errors, &warnings);
        strcpy(item_ptr->priv, "NO");
        strcpy(item_ptr->database, "RTT");
        strcpy(item_ptr->declaration, "STRING");
        item_ptr->characters = 8;
      ...
      /* Check Parameter */
      if (str_NoCaseStrcmp(item_ptr->outflags, "TEXT") == 0 && streq(item_ptr->dualparameter, ""))
        dtt_edit_print_syntaxerror(item_ptr, "Dualparameter i missing", 1, &errors, &warnings);

      if (str_NoCaseStrcmp(item_ptr->outflags, "FLASHTEXT") == 0 && streq(item_ptr->dualparameter, ""))
        dtt_edit_print_syntaxerror(item_ptr, "Dualparameter i missing", 1, &errors, &warnings);
      ...
      while (item_p->number != 0)
      {
        ...
        if (item_ptr->y == item_p->y && item_ptr->x <= item_p->x && item_p->x < item_ptr->x + size)
          dtt_edit_print_syntaxerror(item_ptr, "Field overlap", 0, &errors, &warnings);
        item_p++;
      }
                                      ...
                                      int* warning_count)
{
  ...
  printf("	in entry (%2d,%2d) \"%s U%d\"\n", item_ptr->x, item_ptr->y, item_ptr->text, item_ptr->number);
...
static int dtt_setup(menu_ctx parent_ctx)
{
  ...
  for (i = 0; i < (int)elements; i++)
  {
    ...
    sts = rtt_menu_upd_list_add(&menulist, i, elements, text, 0, 0, 0, pwr_cNObjid, 0, 0, 0, 0, text, priv,
                                parameter_ptr, type, 0, size, pwr_cNDlid, 0, 0, 0, 0, maxlimit, minlimit,
                                RTT_DATABASE_USER, 0);
    if (EVEN(sts))
      return sts;
  }

  sts = rtt_menu_upd_new(parent_ctx, pwr_cNObjid, &menulist, title, 0, RTT_MENUTYPE_DYN);
  if (sts == RTT__FASTBACK)
    return sts;
