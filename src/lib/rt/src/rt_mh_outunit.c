...
)
{
  ...
  LstForEach(hl, &l.handler_l) if (LstEntry(hl, sHandler, handler_l)->nix == targetId->Nix) break;
...
pwr_tStatus mh_OutunitBlock(pwr_tObjid object, /**< The identity of an object to be blocked. */
...
pwr_tStatus
mh_OutunitConnect(pwr_tObjid outunit,          /**< Object identity of an outunit object.*/
                  mh_eOutunitType type,        /**< Type of outunit. */
                  ...
                  mh_cbOutunitReturn cbReturn, /**< Address of a function to receive alarm
                                                  return messages. */
                  mh_cbOutunitAlarmStatus cbAlarmStatus /**< Address of a function to receive alarm status
                                                           messages. */
)
{
  pwr_tStatus sts;
...
pwr_tStatus mh_OutunitUpdate()
{
  ...
  LstForEach(hl, &l.handler_l) sendInfo(LstEntry(hl, sHandler, handler_l));
...
pwr_tStatus mh_OutunitAlarmRequest(mh_sOutunitAlarmReq* msg)
{
  ...
  LstForEach(hl, &l.handler_l) if (LstEntry(hl, sHandler, handler_l)->nix == msg->Nix) break;
...
static void ackListInsert(sHandler* hp, pwr_tUInt32 targetIdx)
{
  ...
  LstForEach(al, &hp->ack_l) if (LstEntry(al, sAck, ack_l)->ack.targetIdx == targetIdx) return;
...
static void handlerEvent(sHandler* hp, mh_sHead* p)
{
  ...
  case mh_eEvent_Ack:
    ...
    LstForEach(al, &hp->ack_l) if (LstEntry(al, sAck, ack_l)->ack.targetIdx == ap->TargetId.Idx)
    {
  ...
  case mh_eEvent_Cancel:
    ...
    LstForEach(al, &hp->ack_l) if (LstEntry(al, sAck, ack_l)->ack.targetIdx == cp->TargetId.Idx)
    {
      ackListDelete(hp, LstEntry(al, sAck, ack_l));
      break;
    }
...
static pwr_tBoolean isValidHandler(mh_sHead* p, qcom_sAid* aid, sHandler** h)
{
  ...
  if (!(p->ver == mh_cVersion || (mh_cVersion == 5 && (p->ver == 3 || p->ver == 4 || p->ver == 6))))
  ...
  LstForEach(hl, &l.handler_l) if (LstEntry(hl, sHandler, handler_l)->qid.qix == p->qid.qix &&
                                   LstEntry(hl, sHandler, handler_l)->qid.nid == p->qid.nid) break;
...
static void linkDown(qcom_sNode* nodep)
{
  ...
  LstForEach(hl, &l.handler_l) if (LstEntry(hl, sHandler, handler_l)->qid.nid == nodep->nid)
  {
    hp = LstEntry(hl, sHandler, handler_l);
    hp->linkUp = FALSE;
...
static pwr_tStatus sendToHandler(sHandler* hp, mh_eMsg type, unsigned int size, void* bp)
{
  pwr_tStatus sts;
  char* mp;
