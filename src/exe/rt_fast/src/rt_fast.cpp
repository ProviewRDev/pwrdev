...
void fastobject::open(double base_scantime)
{
  ...
  if (cdh_ObjidIsNotNull(p->TriggObject.Objid))
  {
    ...
    if (EVEN(sts))
    {
      if (p->Function & fast_mFunction_ManTrigg || p->Function & fast_mFunction_LevelTrigg)
        trigg = 0;
      else
        throw co_error(sts);
    }
  ...
  for (int i = 0; i < FAST_CURVES; i++)
  {
    if (cdh_ObjidIsNotNull(p->Attribute[i].Objid))
    {
      ...
      sts = gdh_GetAttributeCharacteristics(name, &p->AttributeType[i], &attributes_size[i], &offs, &elem);
      if (EVEN(sts))
        throw co_error(sts);
...
void fastobject::scan()
{
  ...
  if (p->Function & fast_mFunction_BeforeTrigg && p->Function & fast_mFunction_AlwaysPrepared && !p->New &&
      !p->Prepare)
  {
    p->Prepare = 1;
    p->Active = 1;
  ...
  if (!p->Active ||
      (p->Function & fast_mFunction_BeforeTrigg && p->Prepare && p->TriggIndex == fast_cNoTrigg))
  {
    // Check trigg conditions

    if (p->Function & fast_mFunction_BeforeTrigg && (p->Prepare && !old_prepare))
    {
    ...
    if ((p->Function & fast_mFunction_ManTrigg && p->TriggMan) || (trigg && *trigg && !old_trigg))
    {
    ...
    if (p->Function & fast_mFunction_LevelTrigg && p->CurveValid[0] &&
        p->AttributeType[0] == pwr_eType_Float32 && *(pwr_tFloat32*)attributes[0] >= p->TriggLevel &&
  ...
  if (p->Function & fast_mFunction_LevelTrigg && p->CurveValid[0] && p->AttributeType[0] == pwr_eType_Float32)
