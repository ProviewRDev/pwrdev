...
static int check_os(char* str, char* os)
{
  ...
  nr = dcli_parse(str, "||", "", (char*)os_vect, sizeof(os_vect) / sizeof(os_vect[0]), sizeof(os_vect[0]), 0);
...
static int generate_module_include()
{
  typedef struct
  {
    char class[40];
  ...
  static sModule files[] = {{"pwr_systemclasses.h", "", ""},
                            ...
                            {"pwr_ssaboxclasses.h", "ssabox.h", ""},
                            {"pwr_miscellaneousclasses.h", "misc.h", ""},
                            {"pwr_profibusclasses.h", "", ""},
                            {"pwr_sevclasses.h", "", ""},
                            {"pwr_simulclasses.h", "simul.h", ""},
                            {"pwr_otherioclasses.h", "", ""},
                            {"pwr_othermanufacturerclasses.h", "", ""},
                            {"pwr_abbclasses.h", "", ""},
                            {"pwr_siemensclasses.h", "", ""},
                            {"pwr_inorclasses.h", "", ""},
                            {"pwr_klocknermoellerclasses.h", "", ""},
  ...
  strcpy(fname, "$pwr_einc/pwr_modules.h");
  dcli_translate_filename(fname, fname);
  fp = fopen(fname, "w");
  if (!fp)
  {
    printf("** co_merge: Unable to open file %s\n", fname);
    return 1;
  }
  fprintf(fp, "/*\n *  File pwr_modules.h\n *\n *  Generated by co_merge. Do not edit this file\n */\n\n");
  for (i = 0; i < sizeof(files) / sizeof(files[0]); i++)
  {

    sprintf(fname, "$pwr_einc/%s", files[i].class);
    dcli_translate_filename(fname, fname);
    if (stat(fname, &info) != -1)
    {
      fprintf(fp, "#include \"%s\"\n", files[i].class);
      if (strcmp(files[i].lib, "") != 0)
        fprintf(fp, "#include \"%s\"\n", files[i].lib);
      if (strcmp(files[i].macro, "") != 0)
        fprintf(fp, "#include \"%s\"\n", files[i].macro);
    }
  }
  fclose(fp);
...
int main(int argc, char* argv[])
{
  ...
  {
  case merge_eMtype_WbBase:
    snprintf(cmd, sizeof(cmd), "%s -c %s %s -I%s -D%s -o %s %s", pwre_cxx, getenvsafe("cross_compile"),
             cflags, incdir, os, ofile, cfile);
    break;
  default:
    snprintf(cmd, sizeof(cmd), "%s -c %s %s -I%s -D%s -o %s %s", pwre_cc, getenvsafe("cross_compile"), cflags,
             incdir, os, ofile, cfile);
  }
  if (arg_verbose)
    printf("co_merge: %s\n", cmd);
  system(cmd);
