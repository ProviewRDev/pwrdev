...
int CnvWblToH::init(char* first)
{
  ...
  fp_struct << "/*  File " << struct_filename << ".\n"
            << " *\n"
...
int CnvWblToH::class_exec()
{
  ...
  get_filename(ctx->rw, struct_filename, ctx->hpp);
  if (ctx->hpp)
    sprintf(fname, "%s%s_c_%s.hpp", ctx->dir, volume_name_low, CnvCtx::low(ctx->rw->class_name));
  else
    sprintf(fname, "%s%s_c_%s.h", ctx->dir, volume_name_low, CnvCtx::low(ctx->rw->class_name));
...
int CnvWblToH::body_exec()
{
  ...
  if (streq(CnvCtx::low(ctx->rw->body_name), "rtbody") && ctx->rw->class_devonly)
  {
...
int CnvWblToH::class_close()
{
  if (!struct_cclass_written)
  {
    fp_struct << '\n'
              ...
              << "#define pwr_cClass_" << ctx->rw->class_name << " " << ctx->rw->class_id << "UL\n"
              << '\n';
...
int CnvWblToH::attribute_exec()
{
  ...
  if (attr_next_alignlw || ctx->rw->attr_pointer || ctx->rw->attr_isclass ||
      streq(CnvCtx::low(ctx->rw->attr_type), "input") || streq(CnvCtx::low(ctx->rw->attr_type), "buffer") ||
      ...
      streq(CnvCtx::low(ctx->rw->attr_typeref), "dataref") || streq(ctx->rw->attr_name, "TimerFlag"))
    strcpy(alignstr, " pwr_dAlignLW");
  else
    strcpy(alignstr, " pwr_dAlignW");

  if (ctx->rw->attr_isclass || streq(CnvCtx::low(ctx->rw->attr_type), "buffer") || ctx->rw->attr_pointer ||
      streq(CnvCtx::low(ctx->rw->attr_typeref), "castid") ||
      streq(CnvCtx::low(ctx->rw->attr_typeref), "disableattr"))
  ...
  if (str_StartsWith(CnvCtx::low(ctx->rw->attr_typeref), "pwr_etype_"))
    ...
    strcpy(ctx->rw->attr_typeref, &ctx->rw->attr_typeref[strlen("pwr_etypedef_")]);
  ...
  if (streq(CnvCtx::low(ctx->rw->attr_type), "input"))
  {
    if (ctx->rw->attr_array && ctx->rw->attr_pointer)
    {
      ...
      fp_struct << " **" << pgmname << "P[" << ctx->rw->attr_elements << "]" << alignstr << ";\n";
      if (!ctx->common_structfile_only)
        fp_cstruc << " **" << pgmname << "P[" << ctx->rw->attr_elements << "]" << alignstr << ";\n";
    }
  ...
  if (ctx->rw->attr_array && ctx->rw->attr_pointer)
  {
    ...
    fp_struct << " " << pgmname << "[" << ctx->rw->attr_elements << "]" << alignstr << ";\n";
    if (!ctx->common_structfile_only)
...
int CnvWblToH::typedef_exec()
{
  if (streq(ctx->rw->typedef_typeref, "Mask") || streq(ctx->rw->typedef_typeref, "Enum"))
  {
    ...
    fp_struct << "/*_* " << ctx->rw->typedef_typeref << ": " << ctx->rw->typedef_name << '\n'
              << "    @Aref " << ctx->rw->typedef_name << " " << ctx->rw->typedef_name << '\n'
              << "*/\n\n"
              << "typedef pwr_t" << ctx->rw->typedef_typeref << " pwr_t" << ctx->rw->typedef_name << ";\n\n"
              << "typedef enum {\n";
  }
  else if (streq(ctx->rw->typedef_typeref, "String"))
  {
    if (ctx->rw->typedef_elements > 1)
      fp_struct << "typedef char pwr_t" << ctx->rw->typedef_name << "[" << ctx->rw->typedef_elements
                << "];\n\n";
    else
  ...
  {
    if (ctx->rw->typedef_elements > 1)
      fp_struct << "typedef pwr_t" << ctx->rw->typedef_typeref << " pwr_t" << ctx->rw->typedef_name << "["
...
int CnvWblToH::check_typename(char* type_volume, char* type_name)
{
  ...
  {
    char* name;
    char valid_names[][40] = {"pwr_tAddress",
                              "pwr_tBit",
                              "pwr_tBitMask",
                              "pwr_tBoolean",
                              "pwr_tFloat32",
    ...
    for (name = valid_names[0]; !streq(name, ""); name += sizeof(valid_names[0]))
    {
