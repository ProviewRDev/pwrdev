...
int CnvWblToHtml::init(char* first)
{
  ...
  // Create index file
  {
    ...
    fp << "<FRAME src=\"" << html_first << ".html\" name=\"classFrame\">\n"
       ...
       << "This document is designed to be viewed using the frames feature. If "
          ...
          "client.\n"
       << "<BR>\n"
       << "Link to <A HREF=\"" << html_first << ".html\">Non-frame version.</A></NOFRAMES>\n"
       << "</HTML>\n";
  }
  ...
  if (ctx->setup->group_cnt)
  {
    ...
    for (int i = 0; i < ctx->setup->group_cnt; i++)
    {
      ...
      fp << "<A HREF=\"" << fname << "\" TARGET=\"_parent\">" << ctx->setup->groups[i] << "</A>\n"
         << "<BR>\n";
    }
  ...
  fp_html_index << "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.0 "
                   "Frameset//EN\"\"http://www.w3.org/TR/REC-html40/frameset.dtd\">\n"
                << "<!-- Generated by co_convert " << timestr << "  -->\n"
                << "<HTML>\n"
                << "<HEAD>\n"
  ...
  for (int i = 0; i < ctx->setup->group_cnt; i++)
  {
    ...
    strcat(fname, "_index.html");
    {
      ...
      if (streq(ctx->setup->groups_startpage[i], ""))
        ...
        fp << "<FRAME src=\"" << ctx->setup->groups_startpage[i] << "\" name=\"classFrame\">\n";
    ...
    fp_html_group[i] << "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.0 "
                        "Frameset//EN\"\"http://www.w3.org/TR/REC-html40/frameset.dtd\">\n"
                     << "<!-- Generated by co_convert " << timestr << "  -->\n"
                     << "<HTML>\n"
                     << "<HEAD>\n"
    ...
    fp_js_group[i] << "function " << ctx->rw->volume_name << "_" << ctx->setup->groups[i] << "(parent)\n"
                   << "{\n"
                   << "parent.addChildren([\n";
  }
  ...
  // Create js map for volume
  {
    ...
    fp << "function " << ctx->rw->volume_name << "(parent)\n"
       << "{\n"
       << "  aux = insFld(parent, gFld(\"AllClasses\",\"" << ctx->rw->volume_name << "_index.html\"))\n"
    ...
    for (int i = 0; i < ctx->setup->group_cnt; i++)
    {
      ...
      fp << "  aux = insFld(parent, gFld(\"" << ctx->setup->groups[i] << "\",\"" << fname << "\"))\n"
         << "  " << ctx->rw->volume_name << "_" << ctx->setup->groups[i] << "(aux)\n";
    }
    fp << "}\n";
  }
...
int CnvWblToHtml::class_exec()
{
  ...
  // Add into index file
  fp_html_index << "<A HREF=\"" << html_file_name << ".html\" TARGET=\"classFrame\">" << ctx->rw->class_name
                << "</A>\n"
                << "<BR>\n";
  ...
  for (int i = 0; i < ctx->rw->doc_group_cnt; i++)
  {
    ...
    {
      if (str_NoCaseStrcmp(ctx->rw->doc_groups[i], ctx->setup->groups[j]) == 0)
      {
        fp_html_group[j] << "<A HREF=\"" << html_file_name << ".html\" TARGET=\"classFrame\">"
                         << ctx->rw->class_name << "</A>\n"
                         << "<BR>\n";
        ...
        fp_js_group[j] << "[\"" << ctx->rw->class_name << "\",\"" << html_file_name << ".html\"]\n";
      }
    }
  }
  ...
  fp_html_clf << "<!DOCTYPE HTML PUBLIC \"-//W3C//DTD HTML 4.0 "
                 "Frameset//EN\"\"http://www.w3.org/TR/REC-html40/frameset.dtd\">\n"
              << "<!-- Generated by co_convert " << timestr << "  -->\n"
              << "<HTML>\n"
              << "<HEAD>\n"
              ...
              << "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;C Binding: \n"
              << "&nbsp;<A HREF=\"" << struct_file << "#" << ctx->rw->class_name << "\">Struct</A>\n"
              << "&nbsp;|&nbsp<A HREF=\"" << hpp_file << "#" << ctx->rw->class_name << "\">Class</A>\n";
  ...
  if (ctx->rw->doc_fresh && !streq(ctx->rw->doc_code, ""))
  {
    ...
    else
    {
      ...
      fp_html_clf << "&nbsp;|&nbsp;<A HREF=\"" << ref_name << "#" << low_class_name << "\">Code</A>\n";
    }
  }
  fp_html_clf << "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp<A HREF=\"" << ctree_file << "\">ClassTree</A>\n";
  ...
  if (ctx->rw->doc_fresh && !streq(ctx->rw->doc_code, ""))
  {
    ...
    else
    {
      fp_html_clf << "<tr id=\"dormw\">"
                  << "<td id=\"dormw\">" << Lng::translate("Code") << "</td>\n"
                  << "<td id=\"dormw2\"><a href=\"" << ref_name << "#" << low_class_name << "\">"
                  << ctx->rw->doc_code << "</a><td>\n"
                  << "</tr>\n";
    }
  }
  fp_html_clf << "</table>\n";
  ...
  if (ctx->rw->doc_fresh)
  {
    for (i = 0; i < ctx->rw->doc_cnt; i++)
    {
      ...
      {
        fp_html_clf << "</PRE><B><FONT SIZE=\"3\">" << txt + 2 << "</FONT></B><BR><PRE>\n";
      }
  ...
  if (ctx->rw->doc_xlink_cnt || ctx->rw->doc_clink_cnt || ctx->rw->doc_link_cnt)
  {
    fp_html_clf << "<ul>\n";
    for (i = 0; i < ctx->rw->doc_xlink_cnt; i++)
    {
      fp_html_clf << "<li><a id=\"desc\" href=\"" << ctx->rw->doc_xlink_ref[i] << "\" target=\"_self\">"
                  << ctx->rw->doc_xlink_text[i] << "</a></li>\n";
    }
    for (i = 0; i < ctx->rw->doc_clink_cnt; i++)
    {
      fp_html_clf << "<li><a id=\"desc\" href=\"" << ctx->rw->doc_clink_ref[i] << "\" target=\"_self\">"
                  << ctx->rw->doc_clink_text[i] << "</a></li>\n";
    }
    for (i = 0; i < ctx->rw->doc_link_cnt; i++)
    {
      fp_html_clf << "<li><a id=\"desc\" href=\"" << ctx->rw->doc_link_ref[i] << "\" target=\"_self\">"
...
int CnvWblToHtml::body_exec()
{
  ...
  fp_html_clf << "<!-- =========== BODY =========== -->\n"
              ...
              << "<a href=\"" << struct_file << "#" << ctx->rw->class_name << "\">" << struct_name
              << "</a></th>\n"
              << "</tr>\n";
...
void CnvWblToHtml::create_cdp_file(const char* volume_name, const char* class_name, const char* attr_typeref)
{
  pwr_tFileName fname;
...
int CnvWblToHtml::attribute_exec()
{
  ...
  if (!streq(ctx->rw->attr_typeref_volume, ""))
  {
    ...
    sprintf(typeref_href, "%s_%s.html", low_volname, CnvCtx::low(ctx->rw->attr_typeref));
  }
  else
  ...
  fp_html_clf << "<td id=\"dorm\"><a href=\"#" << ctx->rw->attr_name << "\">" << ctx->rw->attr_name
              << "</a></td>\n";
  ...
  if (ctx->rw->doc_fresh)
  {
    if (streq(ctx->rw->doc_summary, ""))
    {
      for (i = 0; i < ctx->rw->doc_cnt; i++)
      {
        ...
        {
          fp_html_clf << "</pre><b><font size=\"3\">" << txt + 2 << "</font></b><pre><br>\n";
        }
        else if (str_StartsWith(CnvCtx::low(txt), "@h1"))
        {
  ...
  fp_tmp << "<hr>\n"
         << "<table id=\"dormw\">\n"
         << "<tr id=\"dormw\">\n"
         << "<th id=\"dormw\" colspan=\"2\"><a name=\"" << ctx->rw->attr_name << "\"> " << ctx->rw->attr_name
         << "&nbsp;" << ctx->rw->attr_type << "</a></th>\n"
         << "</tr>\n";

  fp_tmp << "<tr id=\"dormw\">\n"
         << "<td id=\"dormw\">" << Lng::translate("Class") << "</td>\n"
         << "<td id=\"dormw2\"><a href=\"" << attrtype_href << "\">" << ctx->rw->attr_type << "</a></td>\n"
         << "</tr>\n";
  ...
  if (ctx->rw->attr_array && ctx->rw->attr_pointer)
    fp_tmp << "<td id=\"dormw2\">Array of pointers to <a href=\"" << typeref_href << "\">"
           << ctx->rw->attr_typeref << "</a><td>\n";
  else if (ctx->rw->attr_array)
    fp_tmp << "<td id=\"dormw2\">Array of <a href=\"" << typeref_href << "\">" << ctx->rw->attr_typeref
           << "</a><td>\n";
  else if (ctx->rw->attr_pointer)
    fp_tmp << "<td id=\"dormw2\">Pointer to <a href=\"" << typeref_href << "\">" << ctx->rw->attr_typeref
           << "</a><td>\n";
  else
    fp_tmp << "<td id=\"dormw2\"><a href=\"" << typeref_href << "\">" << ctx->rw->attr_typeref
           << "</a><td>\n";
  fp_tmp << "</tr>\n";

  fp_tmp << "<tr id=\"dormw\">\n"
         << "<td id=\"dormw\"><a href=\"" << attrtype_href << "#Flags\">Flags</a></td>\n"
  ...
  if (ctx->rw->doc_fresh)
  {
    for (i = 0; i < ctx->rw->doc_cnt; i++)
    {
      ...
      {
        fp_tmp << "</PRE><B><FONT SIZE=\"3\">" << txt + 2 << "</FONT></B><BR><PRE>\n";
      }
      else if (str_StartsWith(CnvCtx::low(txt), "@h1"))
      {
...
int CnvWblToHtml::bit_exec()
{
  ...
  fp_html_clf << "<tr id=\"dorm\">\n"
              ...
              << "<td id=\"dorm\"><a href=\"#" << ctx->rw->bit_name << "\">" << ctx->rw->bit_text
              << "</a></td>\n"
              << "<td id=\"dorm2\">";
  if (ctx->rw->doc_fresh)
  {
  ...
  fp_tmp << "<hr>\n"
         ...
         << "<a name=\"" << ctx->rw->bit_name << "\">" << ctx->rw->bit_text << "</a></th>\n"
         << "<tr id=\"dormw\">\n"
         ...
         << "<td id=\"dormw2\">pwr_" << bitchar << ctx->rw->typedef_name << "_" << ctx->rw->bit_pgmname
         << "</td>\n"
         << "</tr>\n"
         << "</table>\n";
...
int CnvWblToHtml::typedef_exec()
{
  ...
  if ((streq(ctx->rw->typedef_typeref, "Mask") || streq(ctx->rw->typedef_typeref, "Enum")) &&
      !streq(low_volume_name, "pwrs"))
  {
    sprintf(code_aref, "%s#%s", struct_file, ctx->rw->typedef_name);
    fp_html_clf << " C Binding: &nbsp;<A HREF=\"" << code_aref << "\">Typedef</A>\n";
  }
  ...
  fp_html_clf << "</FONT></TD>\n"
              ...
              << "<table id=\"dormw\">\n"
              << "<tr id=\"dormw\">\n"
              << "<th id=\"dormh\" colspan=\"2\">Type " << ctx->rw->class_name
              << "<img align=\"right\" src=\"../../pwr_logoorm.png\"></th>\n"
              << "</tr>\n";
  if (ctx->rw->doc_fresh && !streq(ctx->rw->doc_author, ""))
  {
    fp_html_clf << "<tr id=\"dormw\">\n"
  ...
  if (ctx->rw->doc_link_cnt || ctx->rw->doc_clink_cnt)
  {
    ...
    {
      fp_html_clf << "<li><a id=\"desc\" href=\"" << ctx->rw->doc_link_ref[i] << "\" target=\"_self\">"
                  << ctx->rw->doc_link_text[i] << "</li>\n";
    }
    for (i = 0; i < ctx->rw->doc_clink_cnt; i++)
    {
      fp_html_clf << "<li><a id=\"desc\" href=\"" << ctx->rw->doc_clink_ref[i] << "\" target=\"_self\">"
                  << ctx->rw->doc_clink_text[i] << "</li>\n";
    }
    fp_html_clf << "</ul>\n";
  }
  fp_html_clf << "</div>\n";
  ...
  if (streq(ctx->rw->typedef_typeref, "Mask") || streq(ctx->rw->typedef_typeref, "Enum"))
  {
    ...
    fp_html_clf << "<hr><br>\n"
                ...
                << " elements&nbsp;&nbsp; <a href=\"" << code_aref << "\">pwr_" << bitchar
                << ctx->rw->typedef_name << "</a></th>\n"
                << "</tr>\n";
