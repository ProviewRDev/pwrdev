...
static PyTypeObject OidType = {
    PyVarObject_HEAD_INIT(NULL, 0) "pwrrt.Oid", /* tp_name */
...
typedef struct
{
  PyObject_HEAD pwr_tAttrRef aref;
  pwr_tTypeId type;
  unsigned int size;
...
static PyTypeObject ArefType = {
    PyVarObject_HEAD_INIT(NULL, 0) "pwrrt.Aref", /* tp_name */
...
static PyTypeObject CidType = {
    PyVarObject_HEAD_INIT(NULL, 0) "pwrrt.Cid", /* tp_name */
...
static PyMethodDef Tid_methods[] = {{"name", (PyCFunction)Tid_name, METH_NOARGS, tid_name_doc},
                                    {"fullName", (PyCFunction)Tid_fullName, METH_NOARGS, tid_fullName_doc},
                                    {NULL}};

static PyTypeObject TidType = {
    PyVarObject_HEAD_INIT(NULL, 0) "pwrrt.Tid", /* tp_name */
...
typedef struct
{
  PyObject_HEAD pwr_tObjName name;
  pwr_tCid cid;
  pwr_tUInt32 offset;
  pwr_tUInt32 size;
  pwr_tUInt32 flags;
...
static PyMethodDef ADef_methods[] = {{"name", (PyCFunction)ADef_name, METH_NOARGS, adef_name_doc},
...
static PyTypeObject ADefType = {
    PyVarObject_HEAD_INIT(NULL, 0) "pwrrt.ADef", /* tp_name */
...
static PyMethodDef Sub_methods[] = {{"value", (PyCFunction)Sub_value, METH_NOARGS, sub_value_doc},
...
static PyTypeObject SubType = {
    PyVarObject_HEAD_INIT(NULL, 0) "pwrrt.Sub", /* tp_name */
...
static PyTypeObject ApplType = {
    PyVarObject_HEAD_INIT(NULL, 0) "pwrrt.Appl", /* tp_name */
...
static int is_authorized(unsigned int access) { return (pwrrt_priv & access) != 0; }
...
static PyObject* Oid_richcompare(PyObject* self, PyObject* other, int op)
{
  ...
  if (Py_TYPE(other) != &OidType)
  {
    if (Py_TYPE(other) == &PyBool_Type)
    {
      ...
      if ((ODD(((OidObject*)self)->sts) && oval) || (EVEN(((OidObject*)self)->sts) && !oval))
        result = Py_True;
      else
        result = Py_False;
    }
...
static PyObject* Oid_oidStr(PyObject* s, PyObject* args)
{
  ...
  if (name == 0)
    sprintf(str, "_O%hhu.%hhu.%hhu.%hhu:%u", 0xFF & (oid.vid >> 24), 0xFF & (oid.vid >> 16),
            0xFF & (oid.vid >> 8), 0xFF & oid.vid, oid.oix);
  else if (strcmp(name, "hex") == 0)
    sprintf(str, "_O%hhu.%hhu.%hhu.%hhu:%x", 0xFF & (oid.vid >> 24), 0xFF & (oid.vid >> 16),
            0xFF & (oid.vid >> 8), 0xFF & oid.vid, oid.oix);
  else if (strcmp(name, "file") == 0)
    sprintf(str, "%03hhu_%03hhu_%03hhu_%hhu_%08x", 0xFF & (oid.vid >> 24), 0xFF & (oid.vid >> 16),
            0xFF & (oid.vid >> 8), 0xFF & oid.vid, oid.oix);
  else
...
static PyObject* Oid_children(PyObject* s)
{
  ...
  for (sts = gdh_GetChild(self->oid, &coid); ODD(sts); sts = gdh_GetNextSibling(coid, &coid))
    cnt++;
...
static PyObject* Cid_objects(PyObject* s)
{
  ...
  for (sts = gdh_GetClassList(self->cid, &oid); ODD(sts); sts = gdh_GetNextObject(oid, &oid))
    cnt++;
...
static PyObject* Cid_attrObjects(PyObject* s)
{
  ...
  for (sts = gdh_GetClassListAttrRef(self->cid, &aref); ODD(sts);
...
static int Tid_init(TidObject* self, PyObject* args, PyObject* kwds) { return 0; }
...
static int ADef_init(ADefObject* self, PyObject* args, PyObject* kwds) { return 0; }
...
static PyObject* ADef_cid(ADefObject* self, PyObject* args) { return Py_BuildValue("i", self->cid); }

static PyObject* ADef_offset(ADefObject* self, PyObject* args) { return Py_BuildValue("i", self->offset); }

static PyObject* ADef_size(ADefObject* self, PyObject* args) { return Py_BuildValue("i", self->size); }

static PyObject* ADef_flags(ADefObject* self, PyObject* args) { return Py_BuildValue("i", self->flags); }
...
static PyObject* ADef_typeref(ADefObject* self, PyObject* args) { return Py_BuildValue("i", self->typeref); }
...
static PyObject* Sub_str(PyObject* s)
{
  ...
  return PyUnicode_FromFormat("%d.%d.%d.%d:%u", 0xFF & (self->refid.nid >> 24),
                              0xFF & (self->refid.nid >> 16), 0xFF & (self->refid.nid >> 8),
                              0xFF & self->refid.nid, self->refid.rix);
}
...
static PyObject* Sub_value(SubObject* self, PyObject* args)
{
  ...
  case pwr_eType_Text:
  {
    ...
    else
    {
      ...
      {
        char* utfstr =
            cnv_iso8859_to_utf8((char*)self->p + elemsize * i, strlen((char*)self->p + elemsize * i) + 1);
...
static PyObject* pwrrt_login(PyObject* self, PyObject* args)
{
  ...
  sts = gdh_GetObjectInfo("pwrNode-System.SystemGroup", (pwr_tAddress)&systemgroup, sizeof(systemgroup));
  if (EVEN(sts))
    return set_error(sts);
...
static PyObject* pwrrt_getPriv(PyObject* self, PyObject* args) { return Py_BuildValue("I", pwrrt_priv); }
...
static PyObject* pwrrt_getSevItemList(PyObject* self, PyObject* args)
{
  ...
  for (i = 0; i < itemcnt; i++)
  {
    if (!filter || str_NoCaseStrncmp(filter, lp->oname, strlen(filter)) == 0)
    {
      ...
      PyTuple_SetItem(
          ituple, 2, PyUnicode_FromString(cnv_iso8859_to_utf8(lp->description, sizeof(lp->description) + 1)));
      ...
      for (j = 0; j < lp->attrnum; j++)
        PyTuple_SetItem(
            ituple, j + 6,
            PyUnicode_FromString(cnv_iso8859_to_utf8(lp->attr[j].aname, sizeof(lp->attr[j]) + 1)));
...
static PyObject* pwrrt_getSevEventsItemList(PyObject* self, PyObject* args)
{
  ...
  for (i = 0; i < itemcnt; i++)
  {
    if (!filter || str_NoCaseStrncmp(filter, itemlist[i].oname, strlen(filter)) == 0)
    {
      ...
      PyTuple_SetItem(
          ituple, 0,
          PyUnicode_FromString(cnv_iso8859_to_utf8(itemlist[i].oname, strlen(itemlist[i].oname) + 1)));
      PyTuple_SetItem(ituple, 1, PyUnicode_FromString(cdh_ObjidToString(itemlist[i].oid, 1)));
      PyTuple_SetItem(ituple, 2,
                      PyUnicode_FromString(
                          cnv_iso8859_to_utf8(itemlist[i].description, strlen(itemlist[i].description) + 1)));
...
static PyObject* pwrrt_getSevItemData(PyObject* self, PyObject* args)
{
  ...
  if (!PyArg_ParseTuple(args, "sssss|Is", &server, &oidstr, &aname, &fromstr, &tostr, &maxrows, &time_format))
    return NULL;
  ...
  sevcli_get_itemdata(&sts, pwrrt_scctx, oid, aname, from, to, maxrows, &tbuf, &vbuf, &rows, &vtype, &vsize);
  ...
  for (i = 0; i < rows; i++)
  {
    ...
    else
    {
      ...
      localtime_r(&sec, &ts);
      date = PyDateTime_FromDateAndTime(ts.tm_year + 1900, ts.tm_mon + 1, ts.tm_mday, ts.tm_hour, ts.tm_min,
                                        ts.tm_sec, (int)tbuf[i].tv_nsec / 1000);
...
static PyObject* pwrrt_getSevItemsDataFrame(PyObject* self, PyObject* args)
{
  ...
  if (!PyArg_ParseTuple(args, "sOOOssf|IsI", &server, &oidobj, &anameobj, &isobjectobj, &fromstr, &tostr,
                        &tdiff, &maxrows, &time_format, &options))
    return NULL;
  ...
  if (oidcnt <= 0 || anamecnt <= 0 || isobjectcnt <= 0 || oidcnt != anamecnt || oidcnt != isobjectcnt)
    return set_error(GDH__ARGCOUNT);
  ...
  if (options & 1)
  {
    ...
    sevcli_get_itemdata(&sts, pwrrt_scctx, oid, aname, from, to, maxrows, &ttbuf, (void**)&vbuf, &rows,
                        &vtcond, &vsize);
    if (sts == SEV__NOPOINTS)
      Py_RETURN_NONE;
    else if (EVEN(sts))
  ...
  for (i = 0; i < oidcnt; i++)
  {
        ...
        for (j = 0; j < valcnt; j++)
        {
          ...
          else
          {
            while (tbuf[k] <= j * tdiff && k < rows - 1)
              k++;
            vvect[i][j] =
                ((pwr_tFloat32*)vbuf)[k - 1] + (((pwr_tFloat32*)vbuf)[k] - ((pwr_tFloat32*)vbuf)[k - 1]) /
                                                   (tbuf[k] - tbuf[k - 1]) * (j * tdiff - tbuf[k - 1]);
          }
        }
        break;
      case pwr_eType_Int32:
        ...
        for (j = 0; j < valcnt; j++)
        {
          ...
          else
          {
            while (tbuf[k] <= j * tdiff && k < rows - 1)
              k++;
            vvect[i][j] =
                (float)((pwr_tInt32*)vbuf)[k - 1] + (((pwr_tInt32*)vbuf)[k] - ((pwr_tInt32*)vbuf)[k - 1]) /
                                                        (tbuf[k] - tbuf[k - 1]) * (j * tdiff - tbuf[k - 1]);
          }
        }
        break;
    ...
    else
    {
      ...
      sevcli_get_objectitemdata(&sts, pwrrt_scctx, oid, "", from, to, maxrows, &ttbuf, (void**)&vbuf, &rows,
                                &attrbuf, &attrnum);
      if (sts == SEV__NOPOINTS)
        Py_RETURN_NONE;
      else if (EVEN(sts))
          ...
          if (streq(anamevect[l], attrbuf[m].aname))
          {
            ...
            for (j = 0; j < valcnt; j++)
            {
              ...
              else
              {
                ...
                case pwr_eType_Float32:
                  vvect[l][j] =
                      (float)(*(pwr_tFloat32*)vp1 + (*(pwr_tFloat32*)vp2 - *(pwr_tFloat32*)vp1) /
                                                        (tbuf[k] - tbuf[k - 1]) * (j * tdiff - tbuf[k - 1]));
                  break;
                case pwr_eType_Int32:
                  vvect[l][j] =
                      (float)(*(pwr_tInt32*)vp1 + (*(pwr_tInt32*)vp2 - *(pwr_tInt32*)vp1) /
                                                      (tbuf[k] - tbuf[k - 1]) * (j * tdiff - tbuf[k - 1]));
                  break;
                case pwr_eType_UInt32:
                case pwr_eType_Boolean:
                  vvect[l][j] =
                      (float)(*(pwr_tUInt32*)vp1 + (*(pwr_tUInt32*)vp2 - *(pwr_tUInt32*)vp1) /
                                                       (tbuf[k] - tbuf[k - 1]) * (j * tdiff - tbuf[k - 1]));
                  break;
                default:
                  vvect[l][j] = 0;
...
static PyObject* pwrrt_getSevItemsDataFrameD(PyObject* self, PyObject* args)
{
  ...
  if (!PyArg_ParseTuple(args, "sOOOssf|Is", &server, &oidobj, &anameobj, &isobjectobj, &fromstr, &tostr,
                        &tdiff, &maxrows, &time_format))
    return NULL;
  ...
  for (i = 0; i < oidcnt; i++)
  {
    ...
    if (!isobjectvect[i])
    {
      sevcli_get_itemdata(&sts, pwrrt_scctx, oid, anamevect[i], from, to, maxrows, &ttbuf, (void**)&vbuf,
                          &rows, &vtype, &vsize);
      if (sts == SEV__NOPOINTS)
        Py_RETURN_NONE;
      else if (EVEN(sts))
        ...
        else
        {
          ...
          else
          {
            ...
            if (fabs(t0buf[j] - tbuf[k - 1]) < FLT_EPSILON)
              ...
              vvect[i][j] =
                  vbuf[k - 1] + (vbuf[k] - vbuf[k - 1]) / (tbuf[k] - tbuf[k - 1]) * (t0buf[j] - tbuf[k - 1]);
          }
        }
      }
      free(vbuf);
...
static PyObject* pwrrt_getSevEvents(PyObject* self, PyObject* args)
{
  ...
  if (!PyArg_ParseTuple(args, "ssssIIss|Is", &server, &oidstr, &fromstr, &tostr, &eventtypemask,
                        &eventpriomask, &utf8eventtext, &utf8eventname, &maxrows, &time_format))
    return NULL;

  strncpy(eventtext, cnv_utf8_to_iso8859(utf8eventtext, strlen(utf8eventtext) + 1), sizeof(eventtext));
  strncpy(eventname, cnv_utf8_to_iso8859(utf8eventname, strlen(utf8eventname) + 1), sizeof(eventname));
  ...
  sevcli_get_events(&sts, pwrrt_scctx, oid, from, to, eventtypemask, eventpriomask, eventtext, eventname,
                    maxrows, &list, &listcnt);
  if (sts == SEV__NOPOINTS)
    Py_RETURN_NONE;
  else if (EVEN(sts))
    return set_error(sts);
  ...
  for (i = 0; i < listcnt; i++)
  {
    ...
    else
    {
      ...
      localtime_r(&sec, &ts);
      date = PyDateTime_FromDateAndTime(ts.tm_year + 1900, ts.tm_mon + 1, ts.tm_mday, ts.tm_hour, ts.tm_min,
                                        ts.tm_sec, (int)list[i].Time.tv_nsec / 1000);
    ...
    PyTuple_SetItem(
        item_tuple, 3,
        PyUnicode_FromString(cnv_iso8859_to_utf8(list[i].EventText, strlen(list[i].EventText) + 1)));
    PyTuple_SetItem(
        item_tuple, 4,
        PyUnicode_FromString(cnv_iso8859_to_utf8(list[i].EventName, strlen(list[i].EventName) + 1)));
...
static PyObject* pwrrt_getSevEventsDataFrame(PyObject* self, PyObject* args)
{
  ...
  if (!PyArg_ParseTuple(args, "ssssIIss|Is", &server, &oidstr, &fromstr, &tostr, &eventtypemask,
                        &eventpriomask, &eventtext, &eventname, &maxrows, &time_format))
    return NULL;
...
static struct PyModuleDef moduledef = {PyModuleDef_HEAD_INIT, "pwrrt", pwrrt_doc, -1, PwrrtMethods};

PyMODINIT_FUNC PyInit_pwrrt(void)
{
  PyObject* m;

  if (PyType_Ready(&OidType) < 0 || PyType_Ready(&ArefType) < 0 || PyType_Ready(&CidType) < 0 ||
      PyType_Ready(&TidType) < 0 || PyType_Ready(&ADefType) < 0 || PyType_Ready(&SubType) < 0 ||
