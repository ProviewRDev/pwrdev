...
void WAttGtk::change_value(int set_focus)
{
  ...
  sts = wattnav->check_attr(&multiline, &input_node, input_name, &value, &input_size);
  if (EVEN(sts))
  {
...
{
  CoXHelp::dhelp("objecteditor_refman", 0, navh_eHelpFile_Other, "$pwr_lang/man_dg.dat", true);
}
...
void WAttGtk::activate_cmd_input(GtkWidget* w, gpointer data)
{
  ...
  {
    if (text)
      sts = ((WAttNav*)watt->wattnav)->set_attr_value(watt->input_node, watt->input_name, text);
...
void WAttGtk::activate_cmd_scrolled_ok(GtkWidget* w, gpointer data)
{
  ...
  if (watt->input_open)
  {
    ...
    textutf8 = gtk_text_buffer_get_text(watt->cmd_scrolled_buffer, &start_iter, &end_iter, FALSE);
    text = g_convert(textutf8, -1, "ISO8859-1", "UTF-8", NULL, NULL, NULL);
    g_free(textutf8);
...
void WAttGtk::pop() { gtk_window_present(GTK_WINDOW(toplevel)); }

void WAttGtk::update_title() { CoWowGtk::update_title(toplevel, editmode); }
...
WAttGtk::WAttGtk(GtkWidget* wa_parent_wid, void* wa_parent_ctx, ldh_tSesContext wa_ldhses,
                 pwr_sAttrRef wa_aref, int wa_editmode, int wa_advanced_user, int wa_display_objectname)
    : WAtt(wa_parent_ctx, wa_ldhses, wa_aref, wa_editmode, wa_advanced_user, wa_display_objectname),
      parent_wid(wa_parent_wid)
{
  ...
  toplevel = (GtkWidget*)g_object_new(GTK_TYPE_WINDOW, "default-height", default_height, "default-width", 410,
                                      "title", CoWowGtk::convert_utf8(namep), NULL);
  ...
  wattnav = new WAttNavGtk((void*)this, wattnav_eType_Object, pane, "Plant", ldhses, aref, wa_editmode,
  ...
  {
    ((Wtt*)parent_ctx)->register_utility((void*)this, wb_eUtility_AttributeEditor);
  }
