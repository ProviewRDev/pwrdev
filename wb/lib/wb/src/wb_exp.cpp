...
static pwr_tStatus exp_vldh_load_window(ldh_tWBContext ldhwb, ldh_tSesContext ldhses, pwr_tObjid window,
                                        vldh_t_wind* loaded_wind, ldh_eAccess access);
static pwr_tStatus exp_node_gen(exp_genctx genctx, vldh_t_node node);
static pwr_tStatus exp_ObjdidToName(vldh_t_node node, pwr_tObjid output_objdid, char* output_name, int size);
...
static pwr_tStatus gen_print_inputs(exp_genctx genctx, vldh_t_node node, unsigned long printtype,
                                    unsigned long* nocondef, unsigned long* nocontype);
...
static pwr_tStatus gen_filename(ldh_tSesContext ldhses, pwr_tObjid objid, char* filename);
...
pwr_tStatus exp_plcpgm(ldh_tSesContext ldhses, char* plcpgm_str, char* output)
{
  ...
  sts = ldh_ObjidToName(ldhses, plcpgm, ldh_eName_Hierarchy, hiername, sizeof(hiername), &size);
  if (EVEN(sts))
    goto error;
  ...
  if (EVEN(sts))
    goto error;
  sts = ldh_ObjidToName(ldhses, parent, ldh_eName_Hierarchy, parent_name, sizeof(parent_name), &size);
  if (EVEN(sts))
    goto error;
  sts = ldh_ObjidToName(ldhses, plcpgm, ldh_eName_Object, plcpgm_name, sizeof(plcpgm_name), &size);
  if (EVEN(sts))
    goto error;
  ...
  while (ODD(sts))
  {
    if (ODD(ldh_GetObjectBuffer(ldhses, window, "DevBody", "PlcWindow", (pwr_eClass*)&cid, &windbuffer,
                                &size)))
    {
      free(windbuffer);
      ...
      if (EVEN(sts))
        goto error;
      sts = ldh_ObjidToName(ldhses, cdh_ClassIdToObjid(cid), ldh_eName_Object, class_name, sizeof(class_name),
                            &size);
      if (EVEN(sts))
        goto error;
      sts = ldh_ObjidToName(ldhses, window, ldh_eName_Hierarchy, hiername, sizeof(hiername), &size);
      if (EVEN(sts))
        goto error;
  ...
  /* Print reset object */
  sts = ldh_GetObjectPar(ldhses, plcpgm, "DevBody", "ResetObject", (char**)&resobjdid_ptr, &size);
  if (EVEN(sts))
    goto error;

  sts = ldh_ObjidToName(ldhses, *resobjdid_ptr, ldh_eName_Hierarchy, hiername, sizeof(hiername), &size);
  if (ODD(sts))
    fprintf(file, " /ResetObject=%s.ActualValue", hiername);
  free((char*)resobjdid_ptr);

  /* Print reset object */
  sts = ldh_GetObjectPar(ldhses, plcpgm, "DevBody", "ScanTime", (char**)&scantime_ptr, &size);
  if (EVEN(sts))
    goto error;
...
pwr_tStatus exp_window(ldh_tWBContext ldhwb, ldh_tSesContext ldhses, char* window_str, char* output)
{
  ...
  sts = exp_vldh_load_window(ldhwb, ldhses, window, &wind, ldh_eAccess_ReadOnly);
  if (EVEN(sts))
    return sts;

  sts = gcg_plcwindow_compile(wind, 0, &syntax_errcount, &syntax_warncount, 0, 0);
  if (EVEN(sts))
    return sts;
  ...
  sts = ldh_ObjidToName(genctx.ldhses, wind->lw.oid, ldh_eName_Hierarchy, hiername, sizeof(hiername), &size);
  if (EVEN(sts))
    return sts;
  ...
  if (EVEN(sts))
    return sts;
  sts = ldh_ObjidToName(ldhses, window, ldh_eName_Object, hiername, sizeof(hiername), &size);
  if (EVEN(sts))
    return sts;
  sts = ldh_GetObjectClass(ldhses, window, &cid);
  if (EVEN(sts))
  ...
  if (EVEN(sts))
    return sts;
  sts = ldh_ObjidToName(ldhses, ((vldh_t_plc)(wind->hw.plc))->lp.oid, ldh_eName_Hierarchy, plcpgm_name,
                        sizeof(plcpgm_name), &size);
  if (EVEN(sts))
    return sts;

  fprintf(genctx.file, "object %s /name=%s /parent=%s /plcpgm=%s\n", class_name, hiername, parent_name,
          plcpgm_name);
...
static pwr_tStatus exp_node_gen(exp_genctx genctx, vldh_t_node node)
{
  ...
  sts = ldh_GetClassBody(genctx->ldhses, node->ln.cid, "GraphPlcNode", &bodyclass, (char**)&graphbody, &size);
  if (EVEN(sts))
    return sts;

  compmethod = graphbody->compmethod;

  if (node->ln.cid == pwr_cClass_and || node->ln.cid == pwr_cClass_or || node->ln.cid == pwr_cClass_or ||
      node->ln.cid == pwr_cClass_csub)
  {
    sts = gen_m2(genctx, node);
  }
  else if (node->ln.cid == pwr_cClass_stodo || node->ln.cid == pwr_cClass_setdo ||
           node->ln.cid == pwr_cClass_resdo || node->ln.cid == pwr_cClass_stodv ||
           node->ln.cid == pwr_cClass_setdv || node->ln.cid == pwr_cClass_resdv ||
           node->ln.cid == pwr_cClass_stoav || node->ln.cid == pwr_cClass_cstoav ||
           node->ln.cid == pwr_cClass_stoao || node->ln.cid == pwr_cClass_cstoao ||
           node->ln.cid == pwr_cClass_stoai || node->ln.cid == pwr_cClass_cstoai)
  {
    sts = gen_m3(genctx, node);
  }
  else if (node->ln.cid == pwr_cClass_stodp || node->ln.cid == pwr_cClass_setdp ||
           node->ln.cid == pwr_cClass_resdp || node->ln.cid == pwr_cClass_stoap ||
           node->ln.cid == pwr_cClass_cstoap || node->ln.cid == pwr_cClass_StoIp ||
           node->ln.cid == pwr_cClass_CStoIp || node->ln.cid == pwr_cClass_stopi)
  {
  ...
  else if (node->ln.cid == pwr_cClass_initstep || node->ln.cid == pwr_cClass_substep ||
  ...
  else if (node->ln.cid == pwr_cClass_aarithm || node->ln.cid == pwr_cClass_darithm ||
           node->ln.cid == pwr_cClass_filter || node->ln.cid == pwr_cClass_maxmin ||
           node->ln.cid == pwr_cClass_ramp || node->ln.cid == pwr_cClass_inc3p ||
           node->ln.cid == pwr_cClass_pos3p || node->ln.cid == pwr_cClass_drive ||
           node->ln.cid == pwr_cClass_mvalve || node->ln.cid == pwr_cClass_Backup ||
           node->ln.cid == pwr_cClass_ASup || node->ln.cid == pwr_cClass_DSup ||
           node->ln.cid == pwr_cClass_Backup)
  {
    sts = gen_m10(genctx, node);
  }
  else if (node->ln.cid == pwr_cClass_ssend || node->ln.cid == pwr_cClass_ssbegin)
  {
...
static pwr_tStatus gen_m2(exp_genctx genctx, vldh_t_node node)
{
  ...
  sts = ldh_ObjidToName(genctx->ldhses, cdh_ClassIdToObjid(node->ln.cid), ldh_eName_Object, class_name,
...
static pwr_tStatus gen_m3(exp_genctx genctx, vldh_t_node node)
{
  ...
  sts = ldh_GetObjectPar(genctx->ldhses, node->ln.oid, "RtBody", "In", &nocondef_ptr, &size);
  if (EVEN(sts))
    return sts;
  ...
  /* Print the connected inputs */
  sts = gen_print_inputs(genctx, node, GCG_PRINT_ALLPAR, (unsigned long*)&nocondef, nocontype);
  if (EVEN(sts))
    return sts;

  /* Get the first attribute in devbody of type pwr_tObjid */
  sts = ldh_GetObjectBodyDef(genctx->ldhses, node->ln.cid, "DevBody", 1, &bodydef, &rows);
  if (EVEN(sts))
    return sts;
  ...
  for (i = 0; i < rows; i++)
  {
    if (bodydef[i].Par->Param.Info.Type == pwr_eType_Objid)
    {
      ...
      sts = ldh_GetObjectPar(genctx->ldhses, node->ln.oid, "DevBody", (bodydef[i].ParName), (char**)&parvalue,
                             &size);
      if (EVEN(sts))
        return sts;
      found = 1;
...
static pwr_tStatus gen_m4(exp_genctx genctx, vldh_t_node node)
{
  ...
  sts = ldh_GetObjectPar(genctx->ldhses, node->ln.oid, "DevBody", "Parameter", &parameter, &size);
  if (EVEN(sts))
    return sts;

  fprintf(genctx->file, "/%s=%s.%s\n", (bodydef[i].ParName), object_name, parameter);
  free((char*)parameter);
  free((char*)bodydef);
...
static pwr_tStatus gen_m5(exp_genctx genctx, vldh_t_node node)
{
  ...
  /* Get the Do signal */
  sts = ldh_GetObjectPar(genctx->ldhses, node->ln.oid, "RtBody", "TimerTime", (char**)&time_ptr, &size);
  if (EVEN(sts))
    return sts;
...
static pwr_tStatus gen_m6(exp_genctx genctx, vldh_t_node node)
{
  ...
  sts = ldh_GetObjectPar(ldhses, plc->lp.oid, "DevBody", "ResetObject", (char**)&resobjdid_ptr, &size);
  if (EVEN(sts))
    return sts;
...
static pwr_tStatus gen_m7(exp_genctx genctx, vldh_t_node node)
{
  ...
  if (point_count > 1)
  {
    ...
    for (k = 1; k < (int)point_count; k++)
    {
          ...
          ((cid == pwr_cClass_ssbegin) && (next_point == 1)))
      {
        ...
        sts = exp_ObjdidToName(node, next_node->ln.oid, hiername, sizeof(hiername));
        if (EVEN(sts))
          return sts;
  ...
  if (ODD(sts))
  {
    ...
    if (output_count > 0)
    {
      ...
      sts = gcg_get_outputstring(&gcgctx_dum, output_node, &output_bodydef, &output_attrref, &output_type,
      ...
      sts = exp_ObjdidToName(node, output_attrref.Objid, hiername, sizeof(hiername));
      if (EVEN(sts))
        return sts;
...
static pwr_tStatus gen_m8(exp_genctx genctx, vldh_t_node node)
{
  ...
  if (ODD(sts))
  {
    ...
    if (output_count > 0)
    {
      ...
      sts = gcg_get_outputstring(&gcgctx_dum, output_node, &output_bodydef, &cond_output_attrref,
                                 &cond_output_type, &output_prefix, cond_output_par);
      if (sts == GSX__NEXTPAR)
        return sts;
      if (EVEN(sts))
        return sts;
  ...
  while (ODD(sts))
  {
    ...
    {
      /* Print class and name */
      sts = ldh_ObjidToName(genctx->ldhses, cdh_ClassIdToObjid(cid), ldh_eName_Object, class_name,
                            sizeof(class_name), &size);
      if (EVEN(sts))
        return sts;

      /* Print class and name */
      sts = ldh_ObjidToName(genctx->ldhses, next_objdid, ldh_eName_Object, hiername, sizeof(hiername), &size);
      if (EVEN(sts))
        return sts;

      fprintf(genctx->file, "object %s /name=%s /order=%s", class_name, hiername, node->hn.name);
      ...
      {
        /* Print the time */
        sts = ldh_GetObjectPar(ldhses, next_objdid, "RtBody", "TimerTime", (char**)&timevalue, &size);
        if (EVEN(sts))
          return sts;
        time = *timevalue;
        free((char*)timevalue);
      ...
      else if (cid == pwr_cClass_corder)
      {
        if (pincond_found)
        {
          sts = exp_ObjdidToName(node, cond_output_attrref.Objid, hiername, sizeof(hiername));
          if (EVEN(sts))
            return sts;
...
static pwr_tStatus gen_m11(exp_genctx genctx, vldh_t_node node)
{
  ...
  sts = ldh_ObjidToName(ldhses, parent, ldh_eName_Hierarchy, hiername, sizeof(hiername), &size);
  if (EVEN(sts))
    return sts;
  ...
  while ((bodydef[i].ParClass == pwr_eClass_Input) && (i < rows))
  {
    ...
    if (ODD(sts))
    {
      ...
      if (output_count > 0)
      {
        ...
        sts = exp_ObjdidToName(node, output_attrref.Objid, output_name, sizeof(output_name));
        if (EVEN(sts))
          return sts;
    ...
    if (!output_found)
    {
      ...
      {
        /* Get the parameter value */
        sts = ldh_GetObjectPar(ldhses, node->ln.oid, "RtBody", bodydef[i].ParName, &buffer, &size);
        if (EVEN(sts))
          return sts;
        ...
        {
        case pwr_eType_Float32:
          if (!(*(pwr_tFloat32*)(buffer) < FLT_EPSILON && *(pwr_tFloat32*)(buffer) > -FLT_EPSILON))
          {
            fprintf(genctx->file, "/%s=%g", bodydef[i].ParName, *(pwr_tFloat32*)(buffer));
          }
          break;
        case pwr_eType_Boolean:
          if (*(pwr_tBoolean*)(buffer))
          {
            fprintf(genctx->file, "/%s=%d", bodydef[i].ParName, *(pwr_tBoolean*)(buffer));
          }
          break;
        default:;
        }
...
static pwr_tStatus exp_ObjdidToName(vldh_t_node node, pwr_tObjid output_objdid, char* output_name, int size)
{
  ...
  wind = node->hn.wind;
  sts = ldh_ObjidToName(wind->hw.ldhses, output_objdid, ldh_eName_Hierarchy, object_name, size, &outsize);
  if (EVEN(sts))
    return sts;
...
static pwr_tStatus gen_print_interns(exp_genctx genctx, vldh_t_node node)
{
  ...
  for (i = 0; i < 2; i++)
  {
    ...
    for (j = 0; j < rows; j++)
    {
      ...
      sts = ldh_GetObjectPar(ldhses, node->ln.oid, body, parname, &object_par, &parsize);
      if (EVEN(sts))
        return sts;

      if (!(bodydef[j].ParClass == pwr_eClass_Intern || bodydef[j].ParClass == pwr_eClass_Param ||
      ...
      sts = ldh_GetObjectPar(ldhses, templ, body, parname, &template_par, &size);
      if (EVEN(sts))
        return sts;
      ...
      for (k = 0; k < 1; k++)
      {
        ...
        if (memcmp(template_element, object_element, parsize / elements) != 0)
        {
          ...
          case pwr_eType_Boolean:
          {
            fprintf(genctx->file, "/%s=%d", bodydef[j].ParName, *(pwr_tBoolean*)(object_element));
            break;
          }
          case pwr_eType_Float32:
          {
            fprintf(genctx->file, "/%s=%g", bodydef[j].ParName, *(pwr_tFloat32*)(object_element));
            break;
          }
          case pwr_eType_Float64:
          {
            fprintf(genctx->file, "/%s=%g", bodydef[j].ParName, *(pwr_tFloat64*)(object_element));
            break;
          }
          case pwr_eType_Char:
          {
            fprintf(genctx->file, "/%s=%d", bodydef[j].ParName, *(pwr_tChar*)(object_element));
            break;
          }
          case pwr_eType_Int8:
          {
            fprintf(genctx->file, "/%s=%d", bodydef[j].ParName, *(pwr_tInt8*)(object_element));
            break;
          }
          case pwr_eType_Int16:
          {
            fprintf(genctx->file, "/%s=%d", bodydef[j].ParName, *(pwr_tInt16*)(object_element));
            break;
          }
          case pwr_eType_Int32:
          {
            fprintf(genctx->file, "/%s=%d", bodydef[j].ParName, *(pwr_tInt32*)(object_element));
            break;
          }
          case pwr_eType_UInt8:
          {
            fprintf(genctx->file, "/%s=%d", bodydef[j].ParName, *(pwr_tUInt8*)(object_element));
            break;
          }
          case pwr_eType_UInt16:
          {
            fprintf(genctx->file, "/%s=%d", bodydef[j].ParName, *(pwr_tUInt16*)(object_element));
            break;
          }
          case pwr_eType_UInt32:
          {
            fprintf(genctx->file, "/%s=%d", bodydef[j].ParName, *(pwr_tUInt32*)(object_element));
            break;
          }
          case pwr_eType_String:
          {
            fprintf(genctx->file, "/%s=\"%s\"", bodydef[j].ParName, object_element);
            break;
          }
          case pwr_eType_Objid:
          {
            sts = exp_ObjdidToName(node, *(pwr_tObjid*)(object_element), hiername, sizeof(hiername));
            if (ODD(sts))
          ...
          {
            sts = ldh_AttrRefToName(ldhses, (pwr_sAttrRef*)object_element, ldh_eName_Aref, &hiername_ptr,
                                    &size);
            if (ODD(sts))
          ...
          case pwr_eType_Time:
          {
            ...
            time_AtoAscii((pwr_tTime*)object_element, time_eFormat_DateAndTime, timbuf, sizeof(timbuf));
            timbuf[20] = '\0';
...
static pwr_tStatus gen_filename(ldh_tSesContext ldhses, pwr_tObjid objid, char* filename)
{
  ...
  sts = ldh_ObjidToName(ldhses, objid, ldh_eName_Hierarchy, hiername, sizeof(hiername), &size);
  if (EVEN(sts))
    return sts;
  ...
  if (strlen(hiername) >= MAX_FILENAME_LEN)
    memmove(hiername, &hiername[strlen(hiername) - MAX_FILENAME_LEN + 1], MAX_FILENAME_LEN);
