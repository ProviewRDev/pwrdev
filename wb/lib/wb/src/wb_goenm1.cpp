...
int goen_create_nodetype_m1(pwr_sGraphPlcNode* graphbody, pwr_tClassId cid, ldh_tSesContext ldhses,
                            ...
                            unsigned long node_width, flow_tNodeClass* node_class, vldh_t_node node)
{
  ...
  f_height = f_header / 2 + f_header / 2 + MAX((MAX(inputpoints, outputpoints) - 1), 1) * f_repeat;

  classname_width = strlen(graphbody->graphname) * f_strlength;
  if (annot_count <= 1)
  {
    f_width = MAX(f_strlength * 2 + MAX(classname_width, annot_width[0]), f_defwidth + f_strlength * 2);
    f_width_left = 0;
  }
  else
  {
    f_width = MAX(f_strlength * 4 + MAX(classname_width, annot_width[0]) + annot_width[1],
  ...
  if (subwindowmark != 0)
  {
    flow_AddLine(nc_pid, 0, f_header - f_yoffs, f_width, f_header - f_yoffs, flow_eDrawType_LineGray, 4);
  }
  ...
  /* Draw the class name */
  flow_AddText(nc_pid, graphbody->graphname, f_namepos, f_header * .75 - f_yoffs, flow_eDrawType_TextRoboto,
               GOEN_F_TEXTSIZE);
  ...
  for (i = 0; i < i_innr; i++)
  {
    if ((*inmask_pointer & pointmask) != 0)
    {
      ...
      {
        flow_AddLine(nc_pid, 0, f_header / 2 + f_repeat * ipoints - f_yoffs, -f_pinlength,
                     f_header / 2 + f_repeat * ipoints - f_yoffs, flow_eDrawType_Line, 2);
      }
      else
      {
        flow_AddLine(nc_pid, -f_pinlength, f_header / 2 + f_repeat * ipoints - f_yoffs, -f_circle,
                     f_header / 2 + f_repeat * ipoints - f_yoffs, flow_eDrawType_Line, 2);
        flow_AddArc(nc_pid, -f_circle, f_header / 2 + f_repeat * ipoints - f_circle / 2 - f_yoffs, 0,
                    f_header / 2 + f_repeat * ipoints + f_circle / 2 - f_yoffs, 0, 360, flow_eDrawType_Line,
                    2);
      }
      flow_AddConPoint(nc_pid, -f_pinlength, f_header / 2 + f_repeat * ipoints - f_yoffs, conpoint_nr++,
  ...
  for (i = i_innr + i_internnr; i < i_innr + i_internnr + i_outnr; i++)
  {
    ...
    {
      flow_AddLine(nc_pid, f_width, f_header / 2 + f_repeat * ipoints - f_yoffs, f_width + f_pinlength,
                   f_header / 2 + f_repeat * ipoints - f_yoffs, flow_eDrawType_Line, 2);
      flow_CreateConPoint(ctx, f_width + f_pinlength, f_header / 2 + f_repeat * ipoints - f_yoffs,
  ...
  flow_AddAnnot(nc_pid, (f_width - f_width_left - annot_width[0]) / 2, f_height - f_header * 0.25 - f_yoffs,
                0, flow_eDrawType_TextRoboto, GOEN_F_TEXTSIZE, flow_eAnnotType_OneLine, flow_mDisplayLevel_1);

  if (annot_count >= 2)
  {
    flow_AddAnnot(nc_pid, f_width - f_width_left + f_strlength, f_height - f_header * 0.75 - f_yoffs, 1,
 ...
 *
 **************************************************************************/
int goen_get_point_info_m1(WGre* grectx, pwr_sGraphPlcNode* graphbody, unsigned long point,
                           ...
                           vldh_t_node node)
{
  ...
  else
  {
    ...
    info_pointer->y = f_height / 2.0 - f_header / 2 - (point - inputpoints) * f_repeat;
 ...
 *
 **************************************************************************/
int goen_get_parameter_m1(pwr_sGraphPlcNode* graphbody, pwr_tClassId cid, ldh_tSesContext ldhses,
 ...
 *
 **************************************************************************/
int goen_get_location_point_m1(WGre* grectx, pwr_sGraphPlcNode* graphbody, unsigned int* mask,
