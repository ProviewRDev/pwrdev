...
wb_print_wbl::wb_print_wbl(ostream& os, int levelIndentation)
    : m_errCnt(0), m_idxFlag(true), m_noFoCodeFlag(false), m_timeFlag(true), m_level(0),
      m_levelInd(levelIndentation), m_keepName(false), m_isTemplateObject(false), m_os(os)
{
...
// printAttribute
//
void wb_print_wbl::printAttribute(wb_volume& v, wb_attribute& attr,
                                  wb_attribute& tattr, ///< template
                                  wb_adef& adef, int force)
{
  if (!force && (adef.flags() & PWR_MASK_POINTER || adef.flags() & PWR_MASK_NOWBL))
    return;
  ...
  else
  {
    ...
    case pwr_eClass_ObjXRef:
      ...
      break;
    default:
      m_os << "! %%WBDUMP-E-Error Unknown attribute class: " << adef.name() << ", cid: " << adef.cid()
           << endl;
      m_errCnt++;
      break;
    }
...
// printBody
//
void wb_print_wbl::printBody(wb_volume& v, wb_object& o, wb_object& templ, wb_cdef& cdef, pwr_eBix bix)
{
  wb_adef adef;
  wb_attribute attr;
  wb_attribute tattr;
...
// printBuffer
//
void wb_print_wbl::printBuffer(wb_volume& v, wb_attribute& attr, wb_attribute& tattr, wb_adef& adef)
{
  ...
  if (!bdef)
  {
    m_os << "! %WBDUMP-E-Error sub class: " << subClass << " not defined" << endl;
    m_errCnt++;
    return;
  }
  bname = bdef.name();
...
// printClass
//
void wb_print_wbl::printClass(wb_volume& v, wb_attribute& attr, wb_attribute& tattr, wb_adef& adef)
{
  pwr_tCid subClass = attr.subClass();
  wb_object templ;
  wb_object sysbo;
...
// printParameter
//
void wb_print_wbl::printParameter(wb_volume& v, wb_attribute& attr,
                                  wb_attribute& tattr, ///< template
                                  wb_adef& adef)
{
  ...
  if (valueb == NULL)
  {
    m_os << "! %WBDUMP-E-Error Failed to get attribute address for " << adef.name() << endl;
    m_errCnt++;
    return;
  }
  ...
  for (int i = 0; i < nElement; i++)
  {
    ...
    case pwr_eType_Struct:
      ...
      break;
    default:
      m_os << "! %WBDUMP-E-Error Attribute " << adef.name() << " is of unknown type: " << adef.type() << endl;
      m_errCnt++;
      break;
    }
  }
...
// printText
//
void wb_print_wbl::printText(wb_volume& v, wb_adef& adef, const char* text, int varSize)
{
  const char* ip;
  int i;
  int end = varSize - 1;
...
// printValue
//
bool wb_print_wbl::printValue(wb_volume& v, wb_adef& adef, void* val, int varSize, char** svalp)
{
  unsigned long sts;
  char timbuf[24];
  char str[40];
  ...
  case pwr_eType_Time:
  {
    ...
    {
      sts = time_AtoAscii((pwr_tTime*)val, time_eFormat_DateAndTime, timbuf, sizeof(timbuf));
      if (ODD(sts))
      {
...
void wb_print_wbl::printVolume(wb_volume& v, bool recursive)
{
  ...
  indent(1) << "Volume " << v.name() << " " << cname << " " << cdh_VolumeIdToString(0, 0, v.vid(), 0, 0)
            << " " << endl;
