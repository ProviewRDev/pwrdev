...
int goen_create_nodetype_m14(pwr_sGraphPlcNode* graphbody, pwr_tClassId cid, ldh_tSesContext ldhses,
                             ...
                             unsigned long node_width, flow_tNodeClass* node_class, vldh_t_node node)
{
  ...
  sts = ldh_GetObjectPar((node->hn.wind)->hw.ldhses, node->ln.oid, "DevBody", size_attr, &parvalue, &size);
  if (EVEN(sts))
    return sts;

  flow_MeasureAnnotText(ctx, parvalue, flow_eDrawType_TextRoboto, GOEN_F_TEXTSIZE, flow_eAnnotType_MultiLine,
  ...
  f_height =
      (MAX(inputpoints, outputpoints) + 2) * f_repeat + (floor(code_height / GOEN_F_GRID) + 1) * GOEN_F_GRID;
  ...
  flow_AddLine(nc_pid, 0, f_height - f_repeat - f_yoffs, f_width, f_height - f_repeat - f_yoffs,
               flow_eDrawType_Line, 2);

  flow_AddLine(nc_pid, 0, (MAX(inputpoints, outputpoints) + 1) * f_repeat - f_yoffs, f_width,
               (MAX(inputpoints, outputpoints) + 1) * f_repeat - f_yoffs, flow_eDrawType_Line, 2);
    ...
    if (bodydef[i].ParClass == pwr_eClass_Input)
    {
      if ((*inmask_pointer & inpointmask) != 0)
      {
        flow_AddText(nc_pid, bodydef[i].Par->Input.Graph.GraphName, f_nameoffin,
                     f_repeat * (1.5 + i_inpoints) + f_strheight / 2 - f_yoffs, flow_eDrawType_TextRoboto, 2);
        ...
        {
          flow_AddLine(nc_pid, -f_pinlength, f_repeat * (1.5 + i_inpoints) - f_yoffs, -f_circle,
                       f_repeat * (1.5 + i_inpoints) - f_yoffs, flow_eDrawType_Line, 2);
          flow_AddArc(nc_pid, -f_circle, f_repeat * (1.5 + i_inpoints) - f_circle / 2 - f_yoffs, 0,
                      f_repeat * (1.5 + i_inpoints) + f_circle / 2 - f_yoffs, 0, 360, flow_eDrawType_Line, 2);
        }
  ...
  if (node_width == 0)
    f_node_width = 6;
  flow_AddAnnot(nc_pid, f_strlength, f_height - (f_repeat - f_strheight) / 2 - f_yoffs, 2,
                flow_eDrawType_TextRoboto, GOEN_F_TEXTSIZE, flow_eAnnotType_OneLine, flow_mDisplayLevel_1);
  flow_AddAnnot(
      nc_pid, f_strlength,
      f_height - (f_repeat - f_strheight) / 2 - f_repeat - code_height + code_height / annot_rows - f_yoffs,
      0, flow_eDrawType_TextRoboto, GOEN_F_TEXTSIZE, flow_eAnnotType_MultiLine, flow_mDisplayLevel_1);
 ...
 *
 **************************************************************************/
int goen_get_point_info_m14(WGre* grectx, pwr_sGraphPlcNode* graphbody, unsigned long point,
 ...
 *
 **************************************************************************/
int goen_get_parameter_m14(pwr_sGraphPlcNode* graphbody, pwr_tClassId cid, ldh_tSesContext ldhses,
 ...
 *
 **************************************************************************/
int goen_get_location_point_m14(WGre* grectx, pwr_sGraphPlcNode* graphbody, unsigned int* mask,
