...
public:
  wb_vrepced(wb_erep* erep, wb_vrep* vrep) : m_erep(erep), m_merep(erep->merep()), m_nRef(0), m_vrep(vrep)
  {
    m_vrep->ref();
    m_vid = m_vrep->vid();
    m_cid = m_vrep->cid();
...
private:
  ...
  void updateTemplateRef(wb_adrep* subattr, char* body, pwr_tAttrRef aref, pwr_tOid toid);
  void error(const char* msg, wb_orep* o);
  bool classeditorCheck(ldh_eDest dest_code, wb_orep* dest, pwr_tCid cid, pwr_tOix* oix, char* name,
                        pwr_tStatus* sts, bool import_paste);
  bool classeditorCheckMove(wb_orep* o, ldh_eDest dest_code, wb_orep* dest, pwr_tStatus* sts);
