...
static wnav_sEnumElement elem_yes_no[] = {{0, "No"}, {1, "Yes"}, {0, ""}};
static wnav_sEnumElement elem_on_off[] = {{0, "Off"}, {1, "On"}, {0, ""}};
static wnav_sEnumElement elem_color_theme[] = {{0, "Standard"},
                                               {1, "Sand"},
                                               {2, "Maroon"},
                                               {3, "Sienna"},
                                               {4, "DarkBlue"},
...
wnav_sEnum wnav_enum_types[] = {{(unsigned int)wnav_eType_YesNo, (wnav_sEnumElement*)&elem_yes_no},
...
int WNav::local_enum_to_string(int type_id, pwr_tEnum enumval, char* str, int strsize)
{
  wnav_sEnumElement* elem_p;
  wnav_sEnum* enum_p;
  int found = 0;
...
void wnav_attrvalue_to_string(ldh_tSesContext ldhses, int type_id, void* value_ptr, char** buff, int* len)
{
  ...
  case pwr_eType_Objid:
  {
    ...
    if (objid.vid == info.Volume)
      sts = ldh_ObjidToName(ldhses, objid, ldh_eName_Hierarchy, str, sizeof(str), len);
    else
      sts = ldh_ObjidToName(ldhses, objid, ldh_eName_VolPath, str, sizeof(str), len);
    if (EVEN(sts))
    {
      strcpy(str, "");
      *len = 0;
  ...
  case pwr_eType_Time:
  {
    ...
    {
      sts = time_AtoAscii((pwr_tTime*)value_ptr, time_eFormat_DateAndTime, str, sizeof(str));
...
// of the object.
//
int WNav::create_object_item(pwr_tObjid objid, brow_tNode dest, flow_eDest dest_code, void** item,
                             int is_root)
{
  int sts;
  pwr_tClassId classid;
...
//
WNav::WNav(void* xn_parent_ctx, const char* xn_name, const char* xn_layout, ldh_tSesContext xn_ldhses,
           wnav_sStartMenu* root_menu, wnav_eWindowType xn_type, pwr_tStatus* status)
    : WUtility(wb_eUtility_WNav), parent_ctx(xn_parent_ctx), window_type(xn_type), ldhses(xn_ldhses),
      wbctx(0), brow(0), brow_cnt(0), trace_started(0), message_cb(NULL), close_cb(NULL), map_cb(NULL),
      change_value_cb(NULL), script_filename_cb(NULL), get_build_options_cb(NULL),
      update_color_theme_cb(NULL), ccm_func_registred(0), menu_tree(NULL), closing_down(0),
      base_priv(pwr_mPrv_System), priv(pwr_mPrv_System), editmode(0), layout_objid(pwr_cNObjid),
      search_last(pwr_cNObjid), search_compiled(0), search_type(wnav_eSearchType_No), selection_owner(0),
      last_selected(0), prev_selected(0), displayed(0), scriptmode(0), dialog_width(0), dialog_height(0),
...
//
void WNav::get_zoom(double* zoom_factor) { brow_GetZoom(brow->ctx, zoom_factor); }

//
//  Zoom
//
void WNav::zoom(double zoom_factor) { brow_Zoom(brow->ctx, zoom_factor); }

//
//  Return to base zoom factor
//
void WNav::unzoom() { brow_UnZoom(brow->ctx); }
...
int WNav::set_attr_value(brow_tObject node, pwr_tObjid objid, char* value_str)
{
  ...
  case wnav_eItemType_AttrOutput:
  {
    ...
    {
      if (item->type_id == pwr_eType_AttrRef && item->classid == pwr_eClass_TargetAttribute)
      {
        pwr_tCid cid;
...
//
int WNav::check_attr_value(brow_tObject node, int* multiline, char** init_value, int* size)
{
  WItem* base_item;
  int sts;
  char* p;
...
int WNav::get_select(pwr_sAttrRef** attrref, int** is_attr, int* cnt)
{
  ...
  *cnt = node_count;
  ...
  for (i = 0; i < node_count; i++)
  {
    ...
    sts = ldh_ObjidToName(ldhses, item->objid, ldh_eName_VolPath, attr_str, sizeof(attr_str), &size);
    if (EVEN(sts))
      return sts;
...
int WNav::open_plc()
{
  ...
  case wnav_eItemType_Object:
    ...
    if (classid == pwr_cClass_plc || classid == pwr_cClass_PlcTemplate)
      sts = open_foe("PlcProgram1", item->objid, &foectx, 1, ldh_eAccess_ReadOnly, pwr_cNOid);
    break;
  default:
    sts = 0;
  }
...
int WNav::brow_cb(FlowCtx* ctx, flow_tEvent event)
{
  ...
  case flow_eEvent_MB1DoubleClick:
    ...
    case flow_eObjectType_Node:
      ...
      {
      case wnav_eItemType_Object:
        ((WItemObject*)item)->open_children(wnav, event->object.x, event->object.y);
        break;
      case wnav_eItemType_Menu:
        ((WItemMenu*)item)->open_children(wnav, event->object.x, event->object.y);
        break;
      case wnav_eItemType_Command:
        ((WItemCommand*)item)->open_children(wnav, event->object.x, event->object.y);
        break;
      case wnav_eItemType_Volume:
        ((WItemVolume*)item)->open_children(wnav, event->object.x, event->object.y);
        break;
      case wnav_eItemType_File:
        ((WItemFile*)item)->open_children(wnav, event->object.x, event->object.y);
        break;
      case wnav_eItemType_AttrArray:
      ...
      case wnav_eItemType_AttrArrayElem:
        ...
        break;
      case wnav_eItemType_Local:
        ((WItemLocal*)item)->open_children(wnav, event->object.x, event->object.y);
        break;
      default:;
      }
      break;
  ...
  case flow_eEvent_MB1DoubleClickShift:
    ...
    case flow_eObjectType_Node:
      ...
      {
      case wnav_eItemType_Object:
        ((WItemObject*)item)->open_attributes(wnav, event->object.x, event->object.y);
        break;
      case wnav_eItemType_AttrArray:
        ((WItemAttrArray*)item)->open_attributes(event->object.x, event->object.y);
        break;
      ...
      case wnav_eItemType_AttrArrayElem:
        ...
        break;
      case wnav_eItemType_AttrObject:
        ((WItemAttrObject*)item)->open_children(event->object.x, event->object.y);
        break;
      default:;
      }
      break;
      ...
      case wnav_eItemType_AttrArrayElem:
      {
        ...
        if (item_attr->type_id == pwr_eType_Objid)
        {
          if (wnav->get_global_select_cb)
            sts = (wnav->get_global_select_cb)(wnav->parent_ctx, &sel_list, &sel_is_attr, &sel_cnt);
          if (sel_cnt > 1)
          {
          ...
          {
            sts =
                ldh_ObjidToName(wnav->ldhses, sel_list[0].Objid, ldh_eName_VolPath, str, sizeof(str), &size);
            if (EVEN(sts))
              return sts;
          }
          else
        ...
        if (item_attr->type_id == pwr_eType_AttrRef)
        {
          ...
          else if (sel_cnt)
          {
            ...
            sts = ldh_AttrRefToName(wnav->ldhses, &sel_list[0], ldh_eName_VolPath, &namep, &size);
            if (EVEN(sts))
              return sts;
  ...
  case flow_eEvent_MB2Click:
  {
    ...
    if (wnav->get_global_select_cb)
      (wnav->get_global_select_cb)(wnav->parent_ctx, &sel_list, &sel_is_attr, &sel_cnt);
    else
      break;
    ...
    if (!sel_cnt)
    {
      ...
      case flow_eObjectType_Node:
        ...
        case wnav_eItemType_Object:
          ...
          sts = ldh_CreateObject(wnav->ldhses, &objid, 0, classid, ((WItemObject*)item)->objid, destcode);
          if (EVEN(sts))
      ...
      default:
        ...
        sts = ldh_CreateObject(wnav->ldhses, &objid, 0, classid, pwr_cNObjid, ldh_eDest_IntoLast);
        if (EVEN(sts))
          wnav->message(' ', wnav_get_message(sts));
      }
    }
    else
    {
      ...
      if (event->object.object_type == flow_eObjectType_Node)
      {
        ...
        {
          sts = ldh_MoveObject(wnav->ldhses, sel_list->Objid, item->objid, destcode);
          if (EVEN(sts))
          {
            wnav->message(' ', wnav_get_message(sts));
            return sts;
...
int WNav::trace_connect_bc(brow_tObject object, char* name, char* attr, flow_eTraceType type, void** p)
{
  WItem* base_item;
...
int WNav::trace_disconnect_bc(brow_tObject object) { return 1; }
...
int WNav::setup()
{
  ...
  new WItemLocal(this, "DefaultDirectory", "setup_defaultdirectory", pwr_eType_String,
                 sizeof(gbl.default_directory), 0, 0, (void*)gbl.default_directory, NULL,
                 flow_eDest_IntoLast);
  new WItemLocal(this, "SymbolFilename", "setup_symbolfilename", pwr_eType_String, sizeof(gbl.symbolfilename),
                 0, 0, (void*)gbl.symbolfilename, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "Verify", "setup_verify", wnav_eType_YesNo, sizeof(gbl.verify), 0, 1,
                 (void*)&gbl.verify, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "EnableComment", "setup_comment", wnav_eType_YesNo, sizeof(gbl.enable_comment), 0, 1,
                 (void*)&gbl.enable_comment, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "EnableRevisions", "setup_revisions", wnav_eType_YesNo, sizeof(gbl.enable_revisions),
                 0, 1, (void*)&gbl.enable_revisions, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "AdvancedUser", "setup_advanceduser", wnav_eType_YesNo, sizeof(gbl.advanced_user), 0,
                 1, (void*)&gbl.advanced_user, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "AllToplevel", "setup_alltoplevel", wnav_eType_YesNo, sizeof(gbl.all_toplevel), 0, 1,
                 (void*)&gbl.all_toplevel, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "Bypass", "setup_bypass", wnav_eType_YesNo, sizeof(gbl.bypass), 0, 1,
                 (void*)&gbl.bypass, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "ShowClass", "setup_show_class", wnav_eType_YesNo, sizeof(gbl.show_class), 0, 1,
                 (void*)&gbl.show_class, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "ShowAlias", "setup_show_alias", wnav_eType_YesNo, sizeof(gbl.show_alias), 0, 1,
                 (void*)&gbl.show_alias, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "ShowDescription", "setup_show_descrip", wnav_eType_YesNo, sizeof(gbl.show_descrip), 0,
                 1, (void*)&gbl.show_descrip, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "ShowAttrRef", "setup_show_attrref", wnav_eType_YesNo, sizeof(gbl.show_attrref), 0, 1,
                 (void*)&gbl.show_attrref, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "ShowAttrXRef", "setup_show_attrxref", wnav_eType_YesNo, sizeof(gbl.show_attrxref), 0,
                 1, (void*)&gbl.show_attrxref, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "ShowObjectRef", "setup_show_objref", wnav_eType_YesNo, sizeof(gbl.show_objref), 0, 1,
                 (void*)&gbl.show_objref, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "ShowObjXRef", "setup_show_objxref", wnav_eType_YesNo, sizeof(gbl.show_objxref), 0, 1,
                 (void*)&gbl.show_objxref, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "ShowTrueDb", "setup_show_truedb", wnav_eType_YesNo, sizeof(gbl.show_truedb), 0, 1,
                 (void*)&gbl.show_truedb, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "Build.Force", "setup_build_force", wnav_eType_YesNo, sizeof(gbl.build.force), 0, 1,
                 (void*)&gbl.build.force, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "Build.Debug", "setup_build_debug", wnav_eType_YesNo, sizeof(gbl.build.debug), 0, 1,
                 (void*)&gbl.build.debug, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "Build.Syntax", "setup_build_syntax", wnav_eType_YesNo, sizeof(gbl.build.syntax), 0, 1,
                 (void*)&gbl.build.syntax, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "Build.CrossReferences", "setup_build_cross", wnav_eType_YesNo,
                 sizeof(gbl.build.crossref), 0, 1, (void*)&gbl.build.crossref, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "Build.CrossReferences.Graph", "setup_build_cross_graph", wnav_eType_YesNo,
                 sizeof(gbl.build.crossref_graph), 0, 1, (void*)&gbl.build.crossref_graph, NULL,
                 flow_eDest_IntoLast);
  new WItemLocal(this, "Build.CrossReferences.Sim", "setup_build_cross_sim", wnav_eType_YesNo,
                 sizeof(gbl.build.crossref_sim), 0, 1, (void*)&gbl.build.crossref_sim, NULL,
                 flow_eDest_IntoLast);
  new WItemLocal(this, "Build.Manual", "setup_build_manual", wnav_eType_YesNo, sizeof(gbl.build.manual), 0, 1,
                 (void*)&gbl.build.manual, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "Build.NoCopy", "setup_build_nocopy", wnav_eType_YesNo, sizeof(gbl.build.nocopy), 0, 1,
                 (void*)&gbl.build.nocopy, NULL, flow_eDest_IntoLast);
  new WItemLocal(this, "ColorTheme", "setup_color_theme", wnav_eType_ColorTheme, sizeof(gbl.color_theme), 0,
                 1, (void*)&gbl.color_theme, NULL, flow_eDest_IntoLast);
...
WNavGbl::WNavGbl()
    : priv(0), verify(0), advanced_user(1), all_toplevel(0), bypass(0), show_class(1), show_alias(0),
      show_descrip(1), show_attrref(0), show_attrxref(0), show_objref(0), show_objxref(0), show_truedb(0),
...
void WNav::set_options(int ena_comment, int ena_revisions, int sh_class, int sh_alias, int sh_descrip,
                       ...
                       int bu_debug, int bu_syntax, int bu_crossref, int bu_crossrefsim, int bu_crossrefgraph,
...
int WNavGbl::load_config(void* wnav) { return WNAV__SUCCESS; }
...
int WNav::brow_pop(wnav_eBrowType type)
{
  ...
  if (brow_cnt >= WNAV_BROW_MAX)
    return 0;
  brow_CreateSecondaryCtx(brow->ctx, &secondary_ctx, WNav::init_brow_cb, (void*)this, flow_eCtxType_Brow);
...
void WNav::menu_tree_build(wnav_sStartMenu* root) { menu_tree = menu_tree_build_children(root, NULL); }

wnav_sMenu* WNav::menu_tree_build_children(wnav_sStartMenu* first_child, wnav_sMenu* parent)
{
  ...
  while (strcmp(start_menu_p->title, ""))
  {
    ...
    case wnav_eItemType_Menu:
      ...
      menu_p->child_list = menu_tree_build_children((wnav_sStartMenu*)start_menu_p->action, menu_p);
      if (first)
      {
        return_menu = menu_p;
        first = 0;
...
void WNav::menu_tree_free() { menu_tree_free_children(menu_tree); }
...
int WNav::menu_tree_insert(char* title, int item_type, char* command, char* destination, int dest_code,
...
int WNav::menu_tree_search_children(char* name, wnav_sMenu* child_list, wnav_sMenu** menu_item)
{
  wnav_sMenu* menu_p;
  char* s;
...
void WNav::ldh_refresh(pwr_tObjid new_open)
{
  ...
  if (last_selected)
  {
    ...
    {
      brow_GetUserData(object_list[j], (void**)&object_item);
      if (cdh_ObjidIsEqual(last_sel_objid, object_item->objid) && last_sel_type == object_item->type)
      {
        switch (object_item->type)
        {
        case wnav_eItemType_Attr:
  ...
  if (prev_selected)
  {
    ...
    {
      brow_GetUserData(object_list[j], (void**)&object_item);
      if (cdh_ObjidIsEqual(prev_sel_objid, object_item->objid) && prev_sel_type == object_item->type)
      {
        switch (object_item->type)
        {
        case wnav_eItemType_Attr:
...
void WNav::ldh_event(ldh_sEvent* e)
{
  ...
  while (event)
  {
    ...
    case ldh_eEvent_ObjectCreated:
      if (cdh_ObjidIsNull(event->NewParent) || find(event->NewParent, (void**)&item))
        ldh_refresh(event->NewParent);
      break;

    case ldh_eEvent_ObjectDeleted:
      if (cdh_ObjidIsNull(event->NewParent) || find(event->OldParent, (void**)&item))
        ldh_refresh(event->NewParent);
      break;

    case ldh_eEvent_ObjectMoved:
      if (cdh_ObjidIsNull(event->NewParent) || cdh_ObjidIsNull(event->OldParent) ||
          find(event->NewParent, (void**)&item) || find(event->OldParent, (void**)&item))
        ldh_refresh(event->NewParent);
      break;
...
{
  brow_EnableEvent(brow->ctx, flow_eEvent_MB1DoubleClickShift, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_MB1DoubleClick, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_MB1DoubleClickCtrl, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_MB1Click, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_MB1ClickShift, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_MB2Click, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_SelectClear, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_ObjectDeleted, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_Key_Up, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_Key_Down, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_Key_ShiftUp, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_Key_ShiftDown, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_Key_PF1, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_Key_PF2, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_Key_PF3, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_Key_PF4, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_Key_Return, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_Key_Right, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_Key_Left, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_Key_PageUp, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_Key_PageDown, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_Key_ShiftRight, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_Key_Tab, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_MB1Press, flow_eEventType_RegionSelect, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_MB1PressShift, flow_eEventType_RegionAddSelect, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_MB3Press, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_MB3Down, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_MB2Down, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_Map, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_Radiobutton, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_ScrollUp, flow_eEventType_CallBack, WNav::brow_cb);
  brow_EnableEvent(brow->ctx, flow_eEvent_ScrollDown, flow_eEventType_CallBack, WNav::brow_cb);
}
...
int WNav::init_brow_base_cb(FlowCtx* fctx, void* client_data)
{
  ...
  if (!wnav->ldhses)
  {
    ...
    wnav->root_item = new WItemMenu(wnav, "Root", NULL, flow_eDest_After, &wnav->menu_tree, 1);

    // Open the root item
    ((WItemMenu*)wnav->root_item)->open_children(wnav, 0, 0);
  }
  sts = brow_TraceInit(ctx, WNav::trace_connect_bc, WNav::trace_disconnect_bc, WNav::trace_scan_bc);
...
ApplListElem::ApplListElem(applist_eType al_type, void* al_ctx, pwr_tObjid al_objid, const char* al_name)
    : type(al_type), ctx(al_ctx), objid(al_objid), next(NULL)
{
  strcpy(name, al_name);
}

void ApplList::insert(applist_eType type, void* ctx, pwr_tObjid objid, const char* name)
{
