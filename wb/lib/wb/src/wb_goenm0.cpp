...
int goen_create_nodetype_m0(pwr_sGraphPlcNode* graphbody, pwr_tClassId cid, ldh_tSesContext ldhses,
                            ...
                            unsigned long node_width, flow_tNodeClass* node_class, vldh_t_node node)
{
  ...
  if (annot_count > 0)
  {
    flow_MeasureAnnotText(ctx, annot_str[0], flow_eDrawType_TextRoboto, GOEN_F_TEXTSIZE,
                          flow_eAnnotType_OneLine, &annot_width, &annot_height, &annot_rows);
  }
  ...
  f_width = MAX(f_strlength * 4 + MAX(classname_width, annot_width), f_defwidth + f_strlength * 2);
    ...
    if ((*inmask_pointer & pointmask) != 0)
    {
      if (bodydef[i].ParClass == pwr_eClass_Input)
      {
        flow_AddText(nc_pid, bodydef[i].Par->Input.Graph.GraphName, f_nameoffin,
                     f_repeat * (1.5 + ipoints) + f_strheight / 2 - f_yoffs, flow_eDrawType_TextRoboto, 2);
      }
      if ((*invertmask_pointer & pointmask) == 0)
      {
        flow_AddLine(nc_pid, 0, f_repeat * (1.5 + ipoints) - f_yoffs, -f_pinlength,
                     f_repeat * (1.5 + ipoints) - f_yoffs, flow_eDrawType_Line, 2);
      }
      else
      {
        flow_AddLine(nc_pid, -f_pinlength, f_repeat * (1.5 + ipoints) - f_yoffs, -f_circle,
                     f_repeat * (1.5 + ipoints) - f_yoffs, flow_eDrawType_Line, 2);
        flow_AddArc(nc_pid, -f_circle, f_repeat * (1.5 + ipoints) - f_circle / 2 - f_yoffs, 0,
                    f_repeat * (1.5 + ipoints) + f_circle / 2 - f_yoffs, 0, 360, flow_eDrawType_Line, 2);
      }
      flow_AddConPoint(nc_pid, -f_pinlength, f_repeat * (1.5 + ipoints) - f_yoffs, conpoint_nr++,
  ...
  for (i = i_innr + i_internnr; i < i_innr + i_internnr + i_outnr; i++)
  {
    if ((*outmask_pointer & pointmask) != 0)
    {
      ...
      flow_AddLine(nc_pid, f_width, f_repeat * (1.5 + ipoints) - f_yoffs, f_width + f_pinlength,
                   f_repeat * (1.5 + ipoints) - f_yoffs, flow_eDrawType_Line, 2);
      flow_CreateConPoint(ctx, f_width + f_pinlength, f_repeat * (ipoints + 1.5) - f_yoffs, conpoint_nr++,
      ...
      if (bodydef[i].Par->Output.Info.Type == pwr_eType_Float32)
        ...
        flow_SetTraceAttr(cp, NULL, bodydef[i].ParName, flow_eTraceType_DataRef, 0);
      ipoints++;
    }
    pointmask <<= 1;
  }
  ...
  flow_AddAnnot(nc_pid, f_width / 2 - annot_width / 2, f_height - (f_repeat - f_strheight) / 2.0 - f_yoffs, 0,
                flow_eDrawType_TextRoboto, GOEN_F_TEXTSIZE, flow_eAnnotType_OneLine, flow_mDisplayLevel_1);
  /* no used ???
    flow_AddAnnot( nc_pid, , f_height/2.0,
           1, flow_eDrawType_TextRoboto, GOEN_F_TEXTSIZE,
 ...
 *
 **************************************************************************/
int goen_get_point_info_m0(WGre* grectx, pwr_sGraphPlcNode* graphbody, unsigned long point,
 ...
 *
 **************************************************************************/
int goen_get_parameter_m0(pwr_sGraphPlcNode* graphbody, pwr_tClassId cid, ldh_tSesContext ldhses,
 ...
 *
 **************************************************************************/
int goen_get_location_point_m0(WGre* grectx, pwr_sGraphPlcNode* graphbody, unsigned int* mask,
