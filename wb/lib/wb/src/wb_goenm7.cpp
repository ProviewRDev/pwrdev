...
int goen_create_nodetype_m7(pwr_sGraphPlcNode* graphbody, pwr_tClassId cid, ldh_tSesContext ldhses,
                            ...
                            unsigned long node_width, flow_tNodeClass* node_class, vldh_t_node node)
{
  ...
  f_height = f_repeat;
  if (annot_count <= 1)
    f_width = MAX(f_strlength * 2 + annot_width[0], f_defwidth) + f_classnamewidth;
  else
  {
    f_width = MAX(f_strlength * 4 + annot_width[0] + annot_width[1], f_defwidth + f_strlength * 2) +
              f_classnamewidth;
  ...
  flow_AddText(nc_pid, graphbody->graphname, f_strlength, f_height / 2 + f_strheight / 2 - f_yoffs,
  ...
  if (inputpoints != 0)
  {
    ...
    {
      flow_AddLine(nc_pid, -f_pinlength, f_height / 2 - f_yoffs, 0, f_height / 2 - f_yoffs,
                   flow_eDrawType_Line, 2);
    }
    else
    {
      flow_AddLine(nc_pid, -f_pinlength, f_height / 2 - f_yoffs, -f_circle, f_height / 2 - f_yoffs,
                   flow_eDrawType_Line, 2);
      flow_AddArc(nc_pid, -f_circle, f_height / 2 - f_circle / 2 - f_yoffs, 0,
                  f_height / 2 + f_circle / 2 - f_yoffs, 0, 360, flow_eDrawType_Line, 2);
    }
    flow_AddConPoint(nc_pid, -f_pinlength, f_height / 2 - f_yoffs, conpoint_nr++, flow_eDirection_Left);
  }

  if (outputs > 0 && outputpoints != 0)
  {
    flow_AddLine(nc_pid, f_width, f_height / 2 - f_yoffs, f_width + f_pinlength, f_height / 2 - f_yoffs,
                 flow_eDrawType_Line, 2);
    flow_CreateConPoint(ctx, f_width + f_pinlength, f_height / 2 - f_yoffs, conpoint_nr++,
  ...
  flow_AddAnnot(nc_pid, f_classnamewidth + f_strlength, f_height / 2 + f_strheight / 2 - f_yoffs, 0,
                flow_eDrawType_TextRoboto, GOEN_F_TEXTSIZE, flow_eAnnotType_OneLine, flow_mDisplayLevel_1);

  if (annot_count >= 2)
  {
    flow_AddAnnot(nc_pid, f_width - f_width_left + f_strlength, f_height / 2 + f_strheight / 2 - f_yoffs, 1,
                  flow_eDrawType_TextRoboto, GOEN_F_TEXTSIZE, flow_eAnnotType_OneLine, flow_mDisplayLevel_1);
  }
 ...
 *
 **************************************************************************/
int goen_get_point_info_m7(WGre* grectx, pwr_sGraphPlcNode* graphbody, unsigned long point,
 ...
 *
 **************************************************************************/
int goen_get_parameter_m7(pwr_sGraphPlcNode* graphbody, pwr_tClassId cid, ldh_tSesContext ldhses,
 ...
 *
 **************************************************************************/
int goen_get_location_point_m7(WGre* grectx, pwr_sGraphPlcNode* graphbody, unsigned int* mask,
