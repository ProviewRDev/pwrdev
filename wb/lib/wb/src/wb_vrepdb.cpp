...
wb_vrepdb::wb_vrepdb(wb_erep* erep, const char* fileName) : m_erep(erep), m_nRef(0), m_ohead(), m_oid_th(0)
{
...
wb_erep* wb_vrepdb::erep() { return m_erep; }
...
wb_merep* wb_vrepdb::merep() const { return m_merep; }

wb_srep* wb_vrepdb::newSession() { return 0; }
...
bool wb_vrepdb::isOffspringOf(pwr_tStatus* sts, const wb_orep* child, const wb_orep* parent) { return false; }
...
bool wb_vrepdb::copyOset(pwr_tStatus* sts, wb_oset* oset, wb_destination& d) { return false; }
...
bool wb_vrepdb::writeAttribute(pwr_tStatus* sts, wb_orep* orp, pwr_eBix bix, size_t offset, size_t size,
                               void* p)
{
  try
  {
...
void* wb_vrepdb::readAttribute(pwr_tStatus* sts, const wb_orep* orp, pwr_eBix bix, size_t offset, size_t size,
                               void* p)
{
  try
  {
...
bool wb_vrepdb::exportVolume(wb_import& i) { return i.importVolume(*this); }
...
bool wb_vrepdb::exportDocBlock(wb_import& i) { return false; }

bool wb_vrepdb::exportMeta(wb_import& i) { return false; }

bool wb_vrepdb::exportTree(wb_treeimport& i, pwr_tOid oid) { return exportTreeHelper(i, oid, true); }
...
bool wb_vrepdb::exportTreeHelper(wb_treeimport& i, pwr_tOid oid, bool isRoot)
{
  ...
  {
    i.importTreeObject(m_merep, m_ohead.oid(), m_ohead.cid(), m_ohead.poid(), m_ohead.boid(), m_ohead.name(),
                       flags, m_ohead.rbSize(), m_ohead.dbSize(), rbody, dbody);
  }
...
void wb_vrepdb::delete_wb_orepdb(void* p) { free(p); }

bool wb_vrepdb::importPasteObject(pwr_tOid doid, ldh_eDest destcode, bool keepoid, pwr_tOid oid, pwr_tCid cid,
                                  pwr_tOid poid, pwr_tOid boid, const char* name, pwr_mClassDef flags,
                                  size_t rbSize, size_t dbSize, void* rbody, void* dbody, pwr_tOid woid,
                                  pwr_tOid* roid)
{
  pwr_tStatus sts;
  ...
  if (cdh_ObjidIsNull(poid) && cdh_ObjidIsNull(boid))
  {
    ...
    m_oid_th = tree_CreateTable(&sts, sizeof(pwr_tOid), offsetof(sOentry, o_oid), sizeof(sOentry), 1000,
                                tree_Comp_oid);
  ...
  m_db->importHead(oep->n_oid, cid, oep->parent->n_oid, pwr_cNOid, pwr_cNOid, pwr_cNOid, pwr_cNOid, name,
                   n.normName(), flags, oTime, oTime, oTime, rbSize, dbSize);
...
pwr_tStatus wb_vrepdb::checkMeta()
{
  ...
  m_class_th =
      tree_CreateTable(&sts, sizeof(pwr_tCid), offsetof(sClass, cid), sizeof(sClass), 1000, tree_Comp_cid);
  m_classchk_th = tree_CreateTable(&sts, sizeof(pwr_tCid), offsetof(sClassChk, cid), sizeof(sClassChk), 1000,
                                   tree_Comp_cid);
  m_aref_th = tree_CreateTable(&sts, sizeof(sArefKey), offsetof(sAref, key), sizeof(sAref), 1000, comp_aref);
  m_attribute_th = tree_CreateTable(&sts, sizeof(sAttributeKey), offsetof(sAttribute, key),
                                    sizeof(sAttribute), 1000, comp_attribute);
       ...
       cp = (sClass*)tree_Successor(&sts, m_class_th, cp))
  {
    ...
    if (cp->count > 0)
    {
      ...
      if (time_IsNull(&cp->n_time))
      {
        ...
        sprintf(buff, "Class \"%s\" [%s], %d object%s, does not exist in global scope", cp->name, o_timbuf,
      ...
      else
      {
        ...
        sprintf(buff, "Class \"%s\" [%s], %d object%s, can be updated to [%s]", cp->name, o_timbuf, cp->count,
                (cp->count == 1 ? "" : "s"), n_timbuf);
  ...
  if (ODD(db_sts) && tree_Cardinality(&sts, m_class_th) != 0)
  {
    ...
    sprintf(buff,
            "A total of %d object%s of %d class%s, and  %d attribute "
            "reference%s can be updated",
            totalObjectCount, (totalObjectCount == 1 ? "" : "s"), tree_Cardinality(&sts, m_class_th),
            (tree_Cardinality(&sts, m_class_th) == 1 ? "" : "es"), nAref, (nAref == 1 ? "" : "s"));
...
pwr_tStatus wb_vrepdb::updateMeta()
{
       ...
       cp = (sClass*)tree_Successor(&sts, m_class_th, cp))
  {
    ...
    if (cp->count > 0)
    {
      ...
      else
      {
        ...
        sprintf(buff, "Class \"%s\" [%s], %d object%s, %s updated to [%s]", cp->name, o_timbuf, cp->count,
                (cp->count == 1 ? "" : "s"), (cp->count == 1 ? "was" : "were"), n_timbuf);
  ...
  if (ODD(db_sts) && tree_Cardinality(&sts, m_class_th) != 0 && totalObjectCount > 0)
  {
    ...
    case pwr_eClass_RootVolume:
      writeAttribute(&sts, orep, pwr_eBix_rt, offsetof(pwr_sRootVolume, Modified), sizeof(pwr_tTime), &time);
      break;
    case pwr_eClass_SubVolume:
      writeAttribute(&sts, orep, pwr_eBix_rt, offsetof(pwr_sSubVolume, Modified), sizeof(pwr_tTime), &time);
      break;
    ...
    {
      sprintf(buff,
              "A total of %d object%s of %d classe%s %s updated, but "
              "could not be saved to database.",
              nObject, (nObject == 1 ? "" : "s"), tree_Cardinality(&sts, m_class_th),
              (tree_Cardinality(&sts, m_class_th) == 1 ? "" : "s"), (nObject == 1 ? "was" : "were"));
    ...
    if (ODD(sts))
    {
      ...
      if (ODD(sts))
      {
        ...
        while (ODD(sts))
        {
          ...
          if (found)
          {
            if ((o_adrep->offset() != n_adrep->offset()) || (o_adrep->size() != n_adrep->size()) ||
                (o_adrep->type() != n_adrep->type()) ||
                //(o_adrep->tid()      != n_adrep->tid())      ||
                (o_adrep->nElement() != n_adrep->nElement()) || (o_adrep->index() != n_adrep->index()))
            {
              sAttributeKey ak;
              sAttribute* ap;
              ...
              if (o_adrep->isArray())
              {
                for (int j = 0; j < MIN(o_adrep->nElement(), n_adrep->nElement()); j++)
                {
                  checkSubClass(cid, o_adrep->subClass(),
                                ...
                                n_adrep->offset() + j * (n_adrep->size() / n_adrep->nElement()), body);
                }
              }
              else
                checkSubClass(cid, o_adrep->subClass(), o_adrep->offset(), n_adrep->offset(), body);
            }
            delete n_adrep;
          }
  ...
  while (ODD(sts))
  {
    ...
    if (found)
    {
      if (((o_adrep->offset() + o_offset) != (n_adrep->offset() + n_offset)) ||
          (o_adrep->size() != n_adrep->size()) || (o_adrep->type() != n_adrep->type()) ||
          //(o_adrep->tid()    != n_adrep->tid())      ||
          (o_adrep->nElement() != n_adrep->nElement()) || (o_adrep->index() != n_adrep->index()))
      {
        sAttributeKey ak;
        ...
        if (o_adrep->isArray())
        {
          for (int j = 0; j < MIN(o_adrep->nElement(), n_adrep->nElement()); j++)
          {
            checkSubClass(cid, n_adrep->subClass(),
                          ...
                          n_adrep->offset() + n_offset + j * (n_adrep->size() / n_adrep->nElement()), body);
          }
        }
        else
          checkSubClass(cid, n_adrep->subClass(), o_adrep->offset() + o_offset, n_adrep->offset() + n_offset,
                        body);
      }
