...
typedef std::map<std::string, std::vector<opcsrv_sub>>::iterator sublist_iterator;
typedef std::map<int, opcsrv_client>::iterator client_iterator;

class opc_server
{
public:
  opc_server() : m_client_access_cnt(0), m_grant_all(true), m_client(0), qid(qcom_cNQid) {}
...
static int opcsrv_set_error(struct soap* soap, const char* faultcode, const char* faultsubcode,
                            const char* faultstring, const char* faultdetail, int soaperror)
{
...
static int opcsrv_fault(struct soap* soap, const char* faultcode, const char* faultsubcode,
...
int main()
{
  ...
  for (int i = 0;
       i < (int)(sizeof(opcsrv->m_config->ClientAccess) / sizeof(opcsrv->m_config->ClientAccess[0])); i++)
  {
                                          ...
                                          _s0__GetStatusResponse* s0__GetStatusResponse)
{
  ...
  {
    s0__GetStatusResponse->GetStatusResult->ClientRequestHandle = soap_new_std__string(soap, -1);
...
SOAP_FMAC5 int SOAP_FMAC6 __s0__Read(struct soap* soap, _s0__Read* s0__Read,
                                     _s0__ReadResponse* s0__ReadResponse)
{
  ...
  for (ii = 0; ii < (int)s0__Read->ItemList->Items.size(); ii++)
  {
    ...
    if (EVEN(sts))
    {
      opcsrv_returnerror(soap, s0__ReadResponse->Errors, &iv->ResultID, opc_eResultCode_E_INVALIDITEMNAME,
                         options);
                                      ...
                                      _s0__WriteResponse* s0__WriteResponse)
{
  ...
  for (ii = 0; ii < (int)s0__Write->ItemList->Items.size(); ii++)
  {
    ...
    if (EVEN(sts))
    {
      opcsrv_returnerror(soap, s0__WriteResponse->Errors, &iv->ResultID, opc_eResultCode_E_INVALIDITEMNAME,
                         options);
...
SOAP_FMAC5 int SOAP_FMAC6 __s0__Subscribe(struct soap* soap, _s0__Subscribe* s0__Subscribe,
                                          _s0__SubscribeResponse* s0__SubscribeResponse)
{
  ...
  {
    s0__SubscribeResponse->SubscribeResult->ClientRequestHandle = soap_new_std__string(soap, -1);
  ...
  if (s0__Subscribe->ItemList)
  {
    ...
    for (int i = 0; i < (int)s0__Subscribe->ItemList->Items.size(); i++)
    {
      ...
      while (1)
      {
        strcpy(aname, cnv_utf8_to_iso8859((char*)s0__Subscribe->ItemList->Items[i]->ItemName->c_str(),
      ...
      if (resultid || s0__Subscribe->ReturnValuesOnReply)
      {
        ...
        if (s0__Subscribe->ItemList->Items[i]->ClientItemHandle)
        {
          ...
          iv->ItemValue->ClientItemHandle->assign(*s0__Subscribe->ItemList->Items[i]->ClientItemHandle);
        }
        if (resultid)
        {
          opcsrv_returnerror(soap, s0__SubscribeResponse->Errors, &iv->ItemValue->ResultID, resultid, 0);
        }
        else if (s0__Subscribe->ReturnValuesOnReply)
        {
          ...
          if (opc_convert_pwrtype_to_opctype(sub.p, buf, sizeof(buf), reqType, a_tid))
          {
            ...
            opcsrv_returnerror(soap, s0__SubscribeResponse->Errors, &iv->ItemValue->ResultID,
                               opc_eResultCode_E_BADTYPE, 0);
        }
...
SOAP_FMAC5 int SOAP_FMAC6 __s0__SubscriptionPolledRefresh(
    struct soap* soap, _s0__SubscriptionPolledRefresh* s0__SubscriptionPolledRefresh,
    _s0__SubscriptionPolledRefreshResponse* s0__SubscriptionPolledRefreshResponse)
{
  ...
  s0__SubscriptionPolledRefreshResponse->SubscriptionPolledRefreshResult = soap_new_s0__ReplyBase(soap, -1);
  ...
  if (s0__SubscriptionPolledRefresh->Options && s0__SubscriptionPolledRefresh->Options->ClientRequestHandle)
  {
    s0__SubscriptionPolledRefreshResponse->SubscriptionPolledRefreshResult->ClientRequestHandle =
        soap_new_std__string(soap, -1);
    s0__SubscriptionPolledRefreshResponse->SubscriptionPolledRefreshResult->ClientRequestHandle->assign(
  ...
  if (s0__SubscriptionPolledRefresh->HoldTime)
  {
    ...
    else
    {
      ...
      opc_time_OPCAsciiToA((char*)s0__SubscriptionPolledRefresh->HoldTime->c_str(), &hold_time);
  ...
  if (has_waittime)
  {
    ...
    for (waited_time = 0; waited_time < wait_time; waited_time += (int)(wait_scan * 1000))
    {
      ...
      {
        sublist_iterator it = client->m_sublist.find(s0__SubscriptionPolledRefresh->ServerSubHandles[i]);
  ...
  for (int i = 0; i < (int)s0__SubscriptionPolledRefresh->ServerSubHandles.size(); i++)
  {
    ...
    if (it != client->m_sublist.end())
    {
      ...
      for (int j = 0; j < (int)it->second.size(); j++)
      {
        ...
        ritem->Value =
            opc_opctype_to_value(soap, it->second[j].p, it->second[j].size, it->second[j].opc_type);
...
__s0__SubscriptionCancel(struct soap* soap, _s0__SubscriptionCancel* s0__SubscriptionCancel,
                         _s0__SubscriptionCancelResponse* s0__SubscriptionCancelResponse)
{
  ...
  sublist_iterator it = client->m_sublist.find(*s0__SubscriptionCancel->ServerSubHandle);
  ...
  if (s0__SubscriptionCancel->ClientRequestHandle)
  {
    ...
    s0__SubscriptionCancelResponse->ClientRequestHandle->assign(*s0__SubscriptionCancel->ClientRequestHandle);
  }
  return SOAP_OK;
}
                                       ...
                                       _s0__BrowseResponse* s0__BrowseResponse)
{
  ...
  if (s0__Browse->ClientRequestHandle)
  {
    ...
    s0__BrowseResponse->BrowseResult->ClientRequestHandle->assign(*s0__Browse->ClientRequestHandle);
  }
  ...
  *s0__BrowseResponse->MoreElements = false;
  ...
  if (s0__Browse->ContinuationPoint)
  {
    ...
    for (sts = 1; ODD(sts); sts = gdh_GetNextSibling(child, &child))
    {
      ...
      if (property_mask)
      {
        ...
        opcsrv_get_properties(soap, element->IsItem, cid, &paref, &aref, property_mask, 0,
                              element->Properties);
      }
      ...
      (!s0__Browse->ItemPath || s0__Browse->ItemPath->empty()))
  {
    ...
    for (sts = gdh_GetRootList(&oid); ODD(sts); sts = gdh_GetNextSibling(oid, &oid))
    {
      ...
      opcsrv_get_properties(soap, false, cid, 0, &aref, property_mask, 0, element->Properties);
  ...
  else
  {
    ...
    if (paref.Flags.b.Array)
    {
      ...
      for (int i = 0; i < (int)a_dim; i++)
      {
        ...
        if (property_mask)
        {
          ...
          opcsrv_get_properties(soap, true, cid, &paref, &aref, property_mask, &bd[bd_idx],
                                element->Properties);
        }
        s0__BrowseResponse->Elements.push_back(element);
      }
    ...
    if (ODD(sts))
    {
      for (int i = 0; i < rows; i++)
      {
        ...
        if (bd[i].attr->Param.Info.Flags & PWR_MASK_ARRAY || bd[i].attr->Param.Info.Flags & PWR_MASK_CLASS)
        {
          s0__BrowseElement* element = soap_new_s0__BrowseElement(soap, -1);
          ...
          if (property_mask)
            opcsrv_get_properties(soap, element->IsItem, cid, &paref, &aref, property_mask, &bd[i],
                                  element->Properties);
    ...
    {
      for (sts = gdh_GetChild(paref.Objid, &child); ODD(sts); sts = gdh_GetNextSibling(child, &child))
      {
        if (has_max_elem && (int)s0__BrowseResponse->Elements.size() > max_elem)
        {
          // Max elements reached, return current oid as continuation point
...
SOAP_FMAC5 int SOAP_FMAC6 __s0__GetProperties(struct soap* soap, _s0__GetProperties* s0__GetProperties,
                                              _s0__GetPropertiesResponse* s0__GetPropertiesResponse)
{
  ...
  {
    s0__GetPropertiesResponse->GetPropertiesResult->ClientRequestHandle = soap_new_std__string(soap, -1);
  ...
  for (int i = 0; i < (int)s0__GetProperties->ItemIDs.size(); i++)
  {
    ...
    plist->ItemName->assign(cnv_utf8_to_iso8859((char*)s0__GetProperties->ItemIDs[i]->ItemName->c_str(),
    ...
    if (aref.Flags.b.Object || aref.Flags.b.ObjectAttr)
    {
      ...
      if (EVEN(sts))
      {
        opcsrv_returnerror(soap, s0__GetPropertiesResponse->Errors, &plist->ResultID, opc_eResultCode_E_FAIL,
                           0);
        s0__GetPropertiesResponse->PropertyLists.push_back(plist);
        continue;
      }
      ...
      opcsrv_get_properties(soap, false, cid, 0, &aref, property_mask, 0, plist->Properties);
    }
    else
    {
      ...
      opcsrv_get_properties(soap, true, cid, &paref, &aref, property_mask, &bd[bd_idx], plist->Properties);
      free((char*)bd);
    }
