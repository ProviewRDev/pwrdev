...
public:
  ...
  opcprv_obj(const opcprv_obj& x) : po(x.po), type(x.type), size(x.size) { strcpy(item_name, x.item_name); }
...
{
public:
  opc_provider(pvd_eEnv env = pvd_eEnv_Wb) : co_provider(env), root(0), next_oix(1), server_state(0)
  {
  ...
  void insert_object(pwr_tOix fth, pwr_tOix bws, s0__BrowseElement* element, int first, int last,
