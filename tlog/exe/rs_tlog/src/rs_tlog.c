...
static pwr_tUInt32 tlog_loglist_sort(tlog_t_loglist *loglist,
                                     int loglist_count);
static pwr_tUInt32 tlog_loglist_print(tlog_t_filectx *filectx,
                                      tlog_t_loglist *loglist,
                                      int *loglist_count,
                                      pwr_sClass_User *UserObject);
static pwr_tUInt32 tlog_loglist_add(tlog_t_filectx *filectx, pwr_tObjid objid,
                                    tlog_e_logtype logtype, const char *logtext,
                                    pwr_sAttrRef *attribute,
                                    const char *attrname, pwr_tTime *logtime,
                                    tlog_t_loglist **loglist,
                                    int *loglist_count, int *alloc);
static pwr_tUInt32 tlog_objectlist_add(pwr_tObjid object_objid,
                                       pwr_tClassId class,
                                       tlog_t_objectlist **objectlist,
                                       int *objectlist_count, int *alloc);
static pwr_tUInt32 tlog_object_add(pwr_tClassId class,
                                   tlog_t_objectlist **objectlist,
                                   int *objectlist_count, int *object_alloc);
static pwr_tUInt32 tlog_timestring(pwr_tDeltaTime *time, char *timestr);
static pwr_tUInt32 tlog_fulltimestring(pwr_tTime *time, char *timestr);
...
static pwr_tUInt32 tlog_selectlist_modify(tlog_t_filectx *filectx,
  ...
  if (log)
    printf("mod      num: %2d curr %2d check: %1d %1d %1d %1d\n",
           selectlist_count, selectlist_current, selectlists[0].check,
           selectlists[1].check, selectlists[2].check, selectlists[3].check);
...
static pwr_tUInt32 tlog_extselectlist_unmodify(tlog_t_filectx *filectx,
  ...
  if (log)
    printf("extunmod num: %2d curr %2d check: %1d %1d %1d %1d\n",
           selectlist_count, selectlist_current, selectlists[0].check,
           selectlists[1].check, selectlists[2].check, selectlists[3].check);
...
static pwr_tUInt32 tlog_extselectlist_modify(tlog_t_filectx *filectx,
  ...
  memcpy(&selectlists[selectlist_count].TLogSelectList,
         &filectx->TLogSelectList, sizeof(selectlists[0].TLogSelectList));
  /*
  ...
  */
  ...
  if (log)
    printf("extmod   num: %2d curr %2d check: %1d %1d %1d %1d\n",
           selectlist_count, selectlist_current, selectlists[0].check,
           selectlists[1].check, selectlists[2].check, selectlists[3].check);
  if (log)
...
static pwr_tUInt32 tlog_selectlist_unmodify(tlog_t_filectx *filectx,
  ...
  if (log)
    printf("unmod    num: %2d curr %2d check: %1d %1d %1d %1d\n",
           selectlist_count, selectlist_current, selectlists[0].check,
           selectlists[1].check, selectlists[2].check, selectlists[3].check);
  if (log)
    printf("alarmsel: %s\n", UserObject->SelectList[0]);
...
static pwr_tStatus tlog_mh_info_bc(mh_sMessage *MsgP) {
  ...
  if ((MsgP->Status & mh_mEventStatus_NotAck) ||
    ...
    sts = tlog_loglist_add(&filectx, pwr_cNObjid, tlog_logtype_Info,
                           EventP->Msg.EventText, NULL, EventP->Msg.EventName,
                           &eventtime, &log_list, &log_count, &log_alloc);
    if (EVEN(sts))
      return sts;
...
static pwr_tStatus tlog_mh_alarm_bc(mh_sMessage *MsgP) {
  ...
  if ((MsgP->Status & mh_mEventStatus_NotAck) ||
    ...
    case mh_eEventPrio_A:
      logtype = tlog_logtype_AlarmA;
      break;
    case mh_eEventPrio_B:
      logtype = tlog_logtype_AlarmB;
      break;
    case mh_eEventPrio_C:
      logtype = tlog_logtype_AlarmC;
      break;
    case mh_eEventPrio_D:
      logtype = tlog_logtype_AlarmD;
      break;
    default:
      logtype = 0;
    }
    eventtime = net_NetTimeToTime(&EventP->Info.EventTime);
    sts = tlog_loglist_add(&filectx, pwr_cNObjid, logtype,
                           EventP->Msg.EventText, NULL, EventP->Msg.EventName,
                           &eventtime, &log_list, &log_count, &log_alloc);
    if (EVEN(sts))
      return sts;
...
static pwr_tUInt32 tlog_alarm_connect(pwr_tObjid user_object) {
  ...
  sts = mh_OutunitConnect(user_object, mh_eOutunitType_Operator, 0, NULL,
                          tlog_mh_alarm_bc, NULL, NULL, NULL, NULL,
                          tlog_mh_info_bc, NULL, NULL);
  if (EVEN(sts))
    return sts;
...
static pwr_tUInt32 tlog_loglist_print(tlog_t_filectx *filectx,
  ...
  for (i = 0; i < *loglist_count; i++, loglist_ptr++) {
    ...
    if (filectx->extselectlist == 1 && cdh_ObjidIsNotNull(loglist_ptr->objid) &&
    ...
    case tlog_logtype_UnModify:
      strcpy(logtypestr, "Sl");
      sts = tlog_selectlist_unmodify(filectx, loglist_ptr->objid, UserObject);
      break;
    ...
    fprintf(filectx->outfile, "%11s %s %-40.40s %s\n", timestr, logtypestr,
            loglist_ptr->attrname, loglist_ptr->logtext);
...
static pwr_tUInt32 tlog_loglist_add(tlog_t_filectx *filectx, pwr_tObjid objid,
  ...
  if (cdh_ObjidIsNotNull(objid) &&
      !(logtype == tlog_logtype_Modify || logtype == tlog_logtype_UnModify ||
  ...
  if (*alloc == 0) {
    ...
    *alloc = TLOG_ALLOC;
    ...
    if (new_loglist == 0)
      return TLOG__NOMEMORY;
    memcpy(new_loglist, *loglist, *loglist_count * sizeof(tlog_t_loglist));
    free(*loglist);
    *loglist = new_loglist;
    (*alloc) += TLOG_ALLOC;
  }
  ...
  if (attribute != NULL) {
    ...
    */
    ...
    strncpy(loglist_ptr->attrname, (char *)attrname,
            sizeof(loglist_ptr->attrname));
...
static pwr_tUInt32 tlog_object_add(pwr_tClassId class,
  ...
  while (ODD(sts)) {
    ...
    sts = tlog_objectlist_add(object_objid, class, objectlist, objectlist_count,
                              object_alloc);
    if (EVEN(sts))
      return sts;
...
static pwr_tUInt32 tlog_timestring(pwr_tDeltaTime *time, char *timestr) {
  ...
  /****
          ...
          struct dsc$descriptor_s	timstr_desc =
  {sizeof(timstr)-1,DSC$K_DTYPE_T, DSC$K_CLASS_S,};
...
static pwr_tUInt32 tlog_getconfigobject(pwr_sClass_TLogConfigure **TLogConf,
  ...
  while (ODD(sts)) {
    ...
    if (class == pwr_cClass_TLogConfigure) {
      ...
      sts = gdh_DLRefObjectInfoAttrref(&attrref, (pwr_tAddress *)TLogConf,
                                       &subid);
      if (EVEN(sts))
        return sts;
  ...
  sts =
      gdh_DLRefObjectInfoAttrref(&attrref, (pwr_tAddress *)UserObject, &subid);
  if (EVEN(sts))
    return TLOG__USEROBJNOTFOUND;
...
int main(int argc, char **argv) {
  ...
  sts = tlog_object_add(pwr_cClass_TLogOpen, &TLogOpen_list, &TLogOpen_count,
                        &TLogOpen_alloc);
  if (EVEN(sts))
    LogAndExit(sts);
  ...
  sts = tlog_object_add(pwr_cClass_DtLog, &DtLog_list, &DtLog_count,
                        &DtLog_alloc);
  if (EVEN(sts))
    LogAndExit(sts);

  sts = tlog_object_add(pwr_cClass_AtLog, &AtLog_list, &AtLog_count,
                        &AtLog_alloc);
  if (EVEN(sts))
    LogAndExit(sts);

  sts = tlog_object_add(pwr_cClass_CAtLog, &CAtLog_list, &CAtLog_count,
                        &CAtLog_alloc);
  if (EVEN(sts))
    LogAndExit(sts);

  sts = tlog_object_add(pwr_cClass_CDtLog, &CDtLog_list, &CDtLog_count,
                        &CDtLog_alloc);
  if (EVEN(sts))
    LogAndExit(sts);

  sts = tlog_object_add(pwr_cClass_COtLog, &COtLog_list, &COtLog_count,
                        &COtLog_alloc);
  if (EVEN(sts))
    LogAndExit(sts);

  sts = tlog_object_add(pwr_cClass_ChgAtLog, &ChgAtLog_list, &ChgAtLog_count,
                        &ChgAtLog_alloc);
  if (EVEN(sts))
    LogAndExit(sts);

  sts = tlog_object_add(pwr_cClass_ChgOtLog, &ChgOtLog_list, &ChgOtLog_count,
                        &ChgOtLog_alloc);
  if (EVEN(sts))
    LogAndExit(sts);

  sts = tlog_object_add(pwr_cClass_TLogExecAllOn, &ExecOn_list, &ExecOn_count,
                        &ExecOn_alloc);
  if (EVEN(sts))
    LogAndExit(sts);
  ...
  while (1) {
    ...
    while (1) {
      ...
      if (filectx.outfile == 0) {
        ...
        for (i = 0; i < TLogOpen_count; i++) {
          if (((pwr_sClass_TLogOpen *)(list_ptr->object_ptr))->OpenSignal) {
            ...
            if (filectx.outfile == 0) {
              ...
              filectx.object_ptr = (pwr_sClass_TLogOpen *)list_ptr->object_ptr;
              ...
              sts = tlog_extselectlist_modify(
                  &filectx, filectx.objid, UserObject,
        ...
        if (first) {
          if (filectx.object_ptr->OpenTime.tv_sec != 0 ||
            ...
            tlog_fulltimestring(&filectx.object_ptr->OpenTime, timestr);
        ...
        for (i = 0; i < TLogModify_count; i++) {
          if (((pwr_sClass_TLogModify *)(list_ptr->object_ptr))->ModifySignal) {
            /* Modify */
            if (cdh_ObjidIsEqual(
                    ((pwr_sClass_TLogModify *)(list_ptr->object_ptr))
                        ->TLogOpenObject,
                    filectx.objid) ||
                cdh_ObjidIsNull(
                    ((pwr_sClass_TLogModify *)(list_ptr->object_ptr))
                        ->TLogOpenObject)) {
              sts = tlog_extselectlist_modify(
                  &filectx, list_ptr->objid, UserObject,
                  ((pwr_sClass_TLogModify *)(list_ptr->object_ptr))
                      ->TLogSelectList,
                  ((pwr_sClass_TLogModify *)(list_ptr->object_ptr))
                      ->AlarmSelectList);
              if (EVEN(sts))
                LogAndExit(sts);
              sts = gdh_ObjidToName(list_ptr->objid, name, sizeof(name),
                                    cdh_mNName);
              if (EVEN(sts))
                LogAndExit(sts);
              sts = tlog_loglist_add(
                  &filectx, list_ptr->objid, tlog_logtype_Modify,
                  "TLogModify selectlist active", NULL, name,
                  &((pwr_sClass_TLogModify *)(list_ptr->object_ptr))
                       ->ModifyTime,
          ...
          if (((pwr_sClass_TLogModify *)(list_ptr->object_ptr))
                  ->UnModifySignal) {
            /* UnModify */
            if (cdh_ObjidIsEqual(
                    ((pwr_sClass_TLogModify *)(list_ptr->object_ptr))
              ...
              if (EVEN(sts))
                LogAndExit(sts);
              sts = tlog_loglist_add(
                  &filectx, list_ptr->objid, tlog_logtype_UnModify,
                  "TLogModify selectlist inactive", NULL, name,
                  &((pwr_sClass_TLogModify *)(list_ptr->object_ptr))
                       ->UnModifyTime,
                  &log_list, &log_count, &log_alloc);
              if (EVEN(sts))
      ...
      list_ptr = ExecOn_list;
      for (i = 0; i < ExecOn_count; i++) {
        if (((pwr_sClass_TLogExecAllOn *)(list_ptr->object_ptr))
                ->ExecOnSignal) {
          sts =
              gdh_ObjidToName(list_ptr->objid, name, sizeof(name), cdh_mNName);
          if (EVEN(sts))
            LogAndExit(sts);
          sts = tlog_loglist_add(
              &filectx, list_ptr->objid, tlog_logtype_ExecOn,
              "Execution all PlcPgms set on", NULL, name,
          ...
          if (EVEN(sts))
            LogAndExit(sts);
          ((pwr_sClass_TLogExecAllOn *)(list_ptr->object_ptr))->ExecOnSignal =
              0;
      ...
      list_ptr = ExecOff_list;
      for (i = 0; i < ExecOff_count; i++) {
        if (((pwr_sClass_TLogExecAllOff *)(list_ptr->object_ptr))
                ->ExecOffSignal) {
          sts =
              gdh_ObjidToName(list_ptr->objid, name, sizeof(name), cdh_mNName);
          if (EVEN(sts))
            LogAndExit(sts);
          sts = tlog_loglist_add(
              &filectx, list_ptr->objid, tlog_logtype_ExecOff,
              "Execution all PlcPgms set off", NULL, name,
          ...
          if (EVEN(sts))
            LogAndExit(sts);
          ((pwr_sClass_TLogExecAllOff *)(list_ptr->object_ptr))->ExecOffSignal =
              0;
      ...
      for (i = 0; i < DtLog_count; i++) {
        if (((pwr_sClass_DtLog *)(list_ptr->object_ptr))->UpSignal) {
          if (((pwr_sClass_DtLog *)(list_ptr->object_ptr))->UpText[0] != 0) {
            sts = tlog_loglist_add(
                &filectx, list_ptr->objid, tlog_logtype_DtLogUp,
                ((pwr_sClass_DtLog *)(list_ptr->object_ptr))->UpText,
                &((pwr_sClass_DtLog *)(list_ptr->object_ptr))->Attribute, NULL,
        ...
        if (((pwr_sClass_DtLog *)(list_ptr->object_ptr))->DownSignal) {
          if (((pwr_sClass_DtLog *)(list_ptr->object_ptr))->DownText[0] != 0) {
            sts = tlog_loglist_add(
                &filectx, list_ptr->objid, tlog_logtype_DtLogDown,
      ...
      for (i = 0; i < AtLog_count; i++) {
        if (((pwr_sClass_AtLog *)(list_ptr->object_ptr))->UpSignal) {
          if (((pwr_sClass_AtLog *)(list_ptr->object_ptr))->UpText[0] != 0) {
            sts = tlog_loglist_add(
                &filectx, list_ptr->objid, tlog_logtype_AtLogUp,
                ((pwr_sClass_AtLog *)(list_ptr->object_ptr))->UpText,
                &((pwr_sClass_AtLog *)(list_ptr->object_ptr))->Attribute, NULL,
        ...
        if (((pwr_sClass_AtLog *)(list_ptr->object_ptr))->DownSignal) {
          if (((pwr_sClass_AtLog *)(list_ptr->object_ptr))->DownText[0] != 0) {
            sts = tlog_loglist_add(
                &filectx, list_ptr->objid, tlog_logtype_AtLogDown,
      ...
      for (i = 0; i < CAtLog_count; i++) {
        if (((pwr_sClass_CAtLog *)(list_ptr->object_ptr))->LogSignal) {
          ...
          strcat(text, ((pwr_sClass_CAtLog *)(list_ptr->object_ptr))->LogText);
          sts = tlog_loglist_add(
              &filectx, list_ptr->objid, tlog_logtype_CAtLog, text,
              &((pwr_sClass_CAtLog *)(list_ptr->object_ptr))->Attribute, NULL,
      ...
      for (i = 0; i < CDtLog_count; i++) {
        if (((pwr_sClass_CDtLog *)(list_ptr->object_ptr))->LogSignal) {
          ...
          strcat(text, ((pwr_sClass_CDtLog *)(list_ptr->object_ptr))->LogText);
          sts = tlog_loglist_add(
              &filectx, list_ptr->objid, tlog_logtype_CDtLog, text,
              &((pwr_sClass_CDtLog *)(list_ptr->object_ptr))->Attribute, NULL,
      ...
      for (i = 0; i < COtLog_count; i++) {
        if (((pwr_sClass_COtLog *)(list_ptr->object_ptr))->LogSignal) {
          sts = gdh_ObjidToName(
              ((pwr_sClass_COtLog *)(list_ptr->object_ptr))->LogValue, text,
              sizeof(text), cdh_mNName);
          if (EVEN(sts)) {
            strcpy(text, "Undefined Objid");
          }
          strcat(text, " ");
          strcat(text, ((pwr_sClass_COtLog *)(list_ptr->object_ptr))->LogText);
          sts = tlog_loglist_add(
              &filectx, list_ptr->objid, tlog_logtype_COtLog, text,
              &((pwr_sClass_COtLog *)(list_ptr->object_ptr))->Attribute, NULL,
      ...
      for (i = 0; i < ChgAtLog_count; i++) {
        if (((pwr_sClass_ChgAtLog *)(list_ptr->object_ptr))->LogSignal) {
          ...
          sts = tlog_loglist_add(
              &filectx, list_ptr->objid, tlog_logtype_ChgAtLog, text,
              &((pwr_sClass_ChgAtLog *)(list_ptr->object_ptr))->Attribute, NULL,
      ...
      for (i = 0; i < ChgOtLog_count; i++) {
        if (((pwr_sClass_ChgOtLog *)(list_ptr->object_ptr))->LogSignal) {
          sts = gdh_ObjidToName(
              ((pwr_sClass_ChgOtLog *)(list_ptr->object_ptr))->LogValue, text,
              sizeof(text), cdh_mNName);
          if (EVEN(sts)) {
            strcpy(text, "Undefined Objid");
          }
          ...
          sts = tlog_loglist_add(
              &filectx, list_ptr->objid, tlog_logtype_ChgOtLog, text,
              &((pwr_sClass_ChgOtLog *)(list_ptr->object_ptr))->Attribute, NULL,
      ...
      if (filectx.outfile != 0) {
        ...
        if (filectx.object_ptr->CloseSignal) {
          ...
          sts = tlog_loglist_add(&filectx, filectx.objid, tlog_logtype_Close,
                                 "File closed", NULL, "",
                                 &filectx.object_ptr->CloseTime, &log_list,
                                 &log_count, &log_alloc);
          if (EVEN(sts))
            LogAndExit(sts);
        ...
        sts = tlog_loglist_print(&filectx, log_list, &log_count, UserObject);
        if (EVEN(sts))
          LogAndExit(sts);
      }
      tlog_sleep(scantime);
