...
void ra_cdhtest::ObjidCompare(void) {
  sObjidCompare d[] = {
      {pwr_cNOid, pwr_cNOid, 0},    {{11111, 1}, {11111, 1}, 0},
      {{11112, 1}, {11111, 1}, 1},  {{11111, 1}, {11112, 1}, -1},
      {{11112, 2}, {11111, 1}, 1},  {{11111, 2}, {11112, 1}, 1},
...
void ra_cdhtest::ObjidIsEqual(void) {
  sObjidCompare d[] = {
      {pwr_cNOid, pwr_cNOid, 1},   {{11111, 1}, {11111, 1}, 1},
      {{11112, 1}, {11111, 1}, 0}, {{11111, 1}, {11112, 1}, 0},
      {{11112, 2}, {11111, 1}, 0}, {{11111, 2}, {11112, 1}, 0},
...
void ra_cdhtest::ObjidIsNull(void) {
  sObjidIsNull d[] = {{pwr_cNOid, 1}, {{11111, 1}, 0}, {{11112, 1}, 0},
...
void ra_cdhtest::ObjidIsNotNull(void) {
  sObjidIsNull d[] = {{pwr_cNOid, 0}, {{11111, 1}, 1}, {{11112, 1}, 1},
...
void ra_cdhtest::TypeObjidToId(void) {
  sTypeObjidToId d[] = {{"pwrs:Type-$Boolean", pwr_eType_Boolean},
...
void ra_cdhtest::TypeIdToObjid(void) {
  sTypeIdToObjid d[] = {{pwr_eType_Boolean, "pwrs:Type-$Boolean"},
...
void ra_cdhtest::TypeIdToIndex(void) {
  sTypeIdToIndex d[] = {{pwr_eType_Boolean, pwr_eTix_Boolean},
...
void ra_cdhtest::StringToClassId(void) {
  sStringToClassId d[] = {
      ...
      {"_C255.255:444", ((unsigned int)255 << 24) + (255 << 16) + (444 << 3),
       CDH__SUCCESS},
...
void ra_cdhtest::StringToObjid(void) {
  sStringToObjid d[] = {
      ...
      {"_o0.1.1.1:1", {1, (0 << 24) + (1 << 16) + (1 << 8) + 1}, CDH__SUCCESS},
      {"200.44.55.66:77777",
       {77777, ((unsigned int)200 << 24) + (44 << 16) + (55 << 8) + 66},
       CDH__SUCCESS},
      {"_O200.44.55.66:77777",
       {77777, ((unsigned int)200 << 24) + (44 << 16) + (55 << 8) + 66},
       CDH__SUCCESS},
  ...
  sStringToAref d[] = {
      ...
      {"0.1.1.1:1(_T3.7:0.3.1)[120.4]",
       {{1, (0 << 24) + (1 << 16) + (1 << 8) + 1},
        ...
        0},
       CDH__SUCCESS},
      {"0.1.1.1:1(_T3.7:1.1.1)[120.4]",
       {{1, (0 << 24) + (1 << 16) + (1 << 8) + 1},
        (3 << 24) + (7 << 16) + (1 << 15) + (1 << 11) + 1,
        120,
        4,
        0},
       CDH__SUCCESS},
      ...
      {"0.1.1.1:1(_T3.7:1.1.3)[120.4]",
       {{1, (0 << 24) + (1 << 16) + (1 << 8) + 1},
        (3 << 24) + (7 << 16) + (1 << 15) + (1 << 11) + 3,
        120,
        4,
        0},
       CDH__SUCCESS}};
...
void ra_cdhtest::StringToTypeId(void) {
  ...
  sStringToTypeId d[] = {
      ...
      {"_T3.7:0.4096.1", 0, CDH__INVTID},
      {"_T3.7:1.1.1", (3 << 24) + (7 << 16) + (1 << 15) + (1 << 11) + 1,
       CDH__SUCCESS},
      {"_T3.7:1.3.1", 0, CDH__INVTID},
      {"_T3.7:1.1.2048", 0, CDH__INVTID},
      {"_T3.7:1.1.3", (3 << 24) + (7 << 16) + (1 << 15) + (1 << 11) + 3,
       CDH__SUCCESS}};
  pwr_tTypeId type;
...
void ra_cdhtest::StringToObjectIx(void) {
  sStringToObjectIx d[] = {{"_X1", 1, CDH__SUCCESS},
                           {"_x1", 1, CDH__SUCCESS},
                           {"1", 1, CDH__SUCCESS},
...
void ra_cdhtest::ClassIdToString(void) {
  sClassIdToString d[] = {
      ...
      {((unsigned int)255 << 24) + (255 << 16) + (444 << 3), 1,
       "_C255.255:444"}};
  char str[40];
...
void ra_cdhtest::ObjectIxToString(void) {
  sObjectIxToString d[] = {
      {1, 1, "_X1"}, {1, 0, "1"}, {123456789, 1, "_X123456789"}};
  char str[40];
...
void ra_cdhtest::ArefToString(void) {
  sArefToString d[] = {{{{1, (0 << 24) + (1 << 16) + (1 << 8) + 1},
                         (3 << 24) + (7 << 16) + (1 << 3) + (1),
                         120,
                         4,
                         0},
...
void ra_cdhtest::OidToFnString(void) {
  sOidToString d[] = {
      ...
      {{1, (0 << 24) + (1 << 16) + (1 << 8) + 1},
       0,
       "000_001_001_001_00000001"},
      {{0x77777, ((unsigned int)200 << 24) + (44 << 16) + (55 << 8) + 66},
       0,
       "200_044_055_066_00077777"}};
  char str[40];
...
void ra_cdhtest::Low(void) {
  sLow d[] = {{"AbCdEfGhIjKl", "abcdefghijkl"},
...
void ra_cdhtest::OpSysToStr(void) {
  sOpSysToStr d[] = {{pwr_mOpSys_X86_LINUX, "x86_linux"},
...
void ra_cdhtest::OpSysToDirStr(void) {
  sOpSysToStr d[] = {{pwr_mOpSys_X86_LINUX, "os_linux/hw_x86"},
...
void ra_cdhtest::CutNameSegments(void) {
  ...
  for (unsigned int i = 0; i < sizeof(d) / sizeof(d[0]); i++) {
    ...
    if (strcmp(outname, d[i].outname) != 0) {
      m_log->vlog('E', "CutNameSegments, %s != %s %d", outname, d[i].outname,
                  i);
      return;
    }
  }
...
void ra_cdhtest::AttrSize(void) {
  sAttrSize d[] = {{{"", pwr_eType_Int32, 0, 4, 0, 1, 1}, 4},
                   {{"", pwr_eType_Int32, 0, 4, PWR_MASK_POINTER, 1, 1}, 8},
                   {{"", pwr_eType_Int32, 0, 4, PWR_MASK_POINTER, 100, 1}, 800},
                   {{"", pwr_eType_Int32, 0, 400,
                     PWR_MASK_POINTER | PWR_MASK_PRIVATE, 100, 1},
                    400}};
  pwr_tUInt32 size;
...
// Destructor
ra_cdhtest::~ra_cdhtest() { delete m_log; }
