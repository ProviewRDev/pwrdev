...
pwr_tStatus ra_mhtest::mh_ack_bc(mh_sAck* mp)
{
  ...
  if ((sup->estatus & mh_mEventStatus_NotRet) == 0 && (sup->estatus & mh_mEventStatus_NotAck) == 0)
  {
    sup->active = 0;
    sup->eactive = 0;
    sup->estatus = 0;
...
pwr_tStatus ra_mhtest::mh_alarm_bc(mh_sMessage* mp)
{
  ...
  if (!sup->eactive)
  {
    m_mh->m_errcnt++;
    if (plog)
      m_mh->m_log->vlog('X', "alarm_bc sup not active, %d %s", sup->eventid.Idx, mp->EventText);
    return 1;
  }
  ...
  if (sup->status != sup->estatus)
  {
    m_mh->m_errcnt++;
    if (plog)
      m_mh->m_log->vlog('X', "alarm_bc status differs %d %d %d", sup->status, sup->estatus, sup->eventid.Idx);
    return 1;
  }
...
pwr_tStatus ra_mhtest::mh_block_bc(mh_sBlock* mp) { return 1; }

pwr_tStatus ra_mhtest::mh_cancel_bc(mh_sReturn* mp) { return 1; }
...
pwr_tStatus ra_mhtest::mh_info_bc(mh_sMessage* mp)
{
  ...
  if (!sup->eactive)
  {
    m_mh->m_errcnt++;
    if (plog)
      m_mh->m_log->vlog('X', "info_bc sup not active, %d %s", sup->eventid.Idx, mp->EventText);
    return 1;
  }
  sup->status = mp->Status & 0x7;
  if (sup->status != sup->estatus)
  {
    m_mh->m_errcnt++;
    if (plog)
      m_mh->m_log->vlog('X', "info_bc status differs %d %d %d", sup->status, sup->estatus, sup->eventid.Idx);
    return 1;
  }
  memcpy(&sup->eventid, &mp->Info.Id, sizeof(mh_sEventId));
  return 1;
...
pwr_tStatus ra_mhtest::mh_clear_alarmlist_bc(pwr_tNodeIndex nix) { return 1; }

pwr_tStatus ra_mhtest::mh_clear_blocklist_bc(pwr_tNodeIndex nix) { return 1; }
  ...
  for (unsigned int i = 0; i < mp->Count; i++)
  {
    ...
    else
    {
      ...
      if (sup->status != (mp->Sts[i].Status & 7))
      {
        m_mh->m_errcnt++;
        if (plog)
          m_mh->m_log->vlog('X', "alarmstatus_bc status differ %d %d", mp->Sts[i].Status, sup->status);
      }
    }
  }
  for (int i = 0; i < SUP_ASIZE + SUP_ISIZE; i++)
...

{
  ...
  for (int i = 0; i < SUP_ASIZE + SUP_ISIZE; i++)
  {
    ...
    for (int j = 0; j < 32; j++)
    {
      ...
      if (ODD(m_sts))
        m_sts = gdh_DLRefObjectInfoAttrref(&aref, (void**)&m_sup[i][j].dp, &dlid);
      if (EVEN(m_sts))
      {
...
// Destructor
ra_mhtest::~ra_mhtest() { delete m_log; }
