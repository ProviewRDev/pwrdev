...
{
  sAidCompare d[] = {{{0, 0}, {0, 0}, 0},          {{11111, 1}, {11111, 1}, 0}, {{11112, 1}, {11111, 1}, 1},
                     {{11111, 1}, {11112, 1}, -1}, {{11112, 2}, {11111, 1}, 1}, {{11111, 2}, {11112, 1}, 1},
                     {{11112, 1}, {11111, 2}, -1}, {{11111, 1}, {11112, 2}, -1}};
  int sts;
...
{
  sAidCompare d[] = {{{0, 0}, {0, 0}, 1},         {{11111, 1}, {11111, 1}, 1}, {{11112, 1}, {11111, 1}, 0},
                     {{11111, 1}, {11112, 1}, 0}, {{11112, 2}, {11111, 1}, 0}, {{11111, 2}, {11112, 1}, 0},
                     {{11112, 1}, {11111, 2}, 0}, {{11111, 1}, {11112, 2}, 0}};
  int sts;
...
{
  sAidIsNull d[] = {{{0, 0}, 1}, {{11111, 1}, 0}, {{11112, 1}, 0},
                    {{0, 1}, 0}, {{11112, 0}, 0}, {{11111, 0}, 0}};
  int sts;
...
{
  sAidIsNull d[] = {{{0, 0}, 0}, {{11111, 1}, 1}, {{11112, 1}, 1},
                    {{0, 1}, 1}, {{11112, 0}, 1}, {{11111, 0}, 1}};
  int sts;
...
{
  sQidCompare d[] = {{{0, 0}, {0, 0}, 0},          {{11111, 1}, {11111, 1}, 0}, {{11112, 1}, {11111, 1}, 1},
                     {{11111, 1}, {11112, 1}, -1}, {{11112, 2}, {11111, 1}, 1}, {{11111, 2}, {11112, 1}, 1},
                     {{11112, 1}, {11111, 2}, -1}, {{11111, 1}, {11112, 2}, -1}};
  int sts;
...
{
  sQidCompare d[] = {{{0, 0}, {0, 0}, 1},         {{11111, 1}, {11111, 1}, 1}, {{11112, 1}, {11111, 1}, 0},
                     {{11111, 1}, {11112, 1}, 0}, {{11112, 2}, {11111, 1}, 0}, {{11111, 2}, {11112, 1}, 0},
                     {{11112, 1}, {11111, 2}, 0}, {{11111, 1}, {11112, 2}, 0}};
  int sts;
...
{
  sQidIsNull d[] = {{{0, 0}, 1}, {{11111, 1}, 0}, {{11112, 1}, 0},
                    {{0, 1}, 0}, {{11112, 0}, 0}, {{11111, 0}, 0}};
  int sts;
...
{
  sQidIsNull d[] = {{{0, 0}, 0}, {{11111, 1}, 1}, {{11112, 1}, 1},
                    {{0, 1}, 1}, {{11112, 0}, 1}, {{11111, 0}, 1}};
  int sts;
...
{
  sAlloc d[] = {{0, QCOM__SUCCESS, QCOM__SUCCESS},       {4, QCOM__SUCCESS, QCOM__SUCCESS},
                {400, QCOM__SUCCESS, QCOM__SUCCESS},     {40000, QCOM__SUCCESS, QCOM__SUCCESS},
                {4000000, QCOM__SUCCESS, QCOM__SUCCESS}, {40000000, QCOM__SUCCESS, QCOM__SUCCESS}};
...
{
  sCreateQ d[] = {{{0, 0}, "queue1", qcom_eQtype_private, 100, QCOM__SUCCESS, 1, HASH__SUCCESS, 1},
...
{
  sPut d[] = {{"Some message 1", (qcom_eBtype)2001, (qcom_eStype)1, QCOM__SUCCESS},
  ...
  for (unsigned int i = 0; i < sizeof(d) / sizeof(d[0]); i++)
  {
    ...
    if (ODD(m_sts))
    {
      ...
      if (strcmp((char*)put.data, (char*)get.data) != 0 || get.type.b != d[i].btype ||
          get.type.s != d[i].stype || get.size != strlen(d[i].msg) + 1)
      {
  ...
  for (unsigned int i = 0; i < 100; i++)
  {
    ...
    if (strcmp((char*)d[0].msg, (char*)get.data) != 0 || get.type.b != d[0].btype ||
        get.type.s != d[0].stype || get.size != strlen(d[0].msg) + 1)
    {
      m_log->vlog('E', "Put, content differs, %d", i);
      return;
    }
...
{
  sGet d[] = {{"Some message 1", (qcom_eBtype)2001, (qcom_eStype)1, 0, qcom_cTmoEternal, 1, QCOM__SUCCESS},
...
{
  sForwarding d[] = {{"Some message 1", (qcom_eBtype)2001, (qcom_eStype)1},
  ...
  for (unsigned int i = 0; i < sizeof(d) / sizeof(d[0]); i++)
  {
    ...
    if (strcmp((char*)d[i].msg, (char*)get.data) != 0 || get.type.b != d[i].btype ||
        get.type.s != d[i].stype || get.size != strlen(d[i].msg) + 1)
    {
      m_log->vlog('E', "Forwarding, content differs, %d", i);
      return;
...
static void* signal_listen(void* arg)
{
  ...
  for (unsigned int i = 0; i < sizeof(d->d) / sizeof(d->d[0]); i++)
  {
    ...
    if (d->d[i].wait_and)
    {
      qcom_WaitAnd(&sts, &d->msg_qid, &d->event_qid, d->d[i].mask, qcom_cTmoEternal);
    }
    else
    {
      qcom_WaitOr(&sts, &d->msg_qid, &d->event_qid, d->d[i].mask, qcom_cTmoEternal, &event);

      if (plog)
        d->qcomtest->log()->vlog('D', "Thread awake %d %d", i, event);
      if (event != d->d[i].event)
      {
        d->qcomtest->log()->vlog('X', "Signalling, event differs (%d != %d), %d", event, d->d[i].event, i);
        d->errcnt++;
      }
...
void ra_qcomtest::Signalling(void)
{
  ...
  for (unsigned int i = 0; i < sizeof(d.d) / sizeof(d.d[0]); i++)
  {
    ...
    {
      m_log->vlog('E', "Signalling, index mismatch (%d != %d)", d.thread_awake, i + 1);
      return;
    }
    time_Sleep(0.1);
  }
  ...
  for (unsigned int i = 0; i < sizeof(d->d) / sizeof(d->d[0]); i++)
  {
    ...
    else
    {
      ...
      case qcom_eBtype_event:
        ...
        {
          d->qcomtest->log()->vlog('X', "BindEvent, event differs %d != %d", ep->mask, d->d[i].event);
          d->errcnt++;
        }
        break;
      default:
...
void ra_qcomtest::BindEvent(void)
{
  ...
  for (unsigned int i = 0; i < sizeof(d.d) / sizeof(d.d[0]); i++)
  {
    ...
    if (d.thread_awake != i + 1)
    {
      m_log->vlog('X', "BindEvent, index mismatch (%d != %d)", d.thread_awake, i + 1);
      d.errcnt++;
    }
    time_Sleep(0.1);
  }
...
void ra_qcomtest::Request(void)
{
  ...
  for (unsigned int i = 0; i < sizeof(d.d) / sizeof(d.d[0]); i++)
  {
    ...
    qcom_Request(&m_sts, &d.request_qid, &put, &d.reply_qid, &get, 100000, 0);
    if (EVEN(m_sts))
    {
    ...
    if (d.thread_awake != i + 1)
    {
      m_log->vlog('E', "Request, index mismatch (%d != %d)", d.thread_awake, i + 1);
      return;
    }
    time_Sleep(0.1);
  }
...
// Destructor
ra_qcomtest::~ra_qcomtest() { delete m_log; }
