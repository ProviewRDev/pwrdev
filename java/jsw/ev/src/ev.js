...
function Ev() {
  ...
  this.init = function () {
    ...
    this.ctx.gdraw.canvas.addEventListener("click", function (event) {
      ...
      var x = event.pageX;
      if (event.shiftKey) ev.ctx.event_handler(Plow.eEvent_MB1ClickShift, x, y);
      else ev.ctx.event_handler(Plow.eEvent_MB1Click, x, y);
    });
    ...
    document
      ...
      .addEventListener("click", function (event) {
        ...
        console.log(
          "toolitem2",
          o.userdata.e.supObject.vid,
          o.userdata.e.supObject.oix,
        );
        self.ctx.gdh.getObjectFromAref(
          o.userdata.e.supObject,
          GdhOp.GET_OP_SELF,
          self.open_navigator_cb,
          null,
        );
    ...
    document
      ...
      .addEventListener("click", function (event) {
        ...
        if (o.userdata instanceof EvItemAlarm) {
          var newwindow = window.open("", "_blank");
          self.ctx.gdh.getObjectFromName(
            o.userdata.e.eventName,
    ...
    // Help event name
    document
      .getElementById("toolitem11")
      .addEventListener("click", function (event) {
        console.log("toolitem11 event");
      });
    // Class help event name
    document
      .getElementById("toolitem12")
      .addEventListener("click", function (event) {
          ...
          case MhC.eEvent_Ack:
            ...
            if (item !== null) {
              if (item.e.eventStatus & MhC.mEventStatus_NotRet) {
                ...
                item.update();
              } else self.ctx.remove(item.node);
            }
            break;
  ...
  this.find_event = function (event_id) {
    for (
      var o = this.ctx.get_first_object();
      o !== null;
      o = this.ctx.get_next_object(o)
    ) {
      var item = o.get_userdata();
      if (
        item.e.eventId.nix == event_id.nix &&
  ...
  this.ack = function () {
    ...
    if (item.e.eventStatus & MhC.mEventStatus_NotRet) {
      item.e.eventStatus &= ~MhC.mEventStatus_NotAck;
      item.update();
    } else this.ctx.remove(item.node);
    this.ctx.configure();
    this.ctx.draw();
  };
  ...
  this.open_navigator_cb = function (id, data, sts, result) {
    ...
    if ((sts & 1) != 0) {
      ...
      console.log("storage", localStorage.getItem("XttMethodNavigator"));
    } else console.log("Error status " + sts);
  };
  ...
  this.plow_event = function (event, object, x, y) {
    ...
    switch (event) {
      ...
      case Plow.eEvent_MB1Click:
        if (
          item != null &&
          item.e.eventMoreText !== "" &&
          x >= object.measure().ll_x + 70 &&
          x <= object.measure().ll_x + 90
        ) {
  ...
  this.get_type = function () {
    ...
    if (query === "") return EvC.eType_AlarmList;

    var vars = query.split("&");
    var typestr = vars[0].substring(5);
    if (typestr == "event") type = EvC.eType_EventList;
    else if (typestr == "block") type = EvC.eType_BlockList;
    else type = EvC.eType_AlarmList;
...
function EvItemAlarm(ev, e, destination, destCode) {
  ...
  this.update = function () {
    if (this.e.eventStatus & MhC.mEventStatus_NotRet)
      this.node.set_annotation_pixmap(1, Bitmaps.alarm);
    else this.node.set_annotation_pixmap(1, null);

    if (this.e.eventStatus & MhC.mEventStatus_NotAck)
      this.node.set_annotation_pixmap(0, Bitmaps.ack);
    else this.node.set_annotation_pixmap(0, null);
  };
}
