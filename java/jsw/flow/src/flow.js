  ...
  this.search_by_name = function (name) {
    ...
    for (var i = 0; i < this.a.length; i++) {
      if (this.a[i] instanceof FlowNode) {
        ...
        if (this.a[i].n_name.toUpperCase() == uname) return this.a[i];
      }
    }
    return null;
  };
...
function FlowLine(ctx) {
  ...
  this.draw = function (g, p, node, highlight) {
    ...
    if (g.lineWidth < 1) g.lineWidth = 1;
    ...
    if (highlight) g.strokeStyle = "red";
    g.beginPath();
    g.moveTo(x1, y1);
    g.lineTo(x2, y2);
    g.stroke();
...
function FlowRect(ctx) {
  ...
  this.draw = function (g, p, node, highlight) {
    if ((this.display_level & this.ctx.display_level) == 0) return;
...
function FlowArc(ctx) {
  ...
  this.draw = function (g, p, node, highlight) {
    ...
    else var a1 = ((this.angel1 - 90) / 180) * Math.PI;
...
function FlowText(ctx) {
  ...
  this.draw = function (g, p0, node, highlight) {
    var tsize = 0;
    var idx =
      (this.ctx.zoom_factor / this.ctx.base_zoom_factor) *
        (this.text_size + 4) -
      4;
    if (idx < 0) return;
...
function FlowAnnot(ctx) {
  ...
  this.draw = function (g, p0, node, highlight) {
    if (node == null) return;
    if (node.annotv[this.number] == null) return;
    if ((this.display_level & this.ctx.display_level) == 0) return;
    ...
    for (var i = row; i < lines.length; i++) {
      ...
      switch (key) {
        ...
        case Flow.eSave_Con_cc:
          ...
          if (!found) console.log("FlowCon: ConClass not found: " + cc_name);
          break;
        case Flow.eSave_Con_c_name:
          if (tokens[1] != null) this.c_name = tokens[1];
          break;
        ...
        case Flow.eSave_Con_trace_object:
          if (tokens[1] != null) this.trace_object = tokens[1];
          break;
        case Flow.eSave_Con_trace_attribute:
          if (tokens[1] != null) this.trace_attribute = tokens[1];
          break;
...
function FlowNode(ctx) {
  ...
  this.draw = function (g, p, node, highlight) {
    ...
    if (this.nc.group != Flow.eNodeGroup_Document) {
      ...
      g.fillStyle = "white";
      if (this.select) g.fillStyle = "lightblue";
    ...
    for (var i = row; i < lines.length; i++) {
      ...
      switch (key) {
        case Flow.eSave_Node_nc:
          ...
          if (!found) console.log("FlowNode: NodeClass not found: " + nc_name);
          break;
        case Flow.eSave_Node_n_name:
          if (tokens[1] != null) this.n_name = tokens[1];
          break;
  ...
  this.connect = function () {
    if (this.trace_object == "" || this.trace_attribute == "") return;
    if (this.trace_attr_type == Flow.eTraceType_User) return;

    var n = this.trace_attribute.indexOf("#");
    if (n != -1) this.trace_attribute = this.trace_attribute.substring(0, n);
  ...
  this.scan = function () {
    ...
    var evaluate = true;
    if (this.first_scan) this.first_scan = false;
    else if (v1 == this.old_value) return;

    if (v1) this.highlight = true;
  ...
  this.set_select = function (select) {
    if (select != this.select) {
      this.select = select;
      this.redraw = true;
      if (this.select) this.ctx.select_object = this;
      else this.ctx.select_object = null;
    }
  };
}
...
function FlowCtx() {
  ...
  this.connect = function () {
    ...
    for (var i = 0; i < this.a.size(); i++) this.a.get(i).connect();
  };
  this.scan = function () {
    for (var i = 0; i < this.a.size(); i++) this.a.get(i).scan();
  };
  ...
  this.set_select = function (select) {
    for (var i = 0; i < this.a.size(); i++) {
      if (this.a.get(i) instanceof FlowNode) this.a.get(i).set_select(select);
    }
  };
  ...
  this.search_object = function (name) {
    ...
    if (node != null) console.log("Found", name);
    return node;
  };
  this.center_object = function (o) {
    console.log(
      "center_object",
      o.pos.x * this.zoom_factor + this.offset_x,
      window.innerWidth,
      o.pos.x * this.zoom_factor + this.offset_x - window.innerWidth / 2,
    );
    console.log(
      "center_object",
      o.pos.y * this.zoom_factor + this.offset_y,
      window.innerHeight,
      o.pos.y * this.zoom_factor + this.offset_y - window.innerHeight / 2,
    );
    window.scrollTo(
      o.pos.x * this.zoom_factor + this.offset_x - window.innerWidth / 2,
      o.pos.y * this.zoom_factor +
        this.offset_y -
...
function FlowFrame() {
  ...
  this.read_func = function (lines, row) {
    ...
    self.ctx.gdraw.gctx.translate(
      -self.ctx.x_left * self.ctx.zoom_factor,
      -self.ctx.y_low * self.ctx.zoom_factor,
    );
  ...
  this.flow_close = function () {
    ...
    for (var i in self.ctx.gdh.pending) delete self.ctx.gdh.pending[i];
  };
  ...
  this.get_filename = function () {
    ...
    for (var i = 0; i < 4; i++) {
      ...
      for (var j = 0; j < 3; j++) {
        if (svid[i].length < 3) svid[i] = "0" + svid[i];
      }
    }
    var soix = oix.toString(16);
    for (var j = 0; j < 8; j++) {
      if (soix.length < 8) soix = "0" + soix;
    }
