...
function Xtt() {
  ...
  this.init = function () {
    ...
    this.ctx.gdraw.canvas.addEventListener("click", function (event) {
      ...
      else xtt.ctx.event_handler(Plow.eEvent_MB1Click, x, y);
    });
    document.addEventListener("keydown", function (event) {
      if (event.keyCode == 40) {
        ...
        if (event.shiftKey) self.ctx.event_handler(Plow.eEvent_Key_ShiftRight);
        else self.ctx.event_handler(Plow.eEvent_Key_Right);
        event.preventDefault();
        ...
        if (event.ctrlKey) self.ctx.event_handler(Plow.eEvent_Key_CtrlR);
        event.preventDefault();
      } else if (event.keyCode == 76) {
        if (event.ctrlKey) self.ctx.event_handler(Plow.eEvent_Key_CtrlL);
        event.preventDefault();
      } else if (event.keyCode == 71) {
        if (event.ctrlKey) self.ctx.event_handler(Plow.eEvent_Key_CtrlG);
        event.preventDefault();
      }
    });
    document
      .getElementById("toolitem1")
      .addEventListener("click", function (event) {
        var o = self.ctx.get_select();
        if (o.userdata instanceof XttItemObject) {
          var newwindow = window.open("", "_blank");
          self.ctx.gdh.getObject(
            o.userdata.objid,
            GdhOp.GET_OP_METHOD_GRAPH,
            self.open_graph_cb,
            newwindow,
          );
        }
    ...
    document
      ...
      .addEventListener("click", function (event) {
        ...
        if (o.userdata instanceof XttItemObject) {
          ...
          console.log(
            "flow.html?vid=" +
              o.userdata.objid.vid +
              "&oix=" +
              o.userdata.objid.oix +
              ostring,
          );
          window.open(
            "flow.html?vid=" +
    ...
    document
      .getElementById("toolitem4")
      .addEventListener("click", function (event) {
        console.log("toolitem4 event");
      });
    document
      .getElementById("toolitem5")
      .addEventListener("click", function (event) {
    ...
    document
      .getElementById("toolitem7")
      .addEventListener("click", function (event) {
        console.log("toolitem7 event");
      });
    document
      .getElementById("toolitem8")
      .addEventListener("click", function (event) {
        ...
        if (o.userdata instanceof XttItemObject) {
          ...
          self.ctx.gdh.getObject(
            o.userdata.objid,
            GdhOp.GET_OP_METHOD_HELPCLASS,
            self.open_helpclass_cb,
            newwindow,
          );
        }
      });
  ...
  this.gdh_init_cb = function () {
    if (self.priv == null) self.ctx.gdh.login("", "", self.login_cb, self);

    var oid = new PwrtObjid(0, 0);
    self.ctx.gdh.getAllXttChildren(
      oid,
      self.open_children_cb,
      ...
      while (child != null) {
        if (child.userdata.name == data.open_next[0]) {
          if (data.open_next.length == 1) {
            ...
            if (!self.ctx.is_visible(child)) self.ctx.scroll(child.y_low, 0.5);
            ...
            } else child.userdata.open_children(self, data.open_next);
          }
          break;
        }
  ...
  this.open_attributes_cb = function (id, data, sts, result) {
    ...
    for (var i = 0; i < result.length; i++) {
      ...
      else new XttItemAttr(self, result[i], data.node, Plow.DEST_INTOLAST);
    }
  ...
  this.open_plc_cb = function (id, data, sts, result) {
    if ((sts & 1) != 0) {
      var param1;
      if (result.param1 !== "") param1 = "&obj=" + result.param1;
      else param1 = "";
      console.log(
        "flow.html?vid=" +
          result.objid.vid +
  ...
  this.open_graph_cb = function (id, data, sts, result) {
    if ((sts & 1) != 0) {
      ...
      if (idx != -1) result.param1 = result.param1.substring(0, idx);

      var instancestr = "";
      if (result.fullname !== "") instancestr = "&instance=" + result.fullname;
  ...
  this.plow_event = function (event, object, x, y) {
    var item = null;
    if (object != null) item = object.get_userdata();
      ...
      case Plow.eEvent_Key_Down: {
        ...
        if (o != null) {
          ...
          if (next != null) {
            ...
            if (!self.ctx.is_visible(next)) self.ctx.scroll(next.y_low, 0.1);
          }
        } else {
          o = self.ctx.a.a[0];
          o.set_select(true);
      ...
      case Plow.eEvent_Key_Up: {
        ...
        if (o != null) {
          ...
          if (next != null) {
            ...
            if (!self.ctx.is_visible(next)) self.ctx.scroll(next.y_low, 0.9);
          }
        }
  ...
  this.createNodeClasses = function () {
    ...
    var a1 = new PlowAnnot(
      this.ctx,
      3,
      0.9,
      4,
      Plow.COLOR_BLACK,
      Plow.NEXT_RELATIVE_POSITION,
      0,
    );
    var a11 = new PlowAnnot(
      this.ctx,
      7.5,
      0.9,
      4,
      Plow.COLOR_BLACK,
      Plow.RELATIVE_POSITION,
      1,
    );
    var a12 = new PlowAnnot(
      this.ctx,
      12,
      0.9,
      4,
  ...
  this.collapse = function () {
    ...
    for (var i = 0; i < this.ctx.a.size(); i++) {
      ...
      if (node.level == 0) node.userdata.close(this);
    }
  ...
  this.display_object = function (name) {
    ...
    if (i != -1) {
      ...
      var j;
      for (j = 0; j < p1.length; j++) path[j] = p1[j];
      path[p1.length] = ".";
      for (j = 0; j < p2.length; j++) path[j + p1.length + 1] = p2[j];
    ...
    for (var j = idx; j < this.ctx.a.size(); j++) {
      if (this.ctx.a.get(j).userdata.name == path[0]) {
        if (j == this.ctx.a.size() - 1) {
          ...
          if (!this.ctx.is_visible(node)) this.ctx.scroll(node.y_low, 0.5);
          window.focus(); // Doesn't work
        } else {
          path.splice(0, 1);
  ...
  this.trace_scan = function (id, sts) {
    ...
    if (self.scan_update) self.ctx.draw();
  ...
  this.openValueInputDialog = function (item, text) {
    console.log(
      "priv acc",
      this.priv,
      Pwr.mAccess_RtWrite | Pwr.mAccess_System,
    );
    if (this.is_authorized(Pwr.mAccess_RtWrite | Pwr.mAccess_System)) {
      var value = prompt(text, "");
      if (value !== null) item.set_value(this, value);
    } else alert("Not authorized for this operation");
  };
}
...
function XttItemObject(xtt, object_info, destination, destCode) {
  ...
  if (object_info.has_children) this.node.set_annotation_pixmap(0, Bitmaps.map);
  else this.node.set_annotation_pixmap(0, Bitmaps.leaf);

  this.open_children = function (xtt, open_next) {
    if (this.node.node_open != 0) this.close(xtt);
    else if (!this.has_children) this.open_attributes(xtt, null);
    else {
      xtt.ctx.gdh.getAllXttChildren(
        this.objid,
        xtt.open_children_cb,
  ...
  this.open_attributes = function (xtt, open_next) {
    if (this.node.node_open != 0) this.close(xtt);
      ...
      xtt.ctx.gdh.getAllClassAttributes(
        this.cid,
        this.objid,
        xtt.open_attributes_cb,
        new XttOpenChildrenData(this.node, open_next),
      );
...
function XttItemAttrArray(xtt, info, destination, destCode) {
  ...
  this.open_attributes = function (xtt, open_next) {
    ...
    for (var i = 0; i < this.elements; i++) {
      ...
      else new XttItemAttr(xtt, info, this.node, Plow.DEST_INTOLAST);
    }
    this.node.node_open |= Plow.OPEN_ATTRIBUTES;
    xtt.ctx.configure();
