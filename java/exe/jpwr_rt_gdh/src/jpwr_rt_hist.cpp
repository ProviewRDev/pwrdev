...
JNIEXPORT void JNICALL Java_jpwr_rt_Hist_initHistIDs(JNIEnv *env, jclass cls) {
  int sts;
...
JNIEXPORT jobjectArray JNICALL Java_jpwr_rt_Hist_getHistList(
    JNIEnv *env, jclass obj, jstring jstartTime, jstring jstopTime,
    jboolean jtypeInfo, jboolean jtypeInfoSuccess, jboolean jtypeAlarm,
    jboolean jtypeMaintenanceAlarm, jboolean jtypeSystemAlarm,
    jboolean jtypeUserAlarm1, jboolean jtypeUserAlarm2,
    jboolean jtypeUserAlarm3, jboolean jtypeUserAlarm4, jboolean jtypeReturn,
    jboolean jtypeAck, jboolean jprioA, jboolean jprioB, jboolean jprioC,
    jboolean jprioD, jstring jname, jstring jtext) {
#ifdef PWRE_CONF_LIBDB_CXX
  const char *str;
  char *cstr_minTime;
  char *cstr_maxTime;
  ...
  jobjectArr =
      (jobjectArray)env->NewObjectArray(nrOfEvents, mhrEventArrCls, NULL);
  ...
  for (i = 0; i < nrOfEvents; i++) {
    ...
    case mh_eEvent_InfoSuccess:
      env->SetObjectArrayElement(
          jobjectArr, i,
          convertAlarmOrInfoToMhrEvent((mh_sMessage *)(&(ev.Mess))));
      // printMess(sp, outFile);
      break;
    case mh_eEvent_Ack:
      env->SetObjectArrayElement(jobjectArr, i,
                                 convertAckToMhrEvent((mh_sAck *)(&(ev.Mess))));
      // printAck(sp, outFile);
      break;
    case mh_eEvent_Cancel:
    case mh_eEvent_Return:
      env->SetObjectArrayElement(
          jobjectArr, i, convertReturnToMhrEvent((mh_sReturn *)(&(ev.Mess))));
      // printRet(sp, outFile);
      break;
...
int check_conditions(sEvent *evp, HistQuery *query) {
  ...
  if (query->eventType_Ack || query->eventType_Alarm ||
      query->eventType_MaintenanceAlarm || query->eventType_SystemAlarm ||
      query->eventType_UserAlarm1 || query->eventType_UserAlarm2 ||
      query->eventType_UserAlarm3 || query->eventType_UserAlarm4 ||
      query->eventType_Info || query->eventType_InfoSuccess ||
      query->eventType_Return || query->eventType_Cancel ||
      query->eventType_Block || query->eventType_Unblock ||
      query->eventType_Reblock || query->eventType_CancelBlock) {
  ...
  if (query->eventPrio_A || query->eventPrio_B || query->eventPrio_C ||
      query->eventPrio_D) {
...
jobject convertAlarmOrInfoToMhrEvent(mh_sMessage *MsgP) {
  ...
  PwrtObjid_cid = env->GetMethodID(PwrtObjid_id, "<init>", "(II)V");

  oix = (jint)objid.oix;
  vid = (jint)objid.vid;
  objid_obj = env->NewObject(PwrtObjid_id, PwrtObjid_cid, oix, vid);

  time_AtoAscii(&time, time_eFormat_ComprDateAndTime, time_str,
                sizeof(time_str));
  time_AtoAscii(&birthTime, time_eFormat_ComprDateAndTime, birthTime_str,
                sizeof(birthTime_str));
  ...
  jmethodID MhrEvent_cid = env->GetMethodID(
      MhrEvent_id, "<init>",
      "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;IIIILjava/lang/"
      "String;IILjava/lang/String;IILjpwr/rt/PwrtObjid;)V");

  // anropa callback metoden i Mh-klassen
  jobject return_obj = env->NewObject(
      MhrEvent_id, MhrEvent_cid, jevText, jevName, jevTime, jevFlags, jevPrio,
      jevStatus, jevNix, jevBirthTime, jevIdx, jevTargetNix, jevTargetBirthTime,
      jevTargetIdx, jevType, objid_obj);
...
jobject convertReturnToMhrEvent(mh_sReturn *MsgP) {
  ...
  time_AtoAscii(&birthTime, time_eFormat_ComprDateAndTime, birthTime_str,
                sizeof(birthTime_str));
  time_AtoAscii(&targetBirthTime, time_eFormat_ComprDateAndTime,
                targetBirthTime_str, sizeof(targetBirthTime_str));
