...
export function pwrGenerateExpose(definition) {
    return `
## Exposes
...
function getExposeDocs(expose, definition) {
    ...
    //console.log(expose);
    console.log("/**");
    console.log("@brief", expose.description);
    for (let i = 0; i < lines.length; i++)
      console.log(lines[i]);
    console.log("*/");
    console.log("Name", expose.name);
    console.log("Description", expose.description);
    console.log("Label", expose.label);
    console.log("Type", expose.type);
    console.log("Access", expose.access);
    console.log("Property", expose.property);
    if (expose.endpoint != null)
      console.log("Endpoint", expose.endpoint);
    if (expose.features != null) {
      if (expose.property != 'color_rgb' && expose.property != 'color_xy' && expose.property != 'color' && expose.property != 'simulated_brightness') {
        console.log("Features **Warning**");
      }
      process.stdout.write("Features ");
      for (let j = 0; j < expose.features.length; j++) {
        process.stdout.write(expose.features[j].name);
        if (j != expose.features.length - 1)
          process.stdout.write(",");
      }
      console.log('');
      //console.log("Features", expose.features);
    }
    if (expose.category != null)
      console.log("Category", expose.category);
    if (expose.unit != null)
      console.log("Unit", expose.unit);
    //if (expose.presets != null)
    //  console.log("Presets", expose.presets);

    if (expose.type === 'numeric') {
      if (expose.value_min != null) 
        console.log("MinValue", expose.value_min);
      if (expose.value_max != null) 
        console.log("MaxValue", expose.value_max);
      if (expose.value_step != null) 
        console.log("StepValue", expose.value_step);
    } else if (expose.type === 'binary') {
      console.log("OnValue", expose.value_on);
      console.log("OffValue", expose.value_off);
      if (expose.value_toggle != null)
        console.log("ToggleValue", expose.value_toggle);
    } else if (expose.type === 'enum') {
      let txt = `${expose.values.map((v) => `${v}`).join(',')}`;
      console.log("EnumValues", txt);
    } else if (expose.type === 'list') {
      let txt = `${expose.values}`;
      console.log("ListValues", txt);
    } else if (expose.type === 'text') {
    }

    if (expose.features != null) {
      console.log("**Features");
      for (let i = 0; i < expose.features.length; i++) {
        console.log("**Expose")
        //console.log(expose.features[i]);
        console.log("/**");
        console.log("@brief", expose.features[i].description);
        for (let i = 0; i < lines.length; i++)
          console.log(lines[i]);
        console.log("*/");
        console.log("Name", expose.features[i].name);
        console.log("Description", expose.features[i].description);
        console.log("Label", expose.features[i].label);
        console.log("Type", expose.features[i].type);
        console.log("Access", expose.features[i].access);
        console.log("Property", expose.features[i].property);
        if (expose.features[i].endpoint != null)
          console.log("Endpoint", expose.features[i].endpoint);
        if (expose.features[i].features != null) {
          if (expose.features[i].property != 'color_rgb' && expose.features[i].property === 'color_xy' && expose.features[i].property === 'color' && expose.features[i].property === 'simulated_brightness') {
            console.log("Features **Warning**");
          }
          process.stdout.write("Features ");
          for (let j = 0; j < expose.features[i].features.length; j++) {
	    process.stdout.write(expose.features[i].features[j].name);
            if (j != expose.features[i].features.length - 1)
              process.stdout.write(",");
          }
          console.log('');
          //console.log("Features", expose.features[i].features);
        }
        if (expose.features[i].category != null)
          console.log("Category", expose.features[i].category);
        if (expose.features[i].unit != null)
          console.log("Unit", expose.features[i].unit);
        //if (expose.features[i].presets != null)
        //  console.log("Presets", expose.features[i].presets);

        if (expose.features[i].type === 'numeric') {
          if (expose.features[i].value_min != null) 
            console.log("MinValue", expose.features[i].value_min);
          if (expose.features[i].value_max != null) 
            console.log("MaxValue", expose.features[i].value_max);
          if (expose.features[i].value_step != null) 
            console.log("StepValue", expose.features[i].value_step);
        } else if (expose.features[i].type === 'binary') {
          console.log("OnValue", expose.features[i].value_on);
          console.log("OffValue", expose.features[i].value_off);
          if (expose.features[i].value_toggle != null)
            console.log("ToggleValue", expose.features[i].value_toggle);
        } else if (expose.features[i].type === 'enum') {
          let txt = `${expose.features[i].values.map((v) => `${v}`).join(',')}`;
          console.log("EnumValues", txt);
        } else if (expose.features[i].type === 'list') {
          let txt = `${expose.features[i].values}`;
          console.log("ListValues", txt);
        } else if (expose.features[i].type === 'text') {
        }
        console.log("**EndExpose")
      }
      console.log("**EndFeatures");
    }    
    console.log("**EndExpose")
